...
print FP "package com.sim.plugin.feature_query;\n\npublic class FeatureQuery {\n";
#print Data::Dumper->Dump([\%component_list],[qw(component_list)]);
...
foreach $key (sort keys %javafeature_list)
{
    ...
    print FP "    public static final boolean $key = $value;\n";
}
print FP "}";
close(FP);
...
foreach $key (sort keys %javadefaultval_list)
{
    ...
    print FP "    public static final $type $RealKey = $value;\n";
}
print FP "}";
close(FP);

#Generate com.sim.plugin.feature_query.jar
my $class_dir = "build/buildplus/target";
open(FP,">$class_dir/Android.mk") or die "write(): can't open $class_dir/Android.mk for write: $!";
print FP "LOCAL_PATH := \$(call my-dir)\n";
print FP "include \$(CLEAR_VARS)\n";
print FP "LOCAL_MODULE               := com.sim.plugin.feature_query\n";
print FP "LOCAL_MODULE_TAGS          := optional\n";
print FP "LOCAL_SRC_FILES            := DefaultQuery.java FeatureQuery.java\n";
print FP "include \$(BUILD_JAVA_LIBRARY)";
close(FP);
