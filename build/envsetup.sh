...
function initbuildspec()
{
    ...
    local INIT_BUILD_SPEC
    DFT_BUILD_SPEC=general
    if [ -f "build/buildplus/buildspec/"$TARGET_PRODUCT/$TARGET_QRD_BUILD_SPEC".spec" ] ; then
        DFT_BUILD_SPEC=$TARGET_PRODUCT/$TARGET_QRD_BUILD_SPEC
    else
        DFT_BUILD_SPEC=default
    fi
    if [ "${INIT_BUILD_SPEC:=$DFT_BUILD_SPEC}" != "default" ] ; then
        GEN_QRDPLUS_ENV_RET=(`perl $GEN_QRDPLUS_ENV_PL $INIT_BUILD_SPEC`)
    else
        GEN_QRDPLUS_ENV_RET=(`perl $GEN_QRDPLUS_ENV_PL`)
    fi

    export TARGET_QRD_BUILD_SPEC=$INIT_BUILD_SPEC
    echo "init spec is "$INIT_BUILD_SPEC
...
{
    local BUILD_SPEC_PATH
    if [ -d build/buildplus/buildspec/$TARGET_PRODUCT ]; then
        BUILD_SPEC_PATH=$TARGET_PRODUCT
    else
        BUILD_SPEC_PATH=default
    fi

    local -a speclist
    speclist=(`/usr/bin/find build/buildplus/buildspec/$BUILD_SPEC_PATH -name *.spec 2>/dev/null|cut -d'/' -f5|cut -d'.' -f1`)
    local index=1
    local p
    ...
    do
        ...
        if [ -z "$ANSWER" ] ; then
            export TARGET_QRD_BUILD_SPEC=$default_value
        elif [ "$ANSWER" == "0" -o "$ANSWER" == "default" ] ; then
            ...
            if [ -f "build/buildplus/buildspec/"$BUILD_SPEC_PATH/$ANSWER".spec" ] ; then
    ...
    if [ "$TARGET_QRD_BUILD_SPEC" != "default" ] ; then
        GEN_QRDPLUS_ENV_RET=(`perl $GEN_QRDPLUS_ENV_PL "$BUILD_SPEC_PATH/$TARGET_QRD_BUILD_SPEC"`)
    else
        GEN_QRDPLUS_ENV_RET=(`perl $GEN_QRDPLUS_ENV_PL`)
    fi
