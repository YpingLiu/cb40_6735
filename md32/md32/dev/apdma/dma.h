...
#define MD32_DMA_EN		0x1
#define MD32_DMA_MEM_SEL_OFFSET	1
#define MD32_DMA_ST_ADDR_OFFSET	4
#define MD32_DMA_CLK_EN 0x40000
...
typedef enum
{
    SCP_DMA_MEM_SEL_PTCM = 0,
    SCP_DMA_MEM_SEL_DTCM
} DMA_TCM_SEL;
...
#define SCP_DMA_RW_ADDR     (0x100583F0)
...
extern int scp_dma_config(unsigned int addr, unsigned int mem_sel);
extern int mt_init_dma(void);
extern void send_flag_info(void);

extern DMA_RESULT apdma_transaction(unsigned int tcm_addr, unsigned int emi_addr, unsigned int len, DMA_DIR dir);
extern DMA_RESULT apdma_transaction_no_wait(unsigned int tcm_addr, unsigned int emi_addr, unsigned int len, DMA_DIR dir);
