...
#ifdef MTK_WATCHDOG_EXTEND_TIMEOUT
...
class Dex2Oat {
 public:
  ...
  const CompilerDriver* CreateOatFile(const std::string& boot_image_option,
    ...
    #ifdef MTK_ART_COMMON
    std::unique_ptr<CompilerDriver> driver(GetCompilerDriver(compiler_options_,
    #else
...
#define CHECK_WATCH_DOG_PTHREAD_CALL(call, args, what) \
 ...
 private:
...
#ifndef MTK_WATCHDOG_EXTEND_TIMEOUT
  ...
  static void Fatal(const std::string& message) {
    ...
    LogMessage::LogLine(data, message.c_str());
#else
    Message('F', message);
#endif
    exit(1);
...
#else
#ifdef MTK_WATCHDOG_EXTEND_TIMEOUT
  // 20 minutes scaled by kSlowdownFactor.
  static const unsigned int kWatchDogTimeoutSeconds = kSlowdownFactor * 20 * 60;
#else
