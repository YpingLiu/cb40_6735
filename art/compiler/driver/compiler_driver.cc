...
#ifdef HOST_AOT
  IPO_Analysis(class_loader, dex_files, thread_pool.get(), timings);
#endif
...
void CompilerDriver::GetCodeAndMethodForDirectCall(InvokeType* type, InvokeType sharp_type,
  ...
  *direct_method = 0;
  ...
  if (!use_dex_cache && force_relocations) {
    #ifdef MTK_ART_COMMON
    use_dex_cache = MTKSetUseDexCache(use_dex_cache, method);
    #else
    bool is_in_image;
    if (IsImage()) {
...
#ifdef MTK_ART_COMMON
void CompilerDriver::ParseDexFile(jobject class_loader, const DexFile& dex_file,
                                  const std::vector<const DexFile*>& dex_files,
                                  ThreadPool* thread_pool, TimingLogger* timings) {
  TimingLogger::ScopedTiming t("Parse Dex File", timings);
  ParallelCompilationManager context(Runtime::Current()->GetClassLinker(), class_loader, this,
                                     &dex_file, dex_files, thread_pool);
  context.ForAll(0, dex_file.NumClassDefs(), *MTKParseClass(), thread_count_);
}

bool CompilerDriver::MTKSetUseDexCache(bool use_dex_cache, mirror::ArtMethod* method) {
  // Copy code from AOSP, act as AOSP.
  mirror::Class* declaring_class = method->GetDeclaringClass();
  bool is_in_image;
  if (IsImage()) {
    is_in_image = IsImageClass(method->GetDeclaringClassDescriptor());
  ...
  return use_dex_cache;
}

ClassLinker* PCMGetClassLinker(const ParallelCompilationManager* manager) {
  return manager->GetClassLinker();
}

jobject PCMGetClassLoader(const ParallelCompilationManager* manager) {
  return manager->GetClassLoader();
}

CompilerDriver* PCMGetCompiler(const ParallelCompilationManager* manager) {
  return manager->GetCompiler();
}

const DexFile* PCMGetDexFile(const ParallelCompilationManager* manager) {
  return manager->GetDexFile();
}

bool MTKSkipClass(jobject class_loader,
                  const DexFile& dex_file, mirror::Class* klass)
                  SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
