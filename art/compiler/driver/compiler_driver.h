...
class CompilerDriver {
 public:
  ...
  virtual Callback* MTKParseClass() {
    LOG(FATAL) << "UNREACHABLE";
    return nullptr;
  }

  void ParseDexFile(jobject class_loader, const DexFile& dex_file,
                    const std::vector<const DexFile*>& dex_files,
                    ThreadPool* thread_pool, TimingLogger* timings);
  virtual void IPO_Analysis(jobject class_loader, const std::vector<const DexFile*>& dex_files,
                            ThreadPool* thread_pool, TimingLogger* timings) {}
  virtual bool MTKSetUseDexCache(bool use_dex_cache, mirror::ArtMethod* method)
      SHARED_LOCKS_REQUIRED(Locks::mutator_lock_);
#endif
