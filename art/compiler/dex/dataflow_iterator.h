...
namespace art {
  ...
  class PreOrderDfsIterator : public DataflowIterator {
    public:
      ...
      explicit PreOrderDfsIterator(MIRGraph* mir_graph, bool isReverse = false)
          : DataflowIterator(mir_graph, 0, isReverse ? mir_graph->GetNumReverseReachableBlocks()
                                                     : mir_graph->GetNumReachableBlocks()) {
        // Extra setup for the PreOrderDfsIterator.
        idx_ = start_idx_;
        if (isReverse) {
          block_id_list_ = mir_graph->GetReverseDfsOrder();
        } else {
  ...
  class RepeatingPreOrderDfsIterator : public DataflowIterator {
    public:
       ...
       */
#ifdef MTK_ART_COMMON
      explicit RepeatingPreOrderDfsIterator(MIRGraph* mir_graph, bool isReverse = false)
          : DataflowIterator(mir_graph, 0, isReverse ? mir_graph->GetNumReverseReachableBlocks()
                                                     : mir_graph->GetNumReachableBlocks()) {
  ...
  class RepeatingPostOrderDfsIterator : public DataflowIterator {
    public:
       ...
       */
#ifdef MTK_ART_COMMON
      explicit RepeatingPostOrderDfsIterator(MIRGraph* mir_graph, bool isReverse = false)
          : DataflowIterator(mir_graph, 0, isReverse ? mir_graph->GetNumReverseReachableBlocks()
                                                     : mir_graph->GetNumReachableBlocks()) {
        // Extra setup for the RepeatingPostOrderDfsIterator.
        idx_ = start_idx_;
        if (isReverse) {
          block_id_list_ = mir_graph->GetReverseDfsPostOrder();
        } else {
  ...
  class ReversePostOrderDfsIterator : public DataflowIterator {
    public:
       ...
       */
#ifdef MTK_ART_COMMON
      explicit ReversePostOrderDfsIterator(MIRGraph* mir_graph, bool isReverse = false)
          : DataflowIterator(mir_graph, isReverse ? mir_graph->GetNumReverseReachableBlocks() - 1
                                                  : mir_graph->GetNumReachableBlocks() - 1, 0) {
  ...
  class RepeatingReversePostOrderDfsIterator : public DataflowIterator {
    public:
       ...
       */
#ifdef MTK_ART_COMMON
      explicit RepeatingReversePostOrderDfsIterator(MIRGraph* mir_graph, bool isReverse = false)
          : DataflowIterator(mir_graph, isReverse ? mir_graph->GetNumReverseReachableBlocks() - 1
                                                  : mir_graph->GetNumReachableBlocks() - 1, 0) {
  ...
  class PostOrderDOMIterator : public DataflowIterator {
    public:
       ...
       */
#ifdef MTK_ART_COMMON
      explicit PostOrderDOMIterator(MIRGraph* mir_graph, bool isReverse = false)
          : DataflowIterator(mir_graph, 0, isReverse ? mir_graph->GetNumReverseReachableBlocks()
                                                     : mir_graph->GetNumReachableBlocks()) {
        // Extra setup for thePostOrderDOMIterator.
        idx_ = start_idx_;
        if (isReverse) {
          block_id_list_ = mir_graph->GetReverseDomPostOrder();
        } else {
