...
MIRGraph::MIRGraph(CompilationUnit* cu, ArenaAllocator* arena)
    : reg_location_(NULL),
      ...
      i_dom_list_(NULL),
#ifdef MTK_ART_COMMON
      num_reverse_reachable_blocks_(0),
      rev_dfs_order_(NULL),
      rev_dfs_post_order_(NULL),
      rev_dom_post_order_traversal_(NULL),
      p_i_dom_list_(NULL),
#endif
...
BasicBlock* MIRGraph::NewMemBB(BBType block_type, int block_id) {
  ...
  #ifdef MTK_ART_COMMON
  bb->rev_predecessors = new (arena_) GrowableArray<BasicBlockId>(arena_,
                                                            (block_type == kEntryBlock) ? 2048 : 2,
                                                            kGrowableArrayPredecessors);
  #endif
...
#ifdef MTK_ART_COMMON
  temp_bit_vector_size_ = cu_->num_dalvik_registers + cu_->num_inlined_temps;
#else
...
#ifdef MTK_ART_COMMON
void BasicBlock::EraseGrowableArrayElement(BasicBlockId id,
                                           GrowableArray<BasicBlockId> *array) {
  GrowableArray<BasicBlockId>::Iterator iterator(array);
  while (true) {
    BasicBlockId curr_id = iterator.Next();

    if (curr_id == NullBasicBlockId) {
      break;
    }

    if (curr_id == id) {
      array->Delete(curr_id);
      break;
    }
  }
}
...
void BasicBlock::ErasePredecessor(BasicBlockId old_pred) {
  EraseGrowableArrayElement(old_pred, predecessors);
}

void BasicBlock::EraseReversePredecessor(BasicBlockId old_pred) {
  EraseGrowableArrayElement(old_pred, rev_predecessors);
}

bool BasicBlock::IsPredecessor(MIRGraph* mir_graph, BasicBlock* pred) {
  return IsPredecessor(mir_graph, pred->id);
}

bool BasicBlock::IsPredecessor(MIRGraph* mir_graph, BasicBlockId pred) {
  GrowableArray<BasicBlockId>::Iterator iter(predecessors);
  BasicBlockId temp = iter.Next();
  BasicBlock* pred_bb = mir_graph->GetBasicBlock(temp);
  for ( ; pred_bb != nullptr; pred_bb = mir_graph->GetBasicBlock(iter.Next())) {
    if (pred_bb->id == pred) {
      return true;
    }
