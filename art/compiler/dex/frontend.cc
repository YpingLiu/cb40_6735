...
__attribute__((weak))
uint32_t InitCompilerOptimizerDisableFlags(uint32_t flag) {
  return kCompilerOptimizerDisableFlags;
}

__attribute__((weak))
MIRGraph* GetMIRGraph(CompilationUnit* cu) {
  return new MIRGraph(cu, &cu->arena);
}

__attribute__((weak))
void TurnOffAdvancedOpt(CompilationUnit* cu) {
}

__attribute__((weak))
void RunPassDriver(CompilationUnit* cu) {
  PassDriverMEOpts pass_driver(cu);
  pass_driver.Launch();
}

__attribute__((weak))
bool MTKSkipCompilation(CompilationUnit& cu, std::string* skip_message,
...
static CompiledMethod* CompileMethod(CompilerDriver& driver,
...
#ifdef MTK_ART_COMMON
  kCompilerOptimizerDisableFlags =
    InitCompilerOptimizerDisableFlags(kCompilerOptimizerDisableFlags);
#endif
  ...
  #ifdef MTK_ART_COMMON
  cu.mir_graph.reset(GetMIRGraph(&cu));
  #else
...
#ifdef MTK_ART_COMMON
  if (MTKSkipCompilation(cu, &skip_message, method_idx, dex_file)) {
    return nullptr;
  }
#else
  /* For non-leaf methods check if we should skip compilation when the profiler is enabled. */
...
#ifdef MTK_ART_COMMON
bool MTKCanCompileMethod(uint32_t method_idx, const DexFile& dex_file,
                         CompilationUnit& cu) {
  return CanCompileMethod(method_idx, dex_file, cu);
}
#endif
