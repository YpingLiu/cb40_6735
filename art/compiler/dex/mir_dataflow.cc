...
bool MIRGraph::FindLocalLiveIn(BasicBlock* bb) {
  ...
  use_v = bb->data_flow_info->use_v =
      new (arena_) ArenaBitVector(arena_, cu_->num_dalvik_registers + cu_->num_inlined_temps,
                                  false, kBitMapUse);
...
#ifdef MTK_ART_COMMON
  DCHECK((dalvik_reg >= 0) && (dalvik_reg < cu_->num_dalvik_registers + cu_->num_inlined_temps));
#else
...
bool MIRGraph::DoSSAConversion(BasicBlock* bb) {
  ...
  bb->data_flow_info->vreg_to_ssa_map_exit =
      static_cast<int*>(arena_->Alloc(sizeof(int) *
                                      (cu_->num_dalvik_registers + cu_->num_inlined_temps),
                                      kArenaAllocDFInfo));
...
#ifdef MTK_ART_COMMON
  size_t num_dalvik_reg = cu_->num_dalvik_registers + cu_->num_inlined_temps;
#else
