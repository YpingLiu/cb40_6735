...
void MIRGraph::ComputeDefBlockMatrix() {
  ...
  int num_registers = cu_->num_dalvik_registers + cu_->num_inlined_temps;
#else
...
#ifdef MTK_ART_COMMON
  uint16_t temp = cu_->num_dalvik_registers + cu_->num_inlined_temps;
  DCHECK_EQ(temp_bit_vector_size_, temp);
#else
