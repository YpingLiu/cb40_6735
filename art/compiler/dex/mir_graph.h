...
struct MIR {
  ...
  struct DecodedInstruction {
    ...
    bool IsUnConditionalBranch() const {
      return (FlagsOf() == (Instruction::kUnconditional | Instruction::kBranch));
    }

    bool IsBranch() const {
      return ((FlagsOf() & Instruction::kBranch) == Instruction::kBranch);
    }

    bool IsTerminator() const {
      return  (((FlagsOf() & (Instruction::kBranch |
                              Instruction::kSwitch)) != 0));
    }
#endif
...
struct BasicBlock {
  ...
  BasicBlockId dfs_id;
#ifdef MTK_ART_COMMON
  BasicBlockId rev_dfs_id;
  BasicBlockId p_i_dom;             // Post immediate dominator.
  BlockListType rev_successor_block_list_type:4;
#endif
...
#ifdef MTK_ART_COMMON
  ArenaBitVector* p_dominators;     // Post dominator
  ArenaBitVector* p_i_dominated;    // Set nodes being immediately post dominated.
  ArenaBitVector* p_dom_frontier;   // Post dominance frontier
  // predecessors of block of reverse MIRGraph, including reverse taken/fall-through
  GrowableArray<BasicBlockId>* rev_predecessors;
#endif
  ...
  void EraseReversePredecessor(BasicBlockId old_pred);
  bool IsPredecessor(MIRGraph* mir_graph, BasicBlock* pred);
  bool IsPredecessor(MIRGraph* mir_graph, BasicBlockId pred);
  void EraseGrowableArrayElement(BasicBlockId id, GrowableArray<BasicBlockId> *array);
#endif
...
class MIRGraph {
 public:
...
#ifdef MTK_ART_COMMON
  GrowableArray<BasicBlockId>* GetReverseDfsOrder() {
    return rev_dfs_order_;
  }

  GrowableArray<BasicBlockId>* GetReverseDfsPostOrder() {
    return rev_dfs_post_order_;
  }

  GrowableArray<BasicBlockId>* GetReverseDomPostOrder() {
    return rev_dom_post_order_traversal_;
  }

  unsigned int GetNumReverseReachableBlocks() const {
    return num_reverse_reachable_blocks_;
  }
#endif
  ...
  #ifdef MTK_ART_COMMON
  // GetNumOfCodeVRs() has an risk, code_item is pointing to the dex code is fixed,
  // hence the source info cannot be changed implicitly, but num_dalvik_registers
  // will be adjusted accordingly once inlining is performed.
 ...
 protected:
  ...
  int* i_dom_list_;
#ifdef MTK_ART_COMMON
  unsigned int num_reverse_reachable_blocks_;
  GrowableArray<BasicBlockId>* rev_dfs_order_;
  GrowableArray<BasicBlockId>* rev_dfs_post_order_;
  GrowableArray<BasicBlockId>* rev_dom_post_order_traversal_;
  int* p_i_dom_list_;
#endif
