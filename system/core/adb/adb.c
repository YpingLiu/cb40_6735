        ...
        if (t->online && p->msg.arg1 != 0) {
            if((s = find_local_socket(p->msg.arg1, p->msg.arg0))) {
                ...
                if (p->msg.arg0 == 0 && s->peer && s->peer->transport != t) {
                    XLOGD("Invalid A_CLSE(0, %u) from transport %s, expected transport %s\n",
...
//@20160317 adb root for user versions
	return 0;
//@20160317 adb root for user versions
...
int adb_main(int is_daemon, int server_port)
{
    ...
    XLOGW("adb_main syc_size_enabled=%d\n", syc_size_enabled);
    if (auth_enabled)
        adb_auth_init();
    ...
    */
    ...
    if (should_drop_privileges()) {
        ...
        char local_name[30];
#ifdef MTK_ALLOW_ADBD_ROOT
        XLOGW("MTK_ALLOW_ADBD_ROOT enabled\n");
#else
