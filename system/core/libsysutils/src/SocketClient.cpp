...
void SocketClient::init(int socket, bool owned, bool useCmdNum) {
    ...
    mRefCount = 1;
    mCmdNum = 0;
    tid_CmdNum = hashmapCreate(10, int_hash, int_equals);
...
SocketClient::~SocketClient() {
    ...
    if(tid_CmdNum){
       hashmapFree(tid_CmdNum);
    }
}

int SocketClient::getCmdNum() {
     if (hashmapContainsKey(tid_CmdNum, (void*) (uintptr_t) gettid())) {
         //SLOGI("%s: gettid=%d, hashmapGet=%d,", __FUNCTION__,  gettid(), (int)(uintptr_t)hashmapGet(tid_CmdNum, (void*)(uintptr_t)gettid()));
         return (int)(uintptr_t)hashmapGet(tid_CmdNum, (void*)(uintptr_t)gettid());
     }
     return mCmdNum; 
}
