...
void update_encrypt_type_prop(int crypt_type) {
    switch(crypt_type) {
         case CRYPT_TYPE_PASSWORD:
             property_set("vold.encryption.type", "password");
             break;

         case CRYPT_TYPE_PATTERN:
             property_set("vold.encryption.type", "pattern");
             break;

         case CRYPT_TYPE_PIN:
             property_set("vold.encryption.type", "pin");
             break;
         ...
         default:
           /** @TODO better error and make sure handled by callers */
             property_set("vold.encryption.type", "error");
             break;
       }
}
...
extern char* get_usb_cmode_path();
extern bool is_nvram_in_data;
...
void copy_log_in_data_tmpfs(char* crypto_blkdev){
    char tmp_mount_point[256];
    char data_tmpfs_log_path[256];
    char cmd[256];

    snprintf(tmp_mount_point, sizeof(tmp_mount_point), "%s/tmp_mnt", DATA_MNT_POINT);
    snprintf(data_tmpfs_log_path, sizeof(data_tmpfs_log_path), "%s/data_tmpfs_log", tmp_mount_point);

    mkdir(tmp_mount_point, 0755);
    
    SLOGD("%s++", __FUNCTION__); 
    if (fs_mgr_do_mount(fstab, DATA_MNT_POINT, crypto_blkdev, tmp_mount_point)) {
      SLOGE("Error temp mounting decrypted block device. err=%d(%s)\n", errno, strerror(errno));
    } else {
      /* Success! */
      unsigned int flags = 0;

      mkdir(data_tmpfs_log_path, 0775);
      chown(data_tmpfs_log_path, AID_SHELL, AID_SYSTEM);

      flags = 0;
      if (selinux_android_restorecon(tmp_mount_point, flags) < 0) {
        SLOGE("%s: restorecon with flags(0x%x) failed for %s: %s\n", __FUNCTION__, flags, tmp_mount_point, strerror(errno));
      }

      flags = SELINUX_ANDROID_RESTORECON_FORCE|SELINUX_ANDROID_RESTORECON_RECURSE;
      if (selinux_android_restorecon(data_tmpfs_log_path, flags) < 0) {
         SLOGE("%s: restorecon with flags(0x%x) failed for %s: %s\n", __FUNCTION__, flags, data_tmpfs_log_path, strerror(errno));
      }

      snprintf(cmd, sizeof(cmd), "rm -rf %s/*", data_tmpfs_log_path); 
      system(cmd);

      snprintf(cmd, sizeof(cmd), "cp -rp %s/log_temp %s", DATA_MNT_POINT, data_tmpfs_log_path); 
      system(cmd);

      snprintf(cmd, sizeof(cmd), "%s/misc", data_tmpfs_log_path);
      mkdir(cmd, 0775);
      chown(cmd, AID_SYSTEM, AID_MISC);
      snprintf(cmd, sizeof(cmd), "cp -rp %s/misc/mblog %s/misc", DATA_MNT_POINT, data_tmpfs_log_path); 
      system(cmd);

      snprintf(cmd, sizeof(cmd), "%s/mdlog", data_tmpfs_log_path);
      mkdir(cmd, 0775);
      chown(cmd, AID_SHELL, AID_SYSTEM);


      snprintf(cmd, sizeof(cmd), "cp -rp %s/mdlog/bootupLog %s/mdlog", DATA_MNT_POINT, data_tmpfs_log_path); 
      system(cmd);

      snprintf(cmd, sizeof(cmd), "cp -rp %s/aee_exp %s", DATA_MNT_POINT, data_tmpfs_log_path); 
      system(cmd);

      if (wait_and_unmount(tmp_mount_point, true)) {
        SLOGE("%s: umount failed for %s: %s\n", __FUNCTION__, tmp_mount_point, strerror(errno));
      }
    }
    SLOGD("%s--", __FUNCTION__); 
...
static int delete_crypto_blk_dev(char *name)
{
  ...
  int retval = -1;
  int i, r;
  #define DM_DEV_REMOVE_RETRIES 10
  ...
  ioctl_init(io, DM_CRYPT_BUF_SIZE, name, 0);
  for (i = 0; i < DM_DEV_REMOVE_RETRIES; i++) {
    if (!(r = ioctl(fd, DM_DEV_REMOVE, io))) {
        break;
    }else{  
        SLOGE("Cannot remove dm-crypt device, retry(%d), rtn(%d)\n", i ,r);
    }
    usleep(500000);
  }

  if (i == DM_DEV_REMOVE_RETRIES) {
    /* We failed to remove the device, return an error */
    goto errout;
  } 
...
{
    int i, err, rc;
    int WAIT_UNMOUNT_COUNT = 20;

    if(is_meta_boot()) {
       WAIT_UNMOUNT_COUNT = 5;
    }
    ...
    for (i=0; i<WAIT_UNMOUNT_COUNT; i++) {
        ...
        if (errno == EINVAL || errno == ENOENT) {
             ...
             */
            SLOGE("unmounting %s failed: %s. but treat as succeeded", mountpoint, strerror(err));
            break;
        }
...
{
    int i;
    create_link_for_mtklogger(NULL);
...
static int cryptfs_restart_internal(int restart_main)
{
    ...
    copy_log_in_data_tmpfs(crypto_blkdev);
    if (! (rc = wait_and_unmount(DATA_MNT_POINT, true)) ) {
        ...
        if (is_nvram_in_data) {
            if (wait_and_unmount(PROTECT_1_MNT_POINT, true)) {
                return -1;
            }
            if (wait_and_unmount(PROTECT_2_MNT_POINT, true)) {
                return -1;
            }
        }
        ...
        while ((mount_rc = fs_mgr_do_mount(fstab, DATA_MNT_POINT,
            ...
            if (mount_rc == FS_MGR_DOMNT_BUSY) {
                ...
                SLOGE("Failed to mount decrypted data. err=%d(%s)\n", errno, strerror(errno));
                cryptfs_set_corrupt();
        ...
        if (is_nvram_in_data) {
            struct fstab_rec* fs_rec =  fs_mgr_get_entry_for_mount_point(fstab, PROTECT_1_MNT_POINT);
            if (fs_rec && !access(fs_rec->mount_point, F_OK) && fs_mgr_do_mount(fstab, fs_rec->mount_point, fs_rec->blk_device, 0)) {
                 SLOGE("Fail: mount '%s', errno=%d\n",fs_rec->mount_point, errno);
                 return -1;
            }
            fs_rec =  fs_mgr_get_entry_for_mount_point(fstab, PROTECT_2_MNT_POINT);
            if (fs_rec && !access(fs_rec->mount_point, F_OK) && fs_mgr_do_mount(fstab, fs_rec->mount_point, fs_rec->blk_device, 0)) {
                 SLOGE("Fail: mount '%s', errno=%d\n",fs_rec->mount_point, errno);
                 return -1;
            }
...
int cryptfs_check_passwd(char *passwd)
{
    ...
    if (rc == 0 && crypt_ftr.crypt_type != CRYPT_TYPE_DEFAULT) {
        ...
        struct sysinfo info;
        sysinfo(&info);
        password_expiry_time = info.uptime + password_max_age_seconds;
    }
...
static void update_progress(struct encryptGroupsData* data, int is_used)
{
    ...
    if (data->new_pct > data->cur_pct) {
        ...
        SLOGI("vold.encrypt_progress %s, used_blocks_already_done %" PRId64, buf, data->used_blocks_already_done);
    }
...
static int encrypt_groups(struct encryptGroupsData* data)
{
    ...
    /* In first run, encrypt the superblock of all groups 
           In second run, encrypt the other blocks of all groups */
    int cur_run = 0;
    int blocks_done = 0;
    int* block_done_fist_run = malloc(aux_info.groups * sizeof(int));
    ...
    for(cur_run = 0; cur_run < 2; cur_run++){
        if (cur_run == 0) {
            SLOGI("To prevent from incomplete encrypted /data to be fixed by e2fsck, encrypt the superblocks in all groups in first run.");
        }
        ...
        for (i = 0; i < aux_info.groups; ++i) {
            ...
            blocks_done = 0;
            if (cur_run == 0) {
               if (aux_info.bg_desc[i].bg_block_bitmap == first_block) {
                  /* the first block is bitmap. Skip it in first run */
                  block_done_fist_run[i] = 0;
                  SLOGI("The first block of group %d is bitmap, Skip it", i);
                  continue;
               }
               else {
                  block = 0;
                  block_count = 1; /* Only encrypt the superblock in first run */
               }
            }
            if (cur_run == 1) {
                if (aux_info.bg_desc[i].bg_block_bitmap == first_block) {
                   block = 0;  /* encrypt bitmap block in secnod run */
                }
                else {
                   block = 1; /* Skip the super block in second run */
            ...
            for (; block < block_count; block++) {
                ...
                if (cur_run == 0) {
                    block_done_fist_run[i] = used;
                    if (!used) {
                        SLOGW("The first block of group %d is NOT used", i);
                    }
                }
            ...
            if (flush_outstanding_data(data)) {
             ...
             if (cur_run == 1) {
                unsigned int expect_used_blocks_counts = 0;
                if (!(aux_info.bg_desc[i].bg_flags & EXT4_BG_BLOCK_UNINIT))
                    expect_used_blocks_counts = block_count - aux_info.bg_desc[i].bg_free_blocks_count;
                if (expect_used_blocks_counts != (unsigned int)(blocks_done + block_done_fist_run[i])) {
                    SLOGW("group %d not sync!. expect_used_blocks_counts %u, blocks_done %u (block_count %u, bg_free_blocks_count %u)",
                           i, expect_used_blocks_counts, blocks_done+ block_done_fist_run[i], block_count, aux_info.bg_desc[i].bg_free_blocks_count);
                }
            }
        }
    }
                                       ...
                                       off64_t previously_encrypted_upto)
{
    ...
    for (i = 0; i < aux_info.groups; ++i) {
        ...
        if (aux_info.bg_desc[i].bg_flags & EXT4_BG_BLOCK_UNINIT)
            data.tot_used_blocks -= block_count;
        else
    ...
    SLOGI("data.numblocks %" PRId64, data.numblocks);
    SLOGI("data.tot_used_blocks %" PRId64, data.tot_used_blocks);
                                       ...
                                       off64_t previously_encrypted_upto)
{
    ...
    for (i/=CRYPT_SECTORS_PER_BUFSIZE; i<numblocks; i++) {
        ...
        if (unix_read(realfd, buf, CRYPT_INPLACE_BUFSIZE) <= 0) {
            SLOGE("Error reading real_blkdev %s for inplace encrypt, errno=%d \n", real_blkdev, errno);
            goto errout;
        }
                            ...
                            int allow_reboot)
{
    ...
    if (how == CRYPTO_ENABLE_INPLACE) {
        ...
        if (fs_mgr_do_tmpfs_mount(DATA_MNT_POINT)) {
            goto error_shutting_down;
             ...
             if (is_nvram_in_data){
                 struct fstab_rec* fs_rec =  fs_mgr_get_entry_for_mount_point(fstab, PROTECT_1_MNT_POINT);
                 if (fs_rec && !access(fs_rec->mount_point, F_OK) && fs_mgr_do_tmpfs_mount(fs_rec->mount_point)) {       
                     SLOGE("Fail: mount tmpfs for '%s', errno=%d\n", fs_rec->mount_point, errno);
                     goto error_shutting_down;
                 }
                 fs_rec =  fs_mgr_get_entry_for_mount_point(fstab, PROTECT_2_MNT_POINT);
                 if (fs_rec && !access(fs_rec->mount_point, F_OK) && fs_mgr_do_tmpfs_mount(fs_rec->mount_point)) {       
    ...
    if (! rc) {
...
#ifdef MTK_EMMC_SUPPORT
        {
         ...
         update_encrypt_type_prop(crypt_type);
...
int cryptfs_get_password_type(void)
{
    ...
    update_encrypt_type_prop(crypt_ftr.crypt_type);
...
char* cryptfs_get_password()
{
    struct sysinfo info;
    sysinfo(&info);
    if ( info.uptime < password_expiry_time) {
        return password;
    } else {
        cryptfs_clear_password();
        return 0;
...
static char cryptfs_env_value[FORCE_ENCRYPT_CONFIG_MAX_LEN];
char* cryptfs_get_cfg_value(){
...
int cryptfs_set_cfg_value(char* value){
    char name[] = FORCE_ENCRYPT_CONFIG;
    unsigned int value_max_len = FORCE_ENCRYPT_CONFIG_MAX_LEN;
