...
#ifdef MTK_NTFS_OPENSOURCE_SUPPORT
            if(mountVolumeByNtfs3g(otgNodePath, getMountpoint())){
                SLOGE("usbotg: Volume mountVol %s Otg failed to mount via NTFS (%s)\n", otgNodePath, strerror(errno));
                setState(Volume::State_Idle);
            } else {
                goto __CONTINUE_MOUNT_OTG_NTFS;  
            }
#endif            
            char cmd[255] = {0};
...
__CHECK_FAT_AGAIN:
        if (Fat::check(devicePath)) {
            ...
            #endif
#ifdef MTK_NTFS_OPENSOURCE_SUPPORT
            if (mountVolumeByNtfs3g(devicePath, getMountpoint())) {
                SLOGE("%s failed to mount via NTFS (%s)\n", devicePath, strerror(errno));
            } else {
                SLOGW("Fat check fail,because this is ntfs device");
                goto __CONTINUE_MOUNT_NTFS;
            }
#endif            
...
int Volume::mountVolumeByNtfs3g(const char* devpath, const char* mntpath)
{
    int ret = 0;
    char cmd[128];

    memset(cmd, 0, 128);
    sprintf(cmd, "%s -o uid=%d,gid=%d,fmask=0007,dmask=0007 %s %s", NTFS_3G_PATH,AID_MEDIA_RW,AID_MEDIA_RW, devpath, mntpath);    
    ret = system(cmd);
    ...
    if(ret == -1 || ret == 127)
    {
        ...
        SLOGI("mountVolumeByNtfs3g success!\n");
    }
    return ret;
}
