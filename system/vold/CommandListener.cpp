...
void CommandListener::sendBroadcast(int code, const char *msg, bool addErrno) {
   int errno_backup = errno;

   FrameworkListener::sendBroadcast(code, msg, addErrno);
   if (errno) {
       SLOGE("Unable to sendBroadcast message. code=%d, message='%s'(%s)", code, msg, strerror(errno));
   }
   errno = errno_backup;
}
...
CommandListener::VoldAsyncCommand::VoldAsyncCommand(const char *cmd) :
    VoldCommand(cmd)  {
}

int CommandListener::VoldAsyncCommand::runCommand(SocketClient *cli,
                                           int argc, char **argv) {
    if (argc < 2) {
    ...
    RunCommandArgs* args = (RunCommandArgs*)malloc(sizeof(RunCommandArgs));
    args->cli = cli;
    args->instance = this;
    args->argc = argc;
    ...
    args->mCmdNum = -1;
    runCommand_async_handler(args);
#else
    if (isAsyncCmd(argv[1])) {
        pthread_t t;
        int ret;

        args->mCmdNum = cli->getCmdNum();
        ret = pthread_create(&t, NULL, runCommand_async_handler, args);
        if (ret) {
            SLOGE("Cannot create thread to do runCommand_async_handler()");
        }       
    }
    else {
        args->mCmdNum = -1;
...
void* CommandListener::VoldAsyncCommand::runCommand_async_handler(void *thread_arg) {
    RunCommandArgs* args = reinterpret_cast<RunCommandArgs *>(thread_arg);

    VoldAsyncCommand* instance = (VoldAsyncCommand*)args->instance;
    SocketClient *cli = args->cli;
    int argc = args->argc;
    char **argv = args->argv;

    if (args->mCmdNum != -1) {
      hashmapPut(cli->tid_CmdNum, (void*) (uintptr_t) gettid(), (void*) (uintptr_t) args->mCmdNum);
    }

    int ret = instance->runCommand_may_async(cli, argc, argv);
    if (ret) {
        SLOGE("runCommand_may_async(), ret=%d", ret);
    }
    ...
    if (args->mCmdNum != -1) {
       hashmapRemove(cli->tid_CmdNum, (void*) (uintptr_t) gettid());
    }
...
CommandListener::VolumeCmd::VolumeCmd() :
                 VoldAsyncCommand("volume") {
}

int CommandListener::VolumeCmd::runCommand_may_async(SocketClient *cli,
                                           int argc, char **argv) {
...
int CommandListener::StorageCmd::runCommand(SocketClient *cli,
    ...
    else if (!strcmp(argv[1], "kill_users")) {
        if (argc < 3) {
              cli->sendMsg(ResponseCode::CommandSyntaxError, "Missing Argument: killuser <mountpoint>", false);
              return 0;
        }
        char path[PATH_MAX];
        strncpy(path, argv[2], sizeof(path)-1);
        Process::killProcessesWithOpenFiles(path, 2); /* Sending SIGKILL to process */
        cli->sendMsg(ResponseCode::CommandOkay, "Storage kill_users complete", false);
    } 
    else {
...
int CommandListener::AsecCmd::runCommand_may_async(SocketClient *cli,
                                                      int argc, char **argv) {
    if (argc < 2) {
        cli->sendMsg(ResponseCode::CommandSyntaxError, "Missing Argument", false);
        return 0;
...
int CommandListener::CryptfsCmd::runCommand(SocketClient *cli,
    ...
    if (!strcmp(argv[1], "checkpw")) {
        ...
        rc = 0;
    } else if (!strcmp(argv[1], "getcfg")) {
        SLOGD("cryptfs getcfg");
        if (argc != 2) {
            cli->sendMsg(ResponseCode::CommandSyntaxError, "Usage: cryptfs getcfg", false);
            return 0;
        }   
        dumpArgs(argc, argv, -1);
  
        char* value = cryptfs_get_cfg_value();
        if (value) {
            cli->sendMsg(ResponseCode::CommandOkay, value, false);
            return 0;
        }
        rc = -1;
    } else if (!strcmp(argv[1], "setcfg")) {
          SLOGD("cryptfs setcfg");
          if (argc != 3) {
              cli->sendMsg(ResponseCode::CommandSyntaxError, "Usage: cryptfs setcfg <value>", false);
              return 0;
          }
          dumpArgs(argc, argv, -1);
          if (cryptfs_set_cfg_value(argv[2])) {
             rc = -1;
          }
          else {
             rc = 0;     
...
int CommandListener::SilkRoad::runCommand(SocketClient *cli,
    ...
    { 
        ...
        buffer[0]=0;
        strncpy(cmd_exit_code,argv[1], BUFF_SIZE-1);
        ...
        pclose(fp);
        SLOGD("run shell command fp:%p",fp);
