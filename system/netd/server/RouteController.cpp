...
/* M: HetComm Feature
   Need to synchronized with DualOnController.cpp
*/
const uint32_t HETCOMM_NETWORK_MARK1  = 90;
const uint32_t HETCOMM_NETWORK_MARK2  = 91;
...
uint32_t getRouteTableForInterface(const char* interface) {
    ...
    if (index) {
		ALOGI("erase interfaceToTable before setting, interface %s", interface);
        interfaceToTable.erase(interface);		
		ALOGI("set interfaceToTable: interface %s, index %u", interface, index);
...
WARN_UNUSED_RESULT int modifyLocalNetwork(unsigned netId, const char* interface, bool add) {
    ...
    if(table == RT_TABLE_UNSPEC)
    	{
    		 ALOGE("GetRouteTableForInterface Fail");
     	}

	  if(int result = modifyOutputInterfaceRule(interface, table, PERMISSION_NONE,INVALID_UID, INVALID_UID, add))
	  	{
	  		 ALOGE("ModifyOutputInterfaceRule for table Error = %d",result);
	  	}  
...
WARN_UNUSED_RESULT int flushRoutes(const char* interface) {
    ...
    // track of its name.
    if (!ret) {
		ALOGI("skip erase interfaceToTable: interface %s", interface);
        //interfaceToTable.erase(interface);
    }
...
int RouteController::Init(unsigned localNetId) {
    ...
    if (int ret = addDirectlyConnectedRule()) {
    	ALOGE("failed to addDirectlyConnectedRule");
        ret = 0; //return ret;
    }
    if (int ret = addUnreachableRule()) {
    	ALOGE("failed to addUnreachableRule");
        ret = 0; //return ret;
    }
...
/* M: HetComm Feature */
int RouteController::addHetComm(const char* netID, const char* physicalInterface) {
    //ip rule add fwmark 0x11 table wlan0
    char priority_str[255];
    sprintf(priority_str, "%d", RULE_PRIORITY_VPN_FALLTHROUGH);

    ALOGE("reserve table number %d & %d", HETCOMM_NETWORK_MARK1, HETCOMM_NETWORK_MARK2);
    ...
    for (size_t i = 0; i < ARRAY_SIZE(IP_VERSIONS); ++i) {
        ...
        if (android_fork_execvp(ARRAY_SIZE(argv), const_cast<char**>(argv), NULL, false, false)) {
            ALOGE("failed to execute addHetComm");
            return -EREMOTEIO;
        }
    }
    return 0;
...
int RouteController::removeHetComm(const char* netID) {
    for (size_t i = 0; i < ARRAY_SIZE(IP_VERSIONS); ++i) {
        const char* argv[] = {
            IP_PATH,
            IP_VERSIONS[i],
        ...
        if (android_fork_execvp(ARRAY_SIZE(argv), const_cast<char**>(argv), NULL, false, false)) {
            ALOGE("failed to execute removeHetComm");
            return -EREMOTEIO;
        }
    }
    return 0;
