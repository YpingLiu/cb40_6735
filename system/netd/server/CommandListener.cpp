...
NetInfoListener *CommandListener::sNetInfo = NULL;
/* M: HetComm Feature */
DualOnController *CommandListener::sDualOnCtrl = NULL;
/**
...
static const char* MANGLE_POSTROUTING[] = {
        ...
        FirewallController::LOCAL_MANGLE_POSTROUTING,
        NULL,
};
...
int CommandListener::TetherCmd::runCommand(SocketClient *cli,
    ...
    if (!strcmp(argv[1], "stop")) {
        ...
        if (!strcmp(argv[1], "interface") && !strcmp(argv[2], "list")) {
            ...
            NetAddressV6Collection *dlistv6 = sTetherCtrl->getDnsForwardersV6();
            NetAddressV6Collection::iterator itv6;
            char addrbuff[INET6_ADDRSTRLEN];
            for (itv6 = dlistv6->begin(); itv6 != dlistv6->end(); ++itv6) {
            	cli->sendMsg(ResponseCode::TetherDnsFwdTgtListResult,  inet_ntop(AF_INET6, &(*itv6), addrbuff, INET6_ADDRSTRLEN), false);
            	ALOGD("[runCommand] addrbuff = %s", addrbuff);
            }
...
int CommandListener::IPv6TetherCmd::runCommand(SocketClient *cli, int argc, char **argv) {
	...
	} else if (!strcmp(argv[1], "setroute")) {
		rc = sIPv6TetherCtrl->setIPv6SecondaryRoute(argc, argv);
	} else if (!strcmp(argv[1], "clearroute")) {
		rc = sIPv6TetherCtrl->clearIPv6SecondaryRoute(argc, argv);
	} else if (!strcmp(argv[1], "setsroute")) {
		rc = sIPv6TetherCtrl->setSourceRoute(argc, argv);
	} else if (!strcmp(argv[1], "clearsroute")) {
		rc = sIPv6TetherCtrl->clearSourceRoute(argc, argv);
...
#else
		ALOGW("IPv6 tethering feature is disabled, ingore this command %s", argv[1]);
#endif
...
CommandListener::NetInfoCmd::NetInfoCmd() :
                 NetdCommand("NetInfo") {
}

int CommandListener::NetInfoCmd::runCommand(SocketClient *cli, int argc, char **argv) {
     int rc = 0;

    if (argc < 3) {
		ALOGE("NetInfoCmd argc error");
        cli->sendMsg(ResponseCode::CommandSyntaxError, "NetInfoCmd Missing argument", false);
        return 0;
    }

    if (!strcmp(argv[1], "setsip")) {
        if(argc < 8){
			ALOGE("NetInfoCmd argc error");
        	cli->sendMsg(ResponseCode::CommandSyntaxError, "NetInfoCmd Missing argument", false);
        	return 0;            
		}   
		ALOGI("NetInfoCmd setsip start");
        rc = sNetInfo->setSipInfo(argc, argv);
    } else if (!strcmp(argv[1], "getsip")){
    	rc = sNetInfo->getSipInfo(argv[2],argv[3],argv[4], cli);
		return 0;
    } else if (!strcmp(argv[1], "clearsip")){
    	rc = sNetInfo->clearSipInfo(argv[2]);
	} else {
        cli->sendMsg(ResponseCode::CommandSyntaxError, "Unknown NetInfo cmd", false);
        return 0;
    }

    if (!rc) {
        cli->sendMsg(ResponseCode::CommandOkay, "NetInfo operation succeeded", false);
    } else {
        cli->sendMsg(ResponseCode::OperationFailed, "NetInfo operation failed", true);
    }
...
int CommandListener::FirewallCmd::runCommand(SocketClient *cli, int argc,
    ...
    if (!strcmp(argv[1], "set_pkg_uid_rule")) {
        if (argc != 4) {
            cli->sendMsg(ResponseCode::CommandSyntaxError,
                         "Usage: firewall set_pkg_uid_rule <1000> <allow|deny>",
                         false);
            return 0;
        }
        ...
        int res = sFirewallCtrl->setPkgUidRule(uid, rule);
        return sendGenericOkFail(cli, res);
    }
    ...
    if (!strcmp(argv[1], "clear_udp_forwarding")) {
        ...
        int res = 0;
        res |= sFirewallCtrl->clearUdpForwarding(inInterface, extInterface);
        return sendGenericOkFail(cli, res);
    }
    ...
    if (!strcmp(argv[1], "get_usb_client")) {
        ...
        int res = 0;
        res |= sFirewallCtrl->getUsbClientIp(argv[2]);
        return sendGenericOkFail(cli, res);
    }
    if (!strcmp(argv[1], "set_nsiot_firewall")) {
        int res = sFirewallCtrl->setNsiotFirewall();
        return sendGenericOkFail(cli, res);
    }
    if (!strcmp(argv[1], "clear_nsiot_firewall")) {
        int res = sFirewallCtrl->clearNsiotFirewall();
        return sendGenericOkFail(cli, res);
    }
	...
	if(!strcmp(argv[1], "set_volte_nsiot_firewall"))
		{   
		 ...
		 if (argc != 3) {
		    ...
		    int res = sFirewallCtrl->setVolteNsiotFirewall(argv[2]);
			return sendGenericOkFail(cli, res);
		}
	...
	if(!strcmp(argv[1], "clear_volte_nsiot_firewall"))
		{  
		  ...
		  if (argc != 3) {
		    ...
		    int res = sFirewallCtrl->clearVolteNsiotFirewall(argv[2]);
			return sendGenericOkFail(cli, res);
		}
...
int CommandListener::NetworkCommand::runCommand(SocketClient* client, int argc, char** argv) {
    ...
    //    0        1          2       3             4
    // network fwmark  add   <interface> <fwmark>
    // network fwmark remove <interface> <fwmark>
    if (!strcmp(argv[1], "fwmark")) {
        if (argc != 5) {
            return syntaxError(client, "Missing argument");
        }
          if (!strcmp(argv[2], "add")) {
            if (int ret = sNetCtrl->setFwmarkPolicyRule(argv[3], atoi(argv[4]), true)) {
                return operationError(client, "setFwmarkPolicyRule()=true failed", ret);
            }
        } else if (!strcmp(argv[2], "remove")) {
            if (int ret = sNetCtrl->setFwmarkPolicyRule(argv[3], atoi(argv[4]), false)) {
                return operationError(client, "setFwmarkPolicyRule()=false failed", ret);
            }
        } else {
            return syntaxError(client, "Unknown argument");
        }
...
CommandListener::PPPOEctlCmd::PPPOEctlCmd() :
                 NetdCommand("pppoectl") {
}
int CommandListener::PPPOEctlCmd::runCommand(SocketClient *cli, int argc, char **argv) {
     int rc = 0;

    if (argc < 2) {
		ALOGD("PPPOE ctrlcmd argc error");
        cli->sendMsg(ResponseCode::CommandSyntaxError, "PPPOEctlCmd Missing command", false);
    ...
    if (!rc) {
        cli->sendMsg(ResponseCode::CommandOkay, "PPPOE operation succeeded", false);
    } else {
        cli->sendMsg(ResponseCode::OperationFailed, "PPPOE failed", true);
    }
...
/* M: HetComm Feature */
CommandListener::DualOnCmd::DualOnCmd() :
                 NetdCommand("dualOn") {
}

int CommandListener::DualOnCmd::runCommand(SocketClient *cli, int argc,
        char **argv) {
    // 0        1        2            3            4            5
    // dualOn    add        <iface1>    <iface2>    <ratio1>    <ratio2>
    // dualOn    remove
    int rc = 0;
	#ifdef HETCOMM_SUPPORT
		ALOGD("HETCOMM_SUPPORT is enabled");
	#else
		ALOGD("HETCOMM_SUPPORT is disabled");
	#endif

    if (!strcmp(argv[1], "add") && argc >= 6) {
        rc = sDualOnCtrl->setHetCommInterface(argv[2], argv[3], argv[5]);
    } else if (!strcmp(argv[1], "remove") && argc >= 2) {
        rc = sDualOnCtrl->removeHetCommInterface();
    } else if (!strcmp(argv[1], "setratio") && argc >= 4) {
        rc = sDualOnCtrl->setHetCommRatio(argv[3]);
    } else {
        cli->sendMsg(ResponseCode::CommandSyntaxError, "Unknown DualOnCmd cmd", false);
        return 0;
    }

    if (!rc) {
        cli->sendMsg(ResponseCode::CommandOkay, "DualOnCmd operation succeeded", false);
    } else {
        cli->sendMsg(ResponseCode::OperationFailed, "DualOnCmd operation failed", true);
    }
