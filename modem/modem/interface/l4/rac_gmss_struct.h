...
typedef struct 
{
    LOCAL_PARA_HDR
    mmdc_mode_enum mode; 
} rac_gmss_mmdc_mode_update_req_struct;
...
} rac_gmss_mmdc_camp_on_status_ind_struct;

typedef struct
{
    LOCAL_PARA_HDR
    mmdc_recovery_search_enum trigger_type;
} rac_gmss_mmdc_recovery_search_req_struct;

typedef struct
{
    LOCAL_PARA_HDR
    plmn_search_abort_cause_enum cause;
} rac_gmss_plmn_search_abort_req_struct;

typedef struct {
    LOCAL_PARA_HDR
    plmn_search_abort_proc_status_enum status;          /* proc_status_enum */
} rac_gmss_plmn_search_abort_cnf_struct;

typedef struct
{
    LOCAL_PARA_HDR
    rat_enum cell_rat;
} rac_gmss_mmdc_plmn_loss_ind_struct;
#endif /* __SGLTE__ */
...
} rac_gmss_ue_oplmn_update_req_struct;
...
typedef struct {
    LOCAL_PARA_HDR
    kal_bool is_wcdma_mode_activated; //91 only
    kal_bool is_tdscdma_mode_activated; //91 only
...
typedef l4crac_set_roaming_data_config_req_struct rac_gmss_roaming_data_config_req_struct;

typedef l4crac_nw_reg_status_ind_struct rac_gmss_reg_status_ind_struct;
