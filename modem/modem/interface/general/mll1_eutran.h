...
*   freq_done_num - the freq. num that EL1 has finished measurement. (valid in LTE ocnnected).
*****************************************************************************/
typedef struct
{
   eutran_time_struct start_time;
...
* PARAMETERS
*   LOCAL_PARA_HDR  
*   is_self_tick   - C2K phase1, ll1 does not communicate with C2K, So set to TRUE, LL1 should tick standby by itself
*   rat_status     - RAT status in EL1 
*****************************************************************************/
typedef struct
{
   LOCAL_PARA_HDR
   kal_bool            is_self_tick;
...
{  /* measure req*/
   ...
   kal_bool                            is_self_tick;        /* C2K phase1, ll1 does not communicate with C2K, So set to TRUE, LL1 should tick standby by itself */
}mll1_el1_info_database_struct;
#if 0
/* under construction !*/
/* under construction !*/
...
*   freq_done_num - how many freq mesaurement has been done, el1 can use (use_num-freq_done_num) * 480ms periods in this run.
* RETURNS
*   none
* GLOBALS AFFECTED
*   none
*****************************************************************************/
void mll1_el1_expand_active_gap_pattern_ind(eutran_time_struct* start_time,kal_uint8 use_num,kal_uint8 skip_num, kal_uint8 freq_done_num);
