...
void phb_main(ilm_struct *ilm_ptr)
{
    ...
    if ((ilm_ptr->msg_id == MSG_ID_NVRAM_READ_CNF) ||
        ...
        (ilm_ptr->msg_id == MSG_ID_SIM_SEARCH_RECORD_CNF) ||
    #endif
...
/* under construction !*/
        ...
        switch (ilm_ptr->msg_id)
        {
            ...
            case MSG_ID_SIM_FILE_INFO_CNF:
                access_id = ((sim_file_info_cnf_struct*) ilm_ptr->local_para_ptr)->access_id;
                break;

        #ifdef __SIM_SEARCH_RECORD_SUPPORT__ 
            case MSG_ID_SIM_SEARCH_RECORD_CNF:
                access_id = ((sim_search_record_cnf_struct*) ilm_ptr->local_para_ptr)->access_id;
                break;
...
kal_bool phb_reset_for_init(void)
{
   ...
   phb_context_selection(SIM1);
#endif
   phb_reset();

#if defined(__GEMINI__) || defined(__SGLTE__)
   phb_context_selection(SIM2);
   phb_reset();	  
#endif
