...
static void phb_sim_search_record_cnf(ilm_struct *ilm_ptr, control_block_type *control_block);
#endif
 ...
 *  KAL_TRUE if success, KAL_FALSE otherwise.
 *****************************************************************************/
kal_bool phb_sim_search_record(control_block_type *control_block, data_desc_enum type)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    kal_trace(TRACE_FUNC, FUNC_PHB_SIM_SEARCH_RECORD);

    ASSERT(control_block != NULL);

    if (control_block->IO_stage == IO_NONE)
    {
        sim_search_record_req_struct *sim_seek_record_req;

        sim_seek_record_req = (sim_search_record_req_struct*) construct_local_para(
                                                            sizeof(sim_search_record_req_struct),
                                                            TD_CTRL);
        sim_seek_record_req->file_idx = control_block->primary_ID;
    #ifdef __PHB_USIM_SUPPORT__
        kal_mem_cpy(sim_seek_record_req->path, control_block->path, 6);
    #endif 
        sim_seek_record_req->para = 0xF801;
        sim_seek_record_req->access_id = control_block->ID;
        sim_seek_record_req->pattern_len = phb_data_desc_get_record_size(type);
        
        /* Clean data before write to SIM */
        kal_mem_set(&sim_seek_record_req->data[0], (kal_uint8) PHB_INVALID_VALUE, MAX_SUPPORT_RECORD_SIZE+6);
        /* Set data */
    #ifdef __PHB_USIM_SYNC__
        if (type == DATA_DESC_PBC)
        {
            kal_mem_set(&sim_seek_record_req->data[5], (kal_uint8) 0, sim_seek_record_req->pattern_len);
        }
        else
    #endif
        {
            kal_mem_set(&sim_seek_record_req->data[5], (kal_uint8) PHB_INVALID_VALUE, sim_seek_record_req->pattern_len);
        }

        control_block->IO_stage = IO_WAIT;
        control_block->IO_cnf_receive = phb_sim_search_record_cnf;
        phb_send_ilm(MOD_SIM, MSG_ID_SIM_SEARCH_RECORD_REQ, sim_seek_record_req, NULL);
...
static void phb_sim_search_record_cnf(ilm_struct *ilm_ptr, control_block_type *control_block)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    sim_search_record_cnf_struct *sim_search_record_cnf;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    kal_trace(TRACE_FUNC, FUNC_PHB_SIM_SEARCH_RECORD_CNF);

    ASSERT(control_block != NULL);

    sim_search_record_cnf = (sim_search_record_cnf_struct*) ilm_ptr->local_para_ptr;

    if (sim_search_record_cnf->result == SIM_CMD_SUCCESS)
    {
        if (control_block->IO_stage == IO_WAIT)
        {
            /* success */
    ...
    {
        control_block->cause = sim_search_record_cnf->result;

        phb_IO_return(control_block, ilm_ptr, KAL_FALSE);
        return;
    }
}   /* end of phb_sim_search_record_cnf */
#endif /* __SIM_SEARCH_RECORD_SUPPORT__ */
