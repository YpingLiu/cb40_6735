...
typedef struct{
    /*  This struct is used to queue informations when [LTE->C2K Stage 1] occur
     *  These informations would be used after Stage 1 AP response(EI3GPPIRAT : 1)
     */
    irat_type_enum irat_type;

    void *as_info_ptr;
     
}l4c_c2k_irat_stage1_temp_info_struct;
...
typedef struct{
    ...
    /* Used to record some info in LTE to C2K inter-RAT stage 1, these info would be used after receive ei3gppirat */
    l4c_c2k_irat_stage1_temp_info_struct lte_to_c2k_stage1_temp_info;
...
/*#ifdef __HOMEZONE_SUPPORT__   
   ...
   */
   ...
   kal_uint8    evdl_mode; // default valude: 1, event downlaod is trigered by modem
#ifdef __MOD_TCM__
...
//evshed_create, mtk02285, 20110328: remove base timer
    ...
    kal_bool is_ims_call_exist;//from EVVS
#endif

    kal_bool phb_poweron_init_sent; 
