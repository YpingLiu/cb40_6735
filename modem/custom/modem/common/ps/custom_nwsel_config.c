...
/*************************************************************************
* This information will be refered if CMCC sim card insert
...
* * This information will be refered if CMCC sim card insert
* NOTE: for 2 digit mnc PLMN, please leave the 3rd digit mnc as 0xf
* Now CMCC is no prefer HPLMN ACT
* If no particular ACT is indicated in an entry,
* it shall be assumed that all ACT supported by the ME apply to the entry
*************************************************************************/
kal_uint8 ME_STORED_EHPLMNWACT_DATA[ME_STORED_HPLMNWACT_NUM *5] = 
{
  /* mcc2|mcc1, mnc3|mcc3, mnc2|mnc1, PLMN ACT, PLMN ACT*/
  0x64, 0xf0, 0x00, 0x00, 0x00,/* 460 00, RAT: NONE*/
  0x64, 0xf0, 0x20, 0x00, 0x00,/* 460 02, RAT: NONE */
  0x64, 0xf0, 0x70, 0x00, 0x00,/* 460 07, RAT: NONE */
  0x64, 0xf0, 0x80, 0x00, 0x00 /* 460 08, RAT: NONE */
};
#endif
...
const char *CUSTOM_FR_BL_PLMN[] = {"001fff","002fff","003fff","004fff","005fff","006fff","254fff"};
const nwsel_rat_enum CUSTOM_FR_BL_RAT[] = {NWSEL_RAT_GSM_UMTS_LTE,NWSEL_RAT_GSM_UMTS_LTE,NWSEL_RAT_GSM_UMTS_LTE
    ,NWSEL_RAT_GSM_UMTS_LTE,NWSEL_RAT_GSM_UMTS_LTE,NWSEL_RAT_GSM_UMTS_LTE,NWSEL_RAT_GSM_UMTS_LTE};
...
const nwsel_rat_enum custom_rat_order_op09[NWSEL_RAT_ORDER_NUM][NWSEL_RAT_ENTRY_NUM] = {
    /* Home MCC RAT search order */
    /* If FDD-LTE cell is found, regard the environment as "Abroad" */
...
*  None
*****************************************************************************/
void nwsel_custom_config_custom_rat_order(kal_bool is_cdma2000_supported)
{
    kal_uint32 i;

    if (is_cdma2000_supported == KAL_TRUE)
    {
        for( i = 0 ; i < NWSEL_RAT_ORDER_NUM ; i ++ )
        {
            custom_rat_order[i] = &custom_rat_order_op09[i];
        }
...
void nwsel_custom_config_custom_irrdt_failure_rat_order(void)
{
    ...
    {
        custom_irrdt_failure_rat_order[i] = &custom_irrdt_failure_rat_order_op01[i];
    }
}
...
const char *CUSTOM_PS_ONLY_PLMN[] = {"45006F","46011F", /* Adding RJIL PLMNs */
                                     ...
                                     "405873","405874",INVALID_CUSTOM_WL_PLMN};
...
*  None
*****************************************************************************/
void nwsel_init_sbp_config(kal_bool is_cdma2000_supported)
{
    nwsel_custom_config_ignore_eqplmn_operator_list();
    nwsel_custom_config_hpplmn_search_mcc_list();   
    nwsel_custom_config_custom_eq_plmn_list();
    nwsel_custom_config_custom_rat_order(is_cdma2000_supported);
    nwsel_custom_config_custom_irrdt_failure_rat_order();
}
