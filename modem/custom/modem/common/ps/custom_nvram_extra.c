...
kal_uint8 csim_profile[MAX_SIM_PROFILE_LEN]={
    /* 1st byte */
    (UTK_TIMER_EXPIRY   << 5) |
    (UTK_SMSPP_DOWNLOAD << 4) |
    (UTK_MENU_SELECTION << 3) |
    (UTK_SMSPP_DOWNLOAD << 1) |
    (UTK_PROFILE_DOWNLOAD),

    /* 2nd byte */
    (SUPPORT_EXTENDED_TEXT << 7) |
    (UTK_UCS2_DISP  << 6)  |
    (UTK_UCS2_ENTRY << 5)  |
    (UTK_CMD_RST),
    ...
    /* 4th byte */
    (UTK_PROVIDELOCALINFO_NMR << 7) |
    (UTK_PROVIDELOCALINFO_MCC_MNC_LAC_CellID_IMEI << 6) |
    (UTK_SET_UP_MENU << 5) |
    (UTK_SET_UP_CALL << 4) |
    (UTK_SEND_SMS << 1) |
    (UTK_SELECT_ITEM),
    ...
    /* 5th byte */
    (UTK_EVENT_CARD_READER_STATUS << 7) |
    (UTK_EVENT_IDLE_SCREEN_AVAIL << 6) |
    (UTK_EVENT_USER_ACTIVITY << 5) |
    (UTK_EVENT_LOCAL_STATUS << 4) |
    (UTK_EVENT_CALL_DISCONNECTED << 3) |
    (UTK_EVENT_CALL_CONNECTED << 2) |
    (UTK_EVENT_MT_CALL << 1) |
    (UTK_SET_UP_EVENT_LIST),

    /* 6th byte */
    (UTK_EVENT_NET_SEARCH_MODE_CHANGE << 7) |
    (UTK_EVENT_LOCAL_CONNECTION << 6) |
    (UTK_EVENT_DISP_PARAMS_CHANGED << 5) |
    (UTK_EVENT_ACCESS_TECH_STATUS << 4) |
    (UTK_EVENT_CHANNEL_STATUS << 3) |
    (UTK_EVENT_DATA_AVAIL << 2) |
    (UTK_EVENT_BROWSER_TERM << 1) |
    (UTK_EVENT_LAN_SELECTION),
    ...
    (UTK_SET_UP_CALL << 6) |
    (UTK_GET_INKEY << 3) |
    (UTK_PROVIDELOCALINFO_DATE_TIME << 2) |
    (UTK_TIMER_MNGT_START)
    , 

    /* 9th byte */
    (UTK_PROVIDELOCALINFO_ACCESS_TECH << 7) |
    (UTK_PROVIDELOCALINFO_ENHANCEDTIMER << 4) |
    (UTK_PROVIDELOCALINFO_LANGUAGE << 3) |
    (UTK_PROVIDELOCALINFO_NMR << 2) |
    (UTK_DISP_TXT), /* 0-8 item */

    /* 10th byte */
    (SUPPORT_SOFTKEY_FOR_SELECT_MENU<<1) |
    (SUPPORT_SOFTKEY_FOR_SELECT_ITEM), 

    /* 11th byte */
    (SOFTKEY_MAX_NUM), 

    /* 12th byte */
    (UTK_GET_CHANNEL_STATUS << 4) |
    (UTK_SEND_DATA << 3) |
    (UTK_RECEIVE_DATA << 2) |
    (UTK_CLOSE_CHANNEL << 1) |
    (UTK_OPEN_CHANNEL),  
    ...
    /* 14th byte */
    (SUPPORT_DISP_HEIGHT_SIZING_PARAM << 7) |
    (DISP_HEIGHT_CHAR_NUM & 0x1f), 

    /* 15th byte */
    (SUPPORT_VAR_SIZE_FORTS << 7) |
    (DISP_WIDTH_CHAR_NUM & 0x1f), 

    /* 16th byte */
    ((WIDTH_REDUCTION & 0x7) << 5)  |
    (SUPPORT_TEXT_SCROLLING << 2) |
    (SUPPORT_TEXT_WRAPPING << 1)  |
    (SUPPORT_DISP_RESIZE), 
    ...
    /* 18th byte */
    (UTK_PROVIDELOCALINFO_ESN << 4) |
    (UTK_GET_INKEY_VAR_TIME << 3) |
    (UTK_GET_INKEY_HELP << 1) |
    (UTK_DISP_TXT_VAR_TIME), 
...
kal_bool custom_sat_clear_loci_on_sat_refresh(sat_refresh_type_enum refresh_type)
{
    ...
    switch (refresh_type)
    {
        case TYPE_SIM_INIT_AND_FULL_FILE_CHANGE:
            result = KAL_FALSE;
            break;

        case TYPE_SIM_INIT:
        case TYPE_USIM_APPLICATION_RESET:
        case TYPE_USIM_SESSION_RESET:
            result = KAL_FALSE;
            break;
            
        case TYPE_SIM_INIT_AND_FILE_CHANGE:
...
kal_bool sml_tmo_UpdateNwTimeAndCheckTempLock(kal_uint8 source, kal_uint8 nw_time_zone, nw_time_zone_time_struct *nw_time)
{
    ...
    else
    {
        ...
        smu_nvram_write(NVRAM_EF_SML_TMO_LID, SML_TMO_NVRAM_ACCESS_ID_UNLOCK_EXPIRE);
