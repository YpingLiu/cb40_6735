...
/* ------------------------------------------------------------------------- */
   ...
   #if defined(MT6268T)
#include "reg_base.h"
// modified for re-read again in TK6268T
#define HW_WRITE(ptr,data) (*(ptr) = (data))
#define HW_READ(ptr)       ( (*(volatile unsigned short *)(BSI_base+0x0000))?*(ptr):*(ptr) ) 
   #else  // defined(MT6268T)
void RegSimSimulateWrite( int adr, int data, int size, const char* file, int line );
kal_uint32  RegSimSimulateRead( int adr, int size, const char* file, int line );
...
#define HW_UPDATE(reg,mask,value)   HW_WRITE((reg), (HW_READ(reg) & ~(mask)) | ((value) & (mask)))
#define HW_SETBITS(ptr,data)        HW_WRITE( (ptr), HW_READ(ptr) | (data) )
#define HW_CLRBITS(ptr,data)        HW_WRITE( (ptr), HW_READ(ptr) & ~(data) )
/* ------------------------------------------------------------------------- */
...
#if MD_DRV_IS_CHIP_MT6583_MD1 || MD_DRV_IS_CHIP_MT6583_MD2 || MD_DRV_IS_CHIP_MT6572 || MD_DRV_IS_CHIP_MT6290 || MD_DRV_IS_CHIP_MT6595 || MD_DRV_IS_CHIP_MT6752_MD1 || MD_DRV_IS_CHIP_MT6735
