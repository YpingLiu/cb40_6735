...
//kal_uint16 L1sim_Cmd(kal_uint8  *txData,kal_uint16  txSize,kal_uint8  expSize, kal_uint8  *result,kal_uint8  *rcvSize, kal_uint8 *Error)
{
   ...
   #if 1
	kal_sprintf(sim_dbg_str, "[MOD_SIM_DRV] CMD header: %x %x %x %x %x, txSize:%d, rxSize:%d\n\r", *txData, *(txData+1), *(txData+2), *(txData+3), *(txData+4),txSize,*rcvSize);
	DRV_ICC_print_str(sim_dbg_str);
  #endif
#if defined(SIM_DEBUG_INFO)
   ...
   {
	//SIM_ActiveClk_MTK(hw_cb);		/* this will ungate SIM clk, which will make following clock resume flow be ignored */
	sim_PDNDisable_MTK(hw_cb);
   }
   else
   {
   ...
   SIM_DisAllIntr();
    ...
    if (SimCard->clkStop == KAL_TRUE)
    {
		...
		if(usim_dcb->present == KAL_FALSE) return 0;
#endif // #if defined(__SIM_HOT_SWAP_SUPPORT__)
   ...
   SIM_FIFO_Flush();
   {
	...
	if (fifo_cnt != 0) {
		kal_sprintf(sim_dbg_str, "SIM FIFO_CNT=%d after clear FIFO !!!!!!!!!!!!!", fifo_cnt);
		DRV_ICC_print_str(sim_dbg_str);
	}
   }
...
/* under construction !*/
   ...
   *rcvSize = SimCard->recDataLen;
	...
	if(SimCard->Fi/SimCard->TOUT_Factor<64)
	{
	...
	if(SimCard->Speed == Speed372) //For FTA test case
		tout+=3;
         t1 = SIM_GetCurrentTime();
		 while((SIM_GetCurrentTime()-t1) < tout); // delay 600 clock cycles (600us)
