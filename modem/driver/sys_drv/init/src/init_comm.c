...
#elif defined(MT6752)
   BuildCode = 0x6583; //HW code of MT6589 is 6583
#else
   BuildCode = 0;
#endif
...
*
* DESCRIPTION
*  Due to denali_1/2/3 share modem load.
*  we need this function for user to distinguish which chip is currently.
*  
*
* PARAMETERS
*  none
...
*************************************************************************/
#if defined(MT6735)
CHIP_NAME INT_DenaliVersion(void)
{
    volatile kal_uint16 HwCode;
    volatile kal_uint16 HwSubCode;
    HwCode = *HW_CODE;
    HwSubCode = *HW_SUBCODE;
    /* Due to denali_1/2/3 share modem load. 
    ** we need this function for user to distinguish which chip is currently. */
    if ((HwCode==0x6735) && (HwSubCode==0x8A00))
    {   //For Denali-1 HW Code is 0x6735, HW SubCode is 0x8A00
        return E_DENALI_1;
    }
    else if ((HwCode==0x6735) && (HwSubCode==0x8B00))
    {   //For Denali-2 HW Code is 0x6735, HW SubCode is 0x8B00
        return E_DENALI_2;
    }
    else if ((HwCode==0x6753) && (HwSubCode==0x8A00))
    {   //For Denali-3 HW Code is 0x6753, HW SubCode is 0x8A00
        return E_DENALI_3;
    }
    else
    {   
        ASSERT(0);
        return E_CHIP_UNKNOWN;
    }
}
#endif
...
/*************************************************************************
* FUNCTION
*  INT_Init_MT6290_Magic
*
* DESCRIPTION
*  To prevent from MT6290 AP access protected memory regions,
*  AP will wait in preloader untill the magic key has been set
*
* PARAMETERS
*  none
*
...
*************************************************************************/
#if defined(__HIF_CCIF_SUPPORT__)
void INT_Init_MT6290_Magic(void)
{
    ...
    kal_uint32 MEM_MAX_USAGE_1MB_ALIGN_SIZE = (RAM_Limit&0xFFFFF) ?
        ((RAM_Limit&(~0xFFFFF))+0x100000) : RAM_Limit;
    kal_uint32 RAM_Length = MEM_MAX_USAGE_1MB_ALIGN_SIZE - RAM_Base;
    kal_uint32 AP_MAGIC_ADDR = ROM_Base+ROM_Length+RAM_Length;
    *((volatile unsigned *)(AP_MAGIC_ADDR)) = 0x09260926;
    cp15_clean_dcache_single(AP_MAGIC_ADDR);
}
#endif
