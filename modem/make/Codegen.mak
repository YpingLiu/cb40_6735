...
ifneq ($(strip $(MODIS_CONFIG)),TRUE)
	...
	@$(IF_EXIST)  $(strip $(TSTDIR))/database/OTA_INFO.csv $(THEN)\
		($(ECHO) -n "$(CGEN) -ota_em_info $(CODEGEN_DATABASE_OUT) $(CODEGEN_DATABASE_IN) $(strip $(TSTDIR))/database/OTA_INFO.csv $(strip $(TSTDIR))/database/GLOBAL_ID_INFO.csv $(strip $(TSTDIR))/database/EM_INFO.csv" >>$(CGEN_CFG_MODEM))\
	$(CMD_END)
...
# generate meta database
	@$(ECHO) -n "$(CGEN) -meta_db_gen $(MODISBUILD_TST_DB)/$(BPMDMETADATABASE) $(CODEGEN_DATABASE_OUT)" >>$(CGEN_CFG_MODEM)
else
