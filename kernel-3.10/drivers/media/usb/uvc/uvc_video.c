		...
		struct uvc_buffer *buf, const __u8 *data, int len)
{
	...
	if (len < 2 || data[0] < 2 || data[0] > len) {
		stream->stats.frame.nb_invalid++;
		//henry add
		uvc_trace(UVC_TRACE_SUSPEND, "%s,len:%d, data[0]:%d\n", __func__, len, data[0]);
		return -EINVAL;
	}
	...
	if (buf == NULL) {
		stream->last_fid = fid;
		//henry add
 		uvc_trace(UVC_TRACE_VIDEO,"%s, buf is null\n", __func__);
		return -ENODATA;
	}
	...
	struct uvc_buffer *buf)
{
	...
	for (i = 0; i < urb->number_of_packets; ++i) {
		...
		mem = urb->transfer_buffer + urb->iso_frame_desc[i].offset;
 		uvc_trace(UVC_TRACE_SUSPEND,"%s, i:%d, data[0]:%d,data[1]:%d,len:%d\n", __func__, i, *mem, *(mem+1), urb->iso_frame_desc[i].actual_length);
		do {
			ret = uvc_video_decode_start(stream, buf, mem,
				urb->iso_frame_desc[i].actual_length);
			if (ret == -EAGAIN)
			{   //henry add
 				uvc_trace(UVC_TRACE_VIDEO,"%s, return -EAGAIN\n", __func__);
		...
		if (ret < 0)
		{
 			//uvc_trace(UVC_TRACE_VIDEO,"%s, return value:%d\n", __func__, ret);
			continue;
		}
