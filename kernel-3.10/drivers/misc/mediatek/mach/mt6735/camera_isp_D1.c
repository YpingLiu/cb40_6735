...
#define	TPIPE_DUMP_SIZE	   200
	...
	if ((ctlStart&0x01)	&& (tpipePA) &&	(ctlTcm&0x80000000)) {
		...
		pMapVa = map_va;
		LOG_ERR("pMapVa(0x%x),map_size(0x%x)", pMapVa, map_size);
		LOG_ERR("ctlStart(0x%x),tpipePA(0x%x),ctlTcm(0x%x)", ctlStart, tpipePA,	ctlTcm);
		if (pMapVa)	{
			for	(i = 0;	i <	TPIPE_DUMP_SIZE; i += 10) {
				LOG_ERR("[idx(%d)]%08X-%08X-%08X-%08X-%08X-%08X-%08X-%08X-%08X-%08X", i, pMapVa[i],	pMapVa[i+1], pMapVa[i+2], pMapVa[i+3],
	...
	/*	*/
		...
		if ((reg_val & 0x01) ||	(reg_val2 &	0x01) || (camsv_reg_cal[0] & 0x01) || (camsv_reg_cal[1]	& 0x01)) {
			if (MTRUE == _bFlag) {
				...
				if ((_INVALID_FRM_CNT_ == deque_buf.data[i].image.frm_cnt) || (m_LastMNum[rt_dma] > _magic)) {
				...
				if (0 != prv_tstamp_s[rt_dma]) {
					if ((prv_tstamp_s[rt_dma] >	deque_buf.data[i].timeStampS) ||
							...
							LOG_ERR("[rtbc]TS rollback,D(%d),prv\"%d.%06d\",cur\"%d.%06d\"",
								rt_dma, prv_tstamp_s[rt_dma], prv_tstamp_us[rt_dma], deque_buf.data[i].timeStampS, deque_buf.data[i].timeStampUs);
					}
				}
