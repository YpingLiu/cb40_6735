...
#define PK_DBG_FUNC(fmt, args...)    printk(KERN_DEBUG PFX  fmt, ##args)
...
u32 pinSet[3][8] = {
    ...
    //power ON
    if (On) {
//Always enable mclk1 both idx=0 and idx=1,because two camera use 1 clk
...
//===end of p1==Alvin==
        ...
        //Alvin add ==p2== 20160125==
        //Add new camera ov8858 in poweron  ,    and power on it only idx=0

        else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV8858_MIPI_RAW, currSensorName)) && (0 == pinSetIdx))
        {
            ...
            if(TRUE != hwPowerOn(SUB_CAMERA_POWER_VCAM_D, VOL_1800,mode_name))//1500
            {
                 PK_DBG("[CAMERA SENSOR] Fail to enable digital power (VCAM_D), power id = %d \n", CAMERA_POWER_VCAM_D);
                 goto _kdCISModulePowerOn_exit_;
            }
...
#if 0
        //henry add for ov9750
        else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV9750_MIPI_YUV, currSensorName)))
        {
            printk("on for ov9750.\n");
            mt_set_gpio_mode(GPIO_SPI_MOSI_PIN,GPIO_MODE_00);
            mt_set_gpio_dir(GPIO_SPI_MOSI_PIN,GPIO_DIR_OUT);
            mt_set_gpio_out(GPIO_SPI_MOSI_PIN,GPIO_OUT_ONE);
            ...
            if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_IO, VOL_1800,mode_name))
            {
                PK_DBG("[CAMERA SENSOR] Fail to enable analog power (VCAM_IO), power id = %d\n",  CAMERA_POWER_VCAM_IO );
                goto _kdCISModulePowerOn_exit_;
            }
            mdelay(1);
            //VCAM_A
            if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_A, VOL_2800, mode_name))
            {
                PK_DBG("[CAMERA SENSOR] Fail to enable digital power (VCAM_A), power id = %d \n", CAMERA_POWER_VCAM_A);
                goto _kdCISModulePowerOn_exit_;
            }
...
#endif
        //End of p2  ==Alvin==
    else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV2710_MIPI_RAW, currSensorName)))
        {
            mt_set_gpio_mode(GPIO_SPI_MOSI_PIN,GPIO_MODE_00);
            mt_set_gpio_dir(GPIO_SPI_MOSI_PIN,GPIO_DIR_OUT);
            mt_set_gpio_out(GPIO_SPI_MOSI_PIN,GPIO_OUT_ONE);
	...
	else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV4689_MIPI_RAW, currSensorName)))

{
            ...
            mdelay(1);
            //VCAM_D
            if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D, VOL_1500,mode_name))//VOL_1500
            {
                 PK_DBG("[CAMERA SENSOR] Fail to enable digital power (VCAM_D), power id = %d \n", CAMERA_POWER_VCAM_D);
                 goto _kdCISModulePowerOn_exit_;
            }
...
#if 0
    else  if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_TW9992_MIPI_YUV, currSensorName)))
        {
    printk("TVP5150 power on!");
        ...
        //Adjust the poweron timing of gc2355,and poweron only idx=1
#endif      
  else  if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_GC2355_MIPI_RAW, currSensorName))&& (1 == pinSetIdx))
        {
            mt_set_gpio_mode(GPIO_SPI_MOSI_PIN,GPIO_MODE_00);
            mt_set_gpio_dir(GPIO_SPI_MOSI_PIN,GPIO_DIR_OUT);
            mt_set_gpio_out(GPIO_SPI_MOSI_PIN,GPIO_OUT_ONE);
      ...
      // #endif
        ...
        //Alvin modify ==20160303==p4==
        //Always disable mclk1 both idx=0 and 1
        #if 0
        if(pinSetIdx == 0 ) {
            ISP_MCLK1_EN(0);
        }
        ...
        //Alvin add ==p5== 20160125==
        //Add new camera ov8858 in poweroff  ,and power off it only idx=0
        else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV8858_MIPI_RAW, currSensorName)) && (0 == pinSetIdx))
        {
	...
	{
            printk("Alvin add poweroff  in ov9750\n");
            //Set Power Pin low and Reset Pin Low
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
                if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_MODE])){PK_DBG("[CAMERA LENS] set gpio mode failed!! (CMPDN)\n");}
                if(mt_set_gpio_dir(pinSet[pinSetIdx][IDX_PS_CMPDN],GPIO_DIR_OUT)){PK_DBG("[CAMERA LENS] set gpio dir failed!! (CMPDN)\n");}
        ...
        {
            printk("Alvin add poweroff  in ov2710\n");
            //Set Power Pin low and Reset Pin Low
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
                if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_MODE])){PK_DBG("[CAMERA LENS] set gpio mode failed!! (CMPDN)\n");}
                if(mt_set_gpio_dir(pinSet[pinSetIdx][IDX_PS_CMPDN],GPIO_DIR_OUT)){PK_DBG("[CAMERA LENS] set gpio dir failed!! (CMPDN)\n");}
