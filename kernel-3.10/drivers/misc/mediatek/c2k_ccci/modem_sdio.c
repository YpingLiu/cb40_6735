...
extern unsigned int get_c2k_reserve_mem_size();
extern char *get_ap_platform();
extern phys_addr_t get_md3_img_buf_phy_addr();
extern unsigned int get_md3_img_size();
...
int sdio_modem_get_ccmni_ch(int md_id, int ccmni_idx, struct ccmni_ch *channel)
{
	...
	if ((ccmni_idx >= 0) && (ccmni_idx < CCMNI_INTF_COUNT)) {
		...
		printk("[ccci%d/net] invalid ccmni index=%d\n", md_id, ccmni_idx);
		ret = -1;
	}
...
static inline int calc_payload_len(const struct sdio_msg_head *head, unsigned int *offset)
{
	unsigned int len = 0;
...
static void sdio_tx_rx_printk(const void *buf, unsigned char type)
{
	...
	if(type == 1){
		printk(KERN_DEBUG "[C2K SDIO] write %d to channel%d/[%d]>>", 
			count, head->chanInfo, sdio_tx_cnt);
	}
	else{
		printk(KERN_DEBUG "[C2K SDIO] read %d from channel%d/[%d]<<", 
	...
	printk(KERN_DEBUG "[C2K SDIO]%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x\n", 
		*print_buf, *(print_buf+1), *(print_buf+2), *(print_buf+3), *(print_buf+4), *(print_buf+5), *(print_buf+6),*(print_buf+7),
		*(print_buf+8), *(print_buf+9), *(print_buf+10), *(print_buf+11), *(print_buf+12), *(print_buf+13), *(print_buf+14),*(print_buf+15),
		*(print_buf+16), *(print_buf+17), *(print_buf+18), *(print_buf+19), *(print_buf+20), *(print_buf+21), *(print_buf+22),*(print_buf+23));
/*
...
static void sdio_rx_check(struct sdio_modem *modem, const void *buf)
{
	...
	printk("[MODEM SDIO] rx_check chan [%d], data_len [%d]\n", *(print_buf+1), count);

		for(i = 0; i < count + sizeof(struct sdio_msg_head); i++)
		{
			if (i%16 == 0)
			{
				printk("[MODEM SDIO] rx_check ");
			}
			printk("%02X-", *(print_buf+i));
			if ((i+1)%16 ==0)
				printk("\n");
		}
		printk("\n");
									...
									unsigned char chan_num, unsigned char opt)
{
	...
	ctrl_port->chan_ctrl_msg.head.hw_head.len_hi	= 0xFF & (sizeof(struct ctrl_port_msg) >> 8);
	ctrl_port->chan_ctrl_msg.head.hw_head.len_low	= 0xFF & sizeof(struct ctrl_port_msg);
#endif
...
{
	struct sdio_modem *modem = container_of(work, struct sdio_modem, poll_hb_work);
	...
	wait_event(ctrl_port->sflow_ctrl_wait_q, (SFLOW_CTRL_DISABLE == atomic_read(&ctrl_port->sflow_ctrl_state)||(modem->status != MD_READY)));
	...
	mod_timer(&modem->heart_beat_timer, jiffies + msecs_to_jiffies(HEART_BEAT_TIMEOUT));
	
	ret = contruct_ctrl_chan_msg(ctrl_port, HEART_BEAT_MSG_ID, (hear_beat_counter>>8)&0xFF, hear_beat_counter&0xFF);
	hear_beat_counter++;
	if (ret < 0){
	...
	modem_sdio_write(modem, SDIO_WRITE_ADDR, &(ctrl_port->chan_ctrl_msg), msg_len);

	mod_timer(&modem->poll_timer, jiffies +  msecs_to_jiffies(POLLING_INTERVAL));
...
{
	struct sdio_modem *modem = container_of(work, struct sdio_modem, force_assert_work);
...
{
	struct sdio_modem *modem = container_of(work, struct sdio_modem, smem_read_done_work);
	...
	head_to_write->chanInfo		= 0x0F & MD_LOG2_CH_ID;	//port->index start from 0, chanInfo start from 1, chan0 is ctrl channel.
	head_to_write->tranHi		= 0x0F & (sizeof(modem->curr_log_blk) >> 8);
	head_to_write->tranLow		= 0xFF & sizeof(modem->curr_log_blk);
	head_to_write->hw_head.len_hi	= 0xFF & ((sizeof(modem->curr_log_blk) + sizeof(struct sdio_msg_head)) >> 8);
	head_to_write->hw_head.len_low	= 0xFF & (sizeof(modem->curr_log_blk) + sizeof(struct sdio_msg_head));

	memcpy(modem->trans_buffer + sizeof(struct sdio_msg_head), &modem->curr_log_blk, sizeof(modem->curr_log_blk));
	//ccci_mem_dump(-1, modem->trans_buffer, (sizeof(modem->curr_log_blk) + sizeof(struct sdio_msg_head)));
	modem_sdio_write(modem, SDIO_WRITE_ADDR, modem->trans_buffer, (sizeof(modem->curr_log_blk) + sizeof(struct sdio_msg_head)));
	up(&modem->sem);
	return;
}
#endif
...
static void c2k_heart_beat_timer(unsigned long data)
{
	...
	LOGPRT(LOG_ERR, "heart beat timer expired, cancel poll timer and force c2k assert now!\n");
	del_timer(&modem->poll_timer);
	schedule_work(&modem->force_assert_work);
	//start force assert timer, if this timer expires, and still no response from c2k, we should trigger md long time no response exception.
	//when receive MD_EX msg from c2k, delete this timer.
	mod_timer(&modem->force_assert_timer, jiffies + msecs_to_jiffies(FORCE_ASSERT_TIMEOUT));	
}
...
static void c2k_force_assert_timer(unsigned long data)
{
	...
	LOGPRT(LOG_ERR, "force assert timer expired, trigger md long time no response now!\n");
	if (!modem || (modem->status != MD_READY)){
		LOGPRT(LOG_ERR,  "%s: md not availiable now\n", __func__);
		goto down_out;
	}
...
int modem_on_off_ctrl_chan(unsigned char on)
{
	...
	if (on){
		...
		mod_timer(&modem->poll_timer, jiffies +  msecs_to_jiffies(POLLING_INTERVAL));		//start poll c2k's status now
	}
#endif//CONFIG_EVDO_DT_VIA_SUPPORT
up_sem:
	up(&modem->sem);
...
void modem_dtr_send(struct work_struct *work)
{
	...
	int ret = 0;
	unsigned int on = 0;
	struct sdio_modem *modem = container_of(work, struct sdio_modem, dtr_work);
	...
	wait_event(ctrl_port->sflow_ctrl_wait_q, (SFLOW_CTRL_DISABLE == atomic_read(&ctrl_port->sflow_ctrl_state)||(modem->status == MD_OFF)));
	if(down_interruptible(&modem->sem)){
		LOGPRT(LOG_ERR,  "%s %d down_interruptible failed.\n", __func__,__LINE__);
		ret =  -ERESTARTSYS;
		goto down_out;
...
{
	struct sdio_modem *modem = container_of(work, struct sdio_modem, dcd_query_work);
	struct sdio_modem_ctrl_port *ctrl_port;
	struct sdio_modem_port *port;
	unsigned char msg_len = 0;
	int ret = 0;
...
int modem_loop_back_chan(unsigned char chan_num, unsigned char opt)
{
	...
	if(ctrl_port->chan_state == CHAN_ON){
		...
		msg_len = 12;
		msg_len = (msg_len + 3) & ~0x03;  /* Round up to nearest multiple of 4 */
		modem_sdio_write(modem, SDIO_WRITE_ADDR, &(loop_back[0]), msg_len);
	}
	else{
		ret = -1;
		LOGPRT(LOG_ERR,  "%s: ctrl channel is off, please turn on first\n", __func__);
...
static int ctrl_msg_analyze(struct sdio_modem *modem)
{
	struct sdio_modem_ctrl_port *ctrl_port;
#ifndef CONFIG_EVDO_DT_VIA_SUPPORT
	struct ctrl_port_msg *msg	= modem->as_packet->buffer;
#else
	...
	switch(msg_id)
	{
		...
		case CHAN_LOOPBACK_TST_MSG_ID:
			{
#ifndef CONFIG_EVDO_DT_VIA_SUPPORT
				chan_num =*(modem->as_packet->buffer + sizeof(struct sdio_msg_head)+4);
				unsigned char res =*(modem->as_packet->buffer + sizeof(struct sdio_msg_head)+5);
#else
...
static void sdio_buffer_in_print(struct sdio_modem_port *port, struct sdio_buf_in_packet *packet)
{
	...
	printk(KERN_DEBUG "[MODEM SDIO] sdio channel%d buffer in %d bytes data<<", (port->index+1), packet->size);
	count = packet->size;
	if( count > 20){
		count = 20;
	}
	for(i = 0; i < count; i++)
	{
		printk(KERN_CONT "%x-", *(packet->buffer+i));
	}
	printk(KERN_CONT "\n");
}
...
static void sdio_write_ccmni_work(struct work_struct *work)
{
	...
	ccmni_port = container_of(work, struct sdio_modem_port, write_ccmni_work);
...
retry_get_skb:
	...
	while (fifo_total_count > 0){
		...
		fifo_read_size = kfifo_out_locked(&ccmni_port->transmit_fifo, &msg_head, todo, &ccmni_port->write_lock);

		fifo_total_count -= fifo_read_size; 

		if (fifo_read_size != todo){
			LOGPRT(LOG_ERR,  "%s %d: fail to get msg head size, just got %d bytes.\n", __func__,__LINE__, fifo_read_size);
			//todo
		}

		LOGPRT(LOG_DEBUG,  "%s %d read %d bytes, check header!(0x%x, 0x%x)\n", __func__,__LINE__, fifo_read_size, msg_head.start_flag, msg_head.chanInfo);
		//sanity check
		if ((msg_head.start_flag != 0xFE) || ((msg_head.chanInfo & 0x0F)!= ccmni_port->index + 1)){
			LOGPRT(LOG_ERR,  "%s %d check head fail when write.(0x%x, 0x%x)\n", __func__,__LINE__, msg_head.start_flag, msg_head.chanInfo);
			ret = -1;
			goto head_err_out;
		}
		...
		LOGPRT(LOG_DEBUG,  "%s %d: sdio head 0x%x, ch%d, tx_id%d, len%d\n",__func__, __LINE__, 
			msg_head.start_flag, msg_head.chanInfo & 0x0F, tx_ch, data_len);

		left = data_len;

		if (ccmni_port->index != CCMNI_AP_LOOPBACK_CH - 1){
			wait_event(ccmni_port->sflow_ctrl_wait_q, (SFLOW_CTRL_DISABLE == atomic_read(&ccmni_port->sflow_ctrl_state)||(modem->status == MD_OFF)));
		...
		do{
			...
			head_to_write->chanInfo		= (0x0F & (ccmni_port->index + 1)) + (0xF0 & (tx_ch << 4));	//port->index start from 0, chanInfo start from 1, chan0 is ctrl channel.
			head_to_write->tranHi		= 0x0F & (todo >> 8);
			head_to_write->tranLow		= 0xFF & todo;
#ifndef CONFIG_EVDO_DT_VIA_SUPPORT
			head_to_write->hw_head.len_hi	= 0xFF & ((todo + sizeof(struct sdio_msg_head)) >> 8);
			head_to_write->hw_head.len_low	= 0xFF & (todo + sizeof(struct sdio_msg_head));
#endif
			if (left > todo)
				head_to_write->tranHi |= MORE_DATA_FOLLOWING; 	//to indicate there is data left, not whole packet

			fifo_read_size = kfifo_out_locked(&ccmni_port->transmit_fifo, modem->trans_buffer + sizeof(struct sdio_msg_head), todo, &ccmni_port->write_lock);

			LOGPRT(LOG_DEBUG,  "%s %d: fifo_read_size(%d).\n", __func__, __LINE__, fifo_read_size);

			if(todo != fifo_read_size){
				LOGPRT(LOG_ERR,  "%s %d: port%d todo(%d) !=  kfifo_lock_out size(%d).\n", __func__, __LINE__,ccmni_port->index, todo, fifo_read_size);
				todo = fifo_read_size;
			}
			...
			//for loop back
			if (ccmni_port->index == CCMNI_AP_LOOPBACK_CH-1){
				memcpy(skb_put(skb, todo), modem->trans_buffer + sizeof(struct sdio_msg_head), todo);
				if (head_to_write->tranHi & 0x20 == 0){		//the last packet of skb received
					LOGPRT(LOG_INFO,  "%s: data loopback to ccmni...\n",__func__);
					sdio_tx_rx_printk(skb, 1);
					ccmni_ops.rx_callback(SDIO_MD_ID, tx_ch, skb, NULL);
				}
			}else{
				LOGPRT(LOG_DEBUG,  "%s %d: port%d sending to md(len %d).\n", __func__, __LINE__,ccmni_port->index, write_len);
				modem_sdio_write(modem, SDIO_WRITE_ADDR, modem->trans_buffer, write_len);
			}
			left -= todo;
		}while(left);
		...
		if (ccmni_port->tx_state == CCMNI_TX_STOP) {
			if (kfifo_len(&ccmni_port->transmit_fifo) <= FIFO_SIZE/2) {
				...
				ccmni_ops.md_state_callback(SDIO_MD_ID, tx_ch, TX_IRQ);
				ccmni_port->tx_state = CCMNI_TX_READY;
			} else {
				LOGPRT(LOG_INFO,  "ccmni(ch:%d) is stoped, fifo_total_count(%d).\n",
...
static void sdio_write_port_work(struct work_struct *work)
{
	...
	while(ready_data_count > 0){
		...
		if(count == 0){
			up(&port->write_sem);
			goto down_out;
		}else if (count > ONE_PACKET_MAX_SIZE){		/*md side buffer max size is 5200, so we set this limitation*/
			LOGPRT(LOG_DEBUG,  "%s %d more than packet limit data in fifo (%d)...\n", __func__,__LINE__, count);
			if (port->index == (DATA_CH_ID - 1))
				count = ONE_PACKET_MAX_SIZE;
		}
		
		left = count;
		wait_event(port->sflow_ctrl_wait_q, (SFLOW_CTRL_DISABLE == atomic_read(&port->sflow_ctrl_state)||(modem->status == MD_OFF)));
		...
		do{
			...
			msg_head->chanInfo		= 0x0F & (port->index + 1);	//port->index start from 0, chanInfo start from 1, chan0 is ctrl channel.
			msg_head->tranHi		= 0x0F & (todo >> 8);
			msg_head->tranLow		= 0xFF & todo;
#ifndef CONFIG_EVDO_DT_VIA_SUPPORT
			msg_head->hw_head.len_hi	= 0xFF & ((todo + sizeof(struct sdio_msg_head)) >> 8);
			msg_head->hw_head.len_low	= 0xFF & (todo + sizeof(struct sdio_msg_head));
#endif
			fifo_size = kfifo_out_locked(&port->transmit_fifo, modem->trans_buffer + sizeof(struct sdio_msg_head), todo, &port->write_lock);
			if(todo != fifo_size){
				LOGPRT(LOG_ERR,  "%s %d: port%d todo(%d) !=  kfifo lock out size(%d).\n", __func__, __LINE__,port->index, todo, fifo_size);
				todo = fifo_size;
			}
	...
	if (count < SDIO_WAKEUP_CHARS) {
		tty = tty_port_tty_get(&port->port);
		if(tty){
			tty_wakeup(tty);		//to inform the line discipline that driver is ready to receive more output data
...
static sdio_func1_wr(struct sdio_func *func, unsigned int addr, void *src,
	int count)
{
	int ret = 0;
	...
	while(cnt--){
		...
		if (ret){
			LOGPRT(LOG_ERR,  "%s %d: write 0x%x failed ret=%d, retry\n", __func__, __LINE__, addr, ret);
			msleep(10);
			continue;
		}
		return 0;
...
static sdio_func1_rd(struct sdio_func *func, void *dst, unsigned int addr,
	int count)
{
	int ret = 0;
	unsigned int cnt = 500;
	...
	while(cnt--){
		...
		if (ret){
			LOGPRT(LOG_ERR,  "%s %d: read 0x%x failed ret=%d, retry\n", __func__, __LINE__, addr, ret);
			msleep(10);
			continue;
		}
		return 0;
...
int sdio_pio_enable_interrupt(struct sdio_func *func)
{
    ...
    if (raw_val & (0x1<<1)) {
        LOGPRT(LOG_DEBUG,  "%s %d: The interrupt of pio-based function have been enabled\n", __func__, __LINE__);
        return 0;
    }
	...
	if(err_ret){
		...
		LOGPRT(LOG_ERR,  "%s %d: Enable pio-based function's interrupt failed\n", __func__,__LINE__);
		goto err_out;
	}else{
		LOGPRT(LOG_DEBUG,  "%s %d: The interrupt of pio-based function enabled success 0x%x\n", __func__,__LINE__,raw_val);
		raw_val =  sdio_f0_readb(func, 0x04, &err_ret);
		if(err_ret){
			LOGPRT(LOG_ERR,  "%s %d: read 0x04 err ret= %d\n", __func__,__LINE__,err_ret);
			//goto err_out;
...
int sdio_pio_disable_interrupt(struct sdio_func *func)
{
	...
	LOGPRT(LOG_DEBUG,  "%s %d: read addr(0x04) 0x%x\n", __func__,__LINE__,raw_val);
    if (raw_val & (0x1<<1)) {
        LOGPRT(LOG_DEBUG,  "%s %d: The interrupt of pio-based function have been disabled\n", __func__,__LINE__);
        return 0;
    }
	...
	if(err_ret){
		LOGPRT(LOG_ERR,  "%s %d: write 0x04 err ret= %d\n", __func__,__LINE__,err_ret);
		LOGPRT(LOG_ERR,  "%s %d: Disable pio-based function's interrupt failed\n", __func__,__LINE__);
		goto err_out;
	}else{
		LOGPRT(LOG_DEBUG,  "%s %d: The interrupt of pio-based function disabled success (0x%x)\n",  __func__,__LINE__,raw_val);
		raw_val =  sdio_f0_readb(func, 0x04, &err_ret);
		if(err_ret){
			LOGPRT(LOG_ERR,  "%s %d: read 0x04 err ret= %d\n", __func__,__LINE__,err_ret);
			//goto err_out;
...
{
	int ret = 0;
	struct sdio_modem *modem = container_of(work, struct sdio_modem, loopback_work);
	char *buf = NULL;
	unsigned int len = 0;
	int cnt = 0;
	...
	do{
		...
		head->hw_head.len_hi	= ((todo+sizeof(struct sdio_msg_head)) & 0xFF00) >> 8;
		head->hw_head.len_low	= (todo+sizeof(struct sdio_msg_head)) & 0xFF; 
	
		if (left > todo)
			head->tranHi |= 0x20; 	//to indicate there is data left, not whole packet


		memcpy(modem->trans_buffer + sizeof(struct sdio_msg_head), buf + sizeof(struct sdio_msg_head) + transed, todo);
		...
		printk(KERN_DEBUG "[C2K] write %d back to c2k\n", write_len);

		//for loop back
		modem_sdio_write(modem, SDIO_CTDR, modem->trans_buffer, write_len);
...
int dump_c2k_sdio_status(struct sdio_modem *modem)
{
		...
		if(con & 0x1){
			unsigned int i = 0;
			while(eint_reg[i][0]){
				eint_reg[i][1] = eint_read32(eint_iobase,eint_reg[i][0]);
				LOGPRT(LOG_ERR,  "eint reg[%x]=%x \n", eint_reg[i][0],eint_reg[i][1]);
				i++;
			}
		} else {
		...
		*/
        LOGPRT(LOG_ERR,  "%s %d: orig int(0x%x)\n", __func__, __LINE__, int_sts.raw_val);
        return ret;
}
EXPORT_SYMBOL(dump_c2k_sdio_status);
...
static int modem_exception_handler(struct sdio_modem *modem)
{
	...
	int db_opt = DB_OPT_DEFAULT;
	char buff[AED_STR_LEN];
	excp_msg_t *excp_msg = NULL;
	unsigned long flags;
	int show_aee = 1;
	
	excp_msg_len = calc_payload_len(&modem->msg->head, NULL);
	excp_msg_len = (excp_msg_len + sizeof(struct sdio_msg_head) + 3) & ~0x03;	//prepare for send ack back to modem

	excp_msg = (excp_msg_t *)(&modem->msg->buffer[0]);
	...
	if (excp_msg->ccci_head.data1 == C2K_MD_EX){
		...
		exception_data_dump(&excp_msg->buffer, excp_msg_len - sizeof(struct sdio_msg_head) - sizeof(ccci_msg_t));

		//parse 512B exception data
		memcpy(&modem->ex_info, &excp_msg->buffer, sizeof(EX_LOG_T));
		...
		case MD_EX_TYPE_EMI_CHECK:
			LOGPRT(LOG_INFO, "md_emi_check: %08X, %08X, %02d, %08X\n", 
		...
		case DSP_EX_FATAL_ERROR:
			...
			LOGPRT(LOG_INFO, "err_code0 = 0x%08x, err_code1 = 0x%08x\n", 
		...
		snprintf(buff, AED_STR_LEN, "md3:%s%s", ex_info, c2k_img_info_str);
	#if defined (CONFIG_MTK_AEE_FEATURE)
	if(debug_info->type == CC_MD1_EXCEPTION && debug_info->fatal_error.err_code1 == MD_EX_LTE_FATAL_ERROR){
...
*/	
	...
	sdio_pio_int_sts_reg *int_sts = &modem->int_sts;
	sdio_pio_int_mask_reg  *int_mask = &modem->int_mask;
    ...
    //for (;;) {
	ret = sdio_func1_rd(func, int_sts, SDIO_CHISR, sizeof(sdio_pio_int_sts_reg));
	if (ret){
		LOGPRT(LOG_ERR,  "%s %d: get interrupt status failed ret=%d\n", __func__, __LINE__, ret);
		goto end;
	}
    ...
    pure_int = int_sts->raw_val & (int_mask->raw_val | SDIO_CHISR_TX_CMPLT_CNT);

	LOGPRT(LOG_DEBUG,  "%s %d: pure_int(0x%x)\n", __func__, __LINE__, pure_int);

    if (!pure_int)
        goto end;

    if (modem->int_clr_ctl == SDIO_INT_CTL_W1C) {
		ret = sdio_func1_wr(func, SDIO_CHISR, &pure_int, sizeof(sdio_pio_int_sts_reg));
		if (ret){
			LOGPRT(LOG_ERR,  "%s %d: write SDIO_CHISR failed ret=%d\n", __func__, __LINE__, ret);
		}
    }
	...
	if (pure_int & SDIO_CHISR_TX_OVERFLOW) {
		//g_tx_overflow_sts = 1;
		/* Error!!!! */
		LOGPRT(LOG_ERR, "!!!! sdio pio function got TX_OVERFLOW interrupt !!!!\n");
	}
    ...
    if (pure_int & SDIO_CHISR_RX_RDY) {
		...
		if (ret){
			LOGPRT(LOG_ERR,  "%s %d: read Rx Packet Length Register failed ret=%d\n", __func__, __LINE__, ret);
		}else {
			if (((crplr&SDIO_CRPLR_RX_PKT_LEN)>>16) != int_sts->u.rx_pkt_len) {
                LOGPRT(LOG_ERR, "!!!! CHISR.RX_PKT_LEN(%d) != CRPLR.RX_PKT_LEN(%d) !!!!\n",
                   int_sts->u.rx_pkt_len, (crplr&SDIO_CRPLR_RX_PKT_LEN)>>16);
            }
			LOGPRT(LOG_DEBUG, "rx pkt len (%d)", int_sts->u.rx_pkt_len);
        }
		...
		if (0 == sdio_pio_rx_pkt(func, modem->msg, int_sts->u.rx_pkt_len)) {
			sdio_rx_cnt++;
			total_copy += int_sts->u.rx_pkt_len;
			if (total_copy > SDIO_ASSEMBLE_MAX)
			{
				LOGPRT(LOG_ERR,  "%s %d error: packet size too large.\n", __func__,__LINE__);
				total_copy = 0;
				goto end;
			}
		}
		...
		if ((modem->status == MD_EXCEPTION || modem->status == MD_EXCEPTION_ONGOING)){
			if(dump_exp_data && modem->msg->head.chanInfo == EXCP_DATA_CH_ID){
				sdio_tx_rx_printk(modem->msg, 0);
				dump_exp_data = 0;
			}
		...
		hw_len = (modem->msg->head.hw_head.len_hi << 8) + modem->msg->head.hw_head.len_low;
		...
		if (!keep_recv){
			...
			if(modem->msg->head.tranHi & MORE_DATA_FOLLOWING){					
				LOGPRT(LOG_DEBUG,  "%s %d now can begin assemble... \n", __func__,__LINE__);
				keep_recv = 1;
			}
			msg_head = (struct sdio_msg_head*)modem->as_packet->buffer;
		...
		//if (keep_recv){
		memcpy(modem->as_packet->buffer + sizeof(struct sdio_msg_head) + modem->as_packet->size, modem->msg->buffer + payload_offset, modem->data_length);
		modem->as_packet->size += modem->data_length;

		if(!(modem->msg->head.tranHi & MORE_DATA_FOLLOWING)){
			LOGPRT(LOG_DEBUG, "No data following\n");
			
			msg_head->tranHi			= (modem->as_packet->size & 0xFF00) >> 8;
			msg_head->tranLow			= modem->as_packet->size & 0xFF;
			msg_head->hw_head.len_hi 	= ((modem->as_packet->size + sizeof(struct sdio_msg_head)) & 0xFF00) >> 8;
			msg_head->hw_head.len_low 	= (modem->as_packet->size + sizeof(struct sdio_msg_head)) & 0xFF;

			LOGPRT(LOG_DEBUG, "hw_len low(0x%x), hi(0x%x)\n", msg_head->hw_head.len_low, msg_head->hw_head.len_hi);
			LOGPRT(LOG_DEBUG, "as_packet(0x%x, 0x%x)\n", *modem->as_packet->buffer, *(modem->as_packet->buffer+1));

			#if 1
			/*Just for test*/
			static int throughput_count = 0;
			if (modem->msg->head.chanInfo == CCMNI_AP_LOOPBACK_CH){
				if(!throughput_count){
					LOGPRT(LOG_INFO, "C2K throughput test begin....\n");
				}
				throughput_count++;
				if(throughput_count == 5000){
					throughput_count = 0;
					LOGPRT(LOG_INFO, "C2K throughput test end....\n");
				}
			...
			if (modem->msg->head.chanInfo == EXCP_CTRL_CH_ID && (modem->status == MD_EXCEPTION || modem->status == MD_EXCEPTION_ONGOING)/*c2k_exception*/){
				modem_exception_handler(modem);
			}
			if ((modem->msg->head.chanInfo == EXCP_MSG_CH_ID) || (modem->msg->head.chanInfo == EXCP_DATA_CH_ID))
			{
				LOGPRT(LOG_DEBUG, "excp msg/data received ch[%d]\n", modem->msg->head.chanInfo);
			}
...
retry_get_skb:
					if (!keep_skb || !skb){
						LOGPRT(LOG_DEBUG,  "%s %d request skb from kernel.\n", __func__,__LINE__);
						skb = dev_alloc_skb(1600);
						total_copy = 0;
					}
					if (!skb){
						LOGPRT(LOG_ERR,  "%s %d retry.\n", __func__,__LINE__);
						msleep(10);
						goto retry_get_skb;
					}
			...
			if((modem->msg->head.chanInfo & 0x0F) == MD_LOG2_CH_ID) {
				...
				memcpy(&modem->curr_log_blk, modem->as_packet->buffer + sizeof(struct sdio_msg_head), sizeof(modem->curr_log_blk));
				LOGPRT(LOG_INFO, "logging in %x %x\n", modem->curr_log_blk.address, modem->curr_log_blk.length);
				modem->log_blk_stamp = sched_clock();
				log_addr = ioremap_nocache(md3_mem_base+modem->curr_log_blk.address,
											modem->curr_log_blk.length<16?16:modem->curr_log_blk.length);
				if(port->inception){
					while(read_ptr<modem->curr_log_blk.length) {
						read_len = modem->curr_log_blk.length-read_ptr;
						read_len = read_len<4096?read_len:4096;
						ret = rawbulk_push_upstream_buffer(MD_LOG_CH_ID-1, log_addr+read_ptr, read_len);
						if(ret > 0)
							read_ptr += read_len;
						else
							mdelay(1);
...
retry_get_log_in_room:
						...
						if(port->sdio_buf_in_size > SDIO_BUF_IN_MAX_SIZE){
							...
							if(!packet){
								LOGPRT(LOG_ERR,  "%s %d: kzalloc packet error for log\n",__func__,__LINE__);
								port->sdio_buf_in_size -= read_len;
								mutex_unlock(&port->sdio_buf_in_mutex);
								msleep(10);
								goto retry_get_log_in_room;
							...
							if(!packet->buffer){
								LOGPRT(LOG_ERR,  "%s %d: kzalloc packet buffer error for log\n",__func__,__LINE__);
								port->sdio_buf_in_size -= read_len;
								kfree(packet);
								mutex_unlock(&port->sdio_buf_in_mutex);
								msleep(10);
							...
							LOGPRT(LOG_INFO, "logging copy from %p to %p\n", (log_addr+read_ptr), packet->buffer);
							memcpy(packet->buffer, log_addr+read_ptr, packet->size);

							list_add_tail(&packet->node, &port->sdio_buf_in_list);
							port->sdio_buf_in_num++;									
							port->sdio_buf_in = 1;
							LOGPRT(LOG_DEBUG,  "%s %d: ttySDIO%d data buffered %d for log!\n", __func__,__LINE__, index, packet->size);
							mutex_unlock(&port->sdio_buf_in_mutex);
...
#endif
			if (msg_head->chanInfo > 0 && msg_head->chanInfo < (SDIO_TTY_NR + 1))
			{
				index = msg_head->chanInfo -1; // pay attention to channel mapping with rawbulk in rawbulk_push_upstream_buffer()
				
				//because we've already processed offset info when copy data to as_packet buffer. so ignore it here.
				...
				//tranHi comes from as_packet size, no other info. audio packet size may be up to 16KB, so we should & 0xFF but not 0x0F here.
				modem->data_length =(((msg_head->tranHi & 0xFF) << 8) |
									(msg_head->tranLow & 0xFF));
				...
				if(port->inception){
				       rawbulk_push_upstream_buffer(index,(modem->as_packet->buffer + sizeof(struct sdio_msg_head) + payload_offset), (modem->data_length - payload_offset));
#if ENABLE_CCMNI
...
retry_get_buf_in_room:
						...
						if(port->sdio_buf_in_size > SDIO_BUF_IN_MAX_SIZE){
							if (modem->status == MD_EXCEPTION || modem->status == MD_EXCEPTION_ONGOING){
								...
								msleep(10);
								if(retry_cnt%20 == 0)
									LOGPRT(LOG_INFO,  "retry_get_buf_in_room: port%d, retry_cnt=%u\n",index, retry_cnt);
								retry_cnt++;
								goto retry_get_buf_in_room;
							}else{
								port->sdio_buf_in_size -= (modem->data_length - payload_offset);
								mutex_unlock(&port->sdio_buf_in_mutex);
								//LOGPRT(LOG_ERR,  "%s %d: ttySDIO%d data buffer overrun %d!\n", __func__,__LINE__, index, (modem->data_length - payload_offset));
								printk(KERN_DEBUG "[C2K] ttySDIO%d data buffer overrun %d!\n", index, (modem->data_length - payload_offset));
							}
						}
						else{
							packet = kzalloc(sizeof(struct sdio_buf_in_packet), GFP_KERNEL);
							...
							memcpy(packet->buffer, (modem->as_packet->buffer + sizeof(struct sdio_msg_head) + payload_offset), packet->size);
#if ENABLE_CHAR_DEV
							list_add_tail(&packet->node, &port->sdio_buf_in_list);
							port->sdio_buf_in_num++;
#else														
...
#endif
							port->sdio_buf_in = 1;
							LOGPRT(LOG_DEBUG,  "%s %d: ttySDIO%d data buffered %d! \n", __func__,__LINE__, index, packet->size);
							mutex_unlock(&port->sdio_buf_in_mutex);
#if ENABLE_CHAR_DEV
							wake_up_all(&port->rx_wq);
#endif
...
static void modem_sdio_irq(struct sdio_func *func)
{
	...
	if (modem->msg->head.start_flag != MSG_START_FLAG){
		LOGPRT(LOG_ERR,  "%s %d: start_flag != MSG_START_FLAG and value is 0x%x, go out.\n",
			__func__, __LINE__, modem->msg->head.start_flag);
		goto out;
	}
	...
	if (modem->msg->head.chanInfo == EXCP_CTRL_CH_ID && (modem->status == MD_EXCEPTION || modem->status == MD_EXCEPTION_ONGOING)){
		modem_exception_handler(modem);
	}
	if ((modem->msg->head.chanInfo == EXCP_MSG_CH_ID) || (modem->msg->head.chanInfo == EXCP_DATA_CH_ID)){
		LOGPRT(LOG_DEBUG, "[MODEM SDIO] excp msg/data received ch[%d]\n", modem->msg->head.chanInfo);
	}
...
retry_get_skb:
	...
	if (modem->msg->head.chanInfo > 0 && modem->msg->head.chanInfo < (SDIO_TTY_NR + 1)) {
		...
		index = modem->msg->head.chanInfo -1;
		modem->data_length = calc_payload_len(&modem->msg->head, &payload_offset);
		...
		if(port->inception){
			rawbulk_push_upstream_buffer(index,(modem->msg->buffer + payload_offset), modem->data_length);
		} else {
			tty = tty_port_tty_get(&port->port);
			if(!tty)
			{
		...
		void *buf, size_t len)
{
	...
	unsigned char ch_id = msg_head->chanInfo & 0x0F;
	unsigned char tport_id = ch_id - 1;
	int err_flag = 0;
	int ret;
	int tx_ready = -1;
...
#ifndef CONFIG_EVDO_DT_VIA_SUPPORT
	if(ch_id == CTRL_CH_ID){
		printk(KERN_DEBUG "[C2K] before wait tx done\n");
	}
	spin_lock_irqsave(&modem->status_lock, flags);
	if (modem->status == MD_OFF){
		spin_unlock_irqrestore(&modem->status_lock, flags);
	...
	if(modem->cbp_data->flow_ctrl_enable){
		if(c2k_gpio_get_value(modem->cbp_data->cbp_flow_ctrl->wait_gpio)!=modem->cbp_data->gpio_flow_ctrl_polar){
			...
			while(1){/*print added for testing,to be removed*/
				if(modem->status == MD_OFF || ((modem->status == MD_EXCEPTION || modem->status == MD_EXCEPTION_ONGOING) && (ch_id != EXCP_CTRL_CH_ID) && (ch_id != EXCP_MSG_CH_ID))){
	...
	if ((modem->status == MD_EXCEPTION || modem->status == MD_EXCEPTION_ONGOING) && (ch_id != EXCP_CTRL_CH_ID) && (ch_id != EXCP_MSG_CH_ID)){
		LOGPRT(LOG_ERR,  "%s %d: modem exception now. channel%d data is dropped\n", __func__, __LINE__, ch_id);
		sdio_tx_rx_printk(buf, 1);
		goto terminate;
	}
	...
	if(modem->cbp_data->ipc_enable){
		...
		tx_ready = asc_tx_auto_ready(modem->cbp_data->tx_handle->name, 1);
		if(tx_ready != 0)
	...
	if (func == modem->func){
		...
		LOGPRT(LOG_ERR,  "%s %d: func changed during writing, terminate\n", __func__, __LINE__);
		goto terminate;
	}
	...
	if((ch_id == DATA_CH_ID) || (ch_id == SDIO_AT_CHANNEL_NUM) || (ch_id == SDIO_AT2_CHANNEL_NUM) || (ch_id == SDIO_AT3_CHANNEL_NUM)
			|| (ch_id == EXCP_CTRL_CH_ID) || (ch_id == EXCP_MSG_CH_ID) || (ch_id == AGPS_CH_ID) || (ch_id == MD_LOG_CH_ID)) {
	...
	if((ch_id == CTRL_CH_ID)){
		pr_info("[C2K SDIO] write ctrl channel done, len = %zd\n", len);
	}
#ifndef CONFIG_EVDO_DT_VIA_SUPPORT	
			     ...
			     char *buf)
{
    char *s = buf;
	struct sdio_modem *modem = c2k_modem;
	struct cbp_platform_data *cbp_pdata;
	...
	if((cbp_pdata!=NULL) && (cbp_pdata->cbp_data_ack!=NULL )){
		...
		s += sprintf(s, "stored:[%d]\n", c2k_gpio_get_value(modem->cbp_data->cbp_flow_ctrl->wait_gpio));
	}
out:
	return (s - buf);
}
...
int modem_buffer_push(int port_num, void *buf, int count)
{
	...
	if(modem->status != MD_OFF){
		...
		if (count > (ONE_PACKET_MAX_SIZE - chars_in_fifo)){
			printk(KERN_DEBUG "[C2K] port%d too many data pending...\n", port->index);
			return -ENOMEM;
		}
		ret = kfifo_in_locked(&port->transmit_fifo, buf, count, &port->write_lock);
		queue_work(port->write_q, &port->write_work);
...
static int via_sdio_probe_func(struct sdio_modem *modem, struct sdio_func *func)
{
	...
	spin_lock_irqsave(&modem->status_lock, flags);
	if (modem->status != MD_EXCEPTION)	//when md exception, sdio will be remounted. but we should keep status not changed.
	{
...
static int sdio_pio_func_wait_rdy(struct sdio_func *func)
{
	...
	while (cnt--){
		...
		if (raw_val & (SDIO_CCIR_F_FUNC_RDY | SDIO_CCIR_G_FUNC_RDY | SDIO_CCIR_B_FUNC_RDY)){
			return 0;
		}
		msleep(100);
	}
...
int sdio_pio_give_fw_own(struct sdio_func *func, struct sdio_modem *modem)
{
	...
	if (err)
		...
		LOGPRT(LOG_INFO,  "%s %d read interrupt enable reg success 0x%x.\n",__func__,__LINE__, int_enable);
	}
...
int sdio_pio_enable_com_interrupt(struct sdio_func *func)
{
    ...
    if (raw_val & SDIO_CHLPCR_INT_EN_SET) {
        LOGPRT(LOG_ERR,  "%s %d: The interrupt of pio-based function have been enabled\n", __func__, __LINE__);
        return 0;
    }
	...
	ret = sdio_func1_wr(func, SDIO_CHLPCR, &raw_val, sizeof(raw_val));
	if (ret){
		LOGPRT(LOG_ERR,  "%s %d: Enable pio-based function's interrupt failed ret=%d\n", __func__, __LINE__, ret);
		return -1;
	}

    LOGPRT(LOG_INFO, "%s %d: Enable pio-based function's com interrupt success (0x%x)\n", __func__, __LINE__, raw_val);
...
int sdio_pio_disable_com_interrupt(struct sdio_func *func)
{
    ...
    if (!(raw_val & SDIO_CHLPCR_INT_EN_SET)) {
        LOGPRT(LOG_ERR,  "%s %d: The interrupt of pio-based function have not been enabled\n", __func__, __LINE__);
        return 0;
    }
	...
	ret = sdio_func1_wr(func, SDIO_CHLPCR, &raw_val, sizeof(raw_val));
	if (ret){
		LOGPRT(LOG_ERR,  "%s %d: Disable pio-based function's interrupt failed ret=%d\n", __func__, __LINE__, ret);
		return -1;
	}

    LOGPRT(LOG_INFO, "%s %d: Disable pio-based function's interrupt success (0x%x)\n", __func__, __LINE__, raw_val);
...
int sdio_pio_set_int_clr_ctl(struct sdio_func *func, sdio_int_clr_ctl mod)
{
    ...
    if (mod == SDIO_INT_CTL_W1C) {
		...
		if (ret){
			LOGPRT(LOG_ERR,  "%s %d: Set interrupt write 1 clear failed ret=%d\n", __func__, __LINE__, ret);
			return -1;
		}
    }
    ...
    if (mod == SDIO_INT_CTL_RC) {
		...
		ret = sdio_func1_wr(func, SDIO_CHCR, &raw_val, sizeof(raw_val));
		if (ret){
			LOGPRT(LOG_ERR,  "%s %d: Set interrupt read clear failed ret=%d\n", __func__, __LINE__, ret);
			return -1;
		}
    }
...
int sdio_pio_set_int_mask(struct sdio_func *func, int set_bits, int clr_bits)
{
	...
	for (loop = 0; loop<1; loop++){
		...
		if (ret){
			LOGPRT(LOG_ERR,  "%s %d: read SDIO_CHIER failed ret=%d\n", __func__, __LINE__, ret);
			//return -1;
		}else
			LOGPRT(LOG_INFO,  "%s %d: read SDIO_CHIER success 0x%x\n", __func__, __LINE__, mask);
		//msleep(100);
	}
    return 0;
}
...
int sdio_pio_enable_async_interrupt(struct sdio_func *func)
{
    ...
    if (raw_val & SDIO_CSDIOCSR_SDIO_INT_CTL) {
		LOGPRT(LOG_INFO,  "%s %d: The async interrupt of pio-based function have been enabled\n", __func__, __LINE__);
        return 0;
    }
	...
	if (ret){
		LOGPRT(LOG_ERR,  "%s %d: Enable pio-based function's async interrupt failed ret=%d\n", __func__, __LINE__, ret);
		return -1;
	}

	LOGPRT(LOG_INFO,  "%s %d: Enable pio-based function's async interrupt success\n", __func__, __LINE__);
...
int sdio_pio_disable_async_interrupt(struct sdio_func *func)
{
    ...
    if (!(raw_val & SDIO_CSDIOCSR_SDIO_INT_CTL)) {
		LOGPRT(LOG_INFO,  "%s %d: The async interrupt of pio-based function have been disabled\n", __func__, __LINE__);
        return 0;
    }
	...
	ret = sdio_func1_wr(func, SDIO_CSDIOCSR, &raw_val, sizeof(raw_val));
	if (ret){
		LOGPRT(LOG_ERR,  "%s %d: Disable pio-based function's async interrupt failed ret=%d\n", __func__, __LINE__, ret);
		return -1;
	}

	LOGPRT(LOG_INFO,  "%s %d: Disable pio-based function's async interrupt success\n", __func__, __LINE__);
    return 0;
}
...
int check_img_header(struct sdio_modem *modem)
{
	...
	header = (struct md_check_header *)kzalloc(sizeof(struct md_check_header), GFP_KERNEL);
	if (!header)
	{
	...
	ret = filp->f_op->read(filp, header, sizeof(struct md_check_header), &filp->f_pos);
	...
	if (ret > 0){
		...
		if(ret) {
			...
			img->img_info.product_ver 	= product_str[header->product_ver];
		...
		LOGPRT(LOG_ERR,  "%s: read c2k md img fail, cannot get check header!\n",__func__);
	}
...
static int c2k_sdio_probe_func(struct sdio_modem *modem, struct sdio_func *func)
{
	...
	if (ret){
		LOGPRT(LOG_ERR,  "%s %d: disable interrupt output own failed with ret = %d\n",__func__, __LINE__, ret);
		goto give_own;
	}
	...
	if (ret){
		LOGPRT(LOG_ERR,  "%s %d: set int write 1 clear failed with ret = %d\n",__func__, __LINE__, ret);
        goto give_own;
    }
	...
	if (ret){
		LOGPRT(LOG_ERR,  "%s %d: enable async interrupt failed with ret = %d\n",__func__, __LINE__, ret);
		goto give_own;
	}
	LOGPRT(LOG_INFO, "%s %d: sdio_pio_enable_async_interrupt done\n", __func__, __LINE__);

	ret = sdio_pio_set_int_mask(func, /*SDIO_CHIER_FW_OWN_BACK_EN|*/SDIO_CHIER_RX_RDY_EN|
          SDIO_CHIER_TX_EMPTY_EN|SDIO_CHIER_TX_UNDER_THOLD_EN|SDIO_CHIER_TX_OVERFLOW_EN|
          SDIO_CHIER_FW_INT_INDICATOR_EN|SDIO_CHIER_FIRMWARE_INT_EN, 0);
    if (ret){
		LOGPRT(LOG_ERR,  "%s %d: set interrupts' mask failed with ret = %d\n",__func__, __LINE__, ret);
		goto give_own;
	}
	...
	if (modem->status != MD_EXCEPTION)	//when md exception, sdio will be remounted. but we should keep status not changed.
	{
		...
		mutex_unlock(&port->sdio_buf_in_mutex);
		LOGPRT(LOG_INFO,  "%s: clear pending data for log channel done\n", __func__);
#endif

	}else{
		spin_unlock_irqrestore(&modem->status_lock, flags);
	}
	LOGPRT(LOG_INFO,  "%s %d: md status (%d).\n", __func__, __LINE__, modem->status);
	...
	if (ret) {
        LOGPRT(LOG_ERR,  "%s %d: enable interrupt out failed with ret = %d\n",__func__, __LINE__, ret);
        goto give_own;
    }
...
void modem_reset_handler(void)
{
	...
	spin_lock_irqsave(&modem->status_lock, flags);
	if (modem->status != MD_EXCEPTION){	//when md exception, we will trigger this reset function. but we should keep status not changed.
		modem->status = MD_OFF;
...
int sdio_modem_ccmni_send_pkt(int md_id, int tx_ch, void *data)
{
	...
	unsigned int left, todo, fifo_size, write_len;
	int chars_in_fifo = 0;

	LOGPRT(LOG_DEBUG,  "%s: enter...\n",__func__);
	if (md_id != SDIO_MD_ID){
		LOGPRT(LOG_ERR,  "%s: sdio_modem_send_pkt failed: wrong md_id.\n",__func__);
	...
	if (!ccmni_port || modem->status == 0){
		LOGPRT(LOG_ERR,  "%s: sdio_modem_send_pkt failed: ccmni port is NULL.\n",__func__);
	...
	todo = skb->len;
	msg_head = (struct sdio_msg_head*)skb_push(skb, sizeof(struct sdio_msg_head));
	msg_head->start_flag 	= 0xFE;
	msg_head->chanInfo 		= (0x0F & (ccmni_port->index + 1)) + (0xF0 & (tx_ch << 4));	
	msg_head->tranHi 		= 0x0F & (todo >> 8);
	msg_head->tranLow		= 0xFF & todo;
#ifndef CONFIG_EVDO_DT_VIA_SUPPORT
	msg_head->hw_head.len_hi = ((todo+sizeof(struct sdio_msg_head)) & 0xFF00) >> 8;
	msg_head->hw_head.len_low = (todo+sizeof(struct sdio_msg_head)) & 0xFF;
#endif
	data_len = skb->len;	//as skb->len already included sdio_msg_head after skb_push

	data = (void *)(skb->data);

	LOGPRT(LOG_DEBUG,  "%s %d msg_head(0x%x, 0x%x, 0x%x, 0x%x), data_len(%d), todo(%d)\n", __func__,__LINE__, msg_head->start_flag, msg_head->chanInfo, msg_head->tranHi, msg_head->tranLow, data_len, todo);

	LOGPRT(LOG_DEBUG,  "%s %d msg_head addr(%p) data addr(%p)\n", __func__,__LINE__, (int *)msg_head, (int *)data);
	...
	if(data_len > (FIFO_SIZE - chars_in_fifo)){
		...
		/* stop tx queue */
		LOGPRT(LOG_INFO,  "ccmni port %d is stopping.\n", ccmni_port->index);
	...
	if(modem->status != MD_OFF){
		spin_unlock_irqrestore(&modem->status_lock, flags);
		ret = kfifo_in_locked(&ccmni_port->transmit_fifo, data, data_len, &ccmni_port->write_lock);
		if(ret != data_len)
		{
			LOGPRT(LOG_ERR,  "%s %d not all data pushed into fifo(%d/%d)!\n", __func__,__LINE__, ret, data_len);
		}
		//free skb if we return tx OK
		dev_kfree_skb_any(skb);
		LOGPRT(LOG_DEBUG,  "%s %d push %d bytes to fifo!(0x%x, 0x%x, 0x%x, 0x%x)\n", __func__,__LINE__, ret, *(char *)data, *((char *)data+1), *((char *)data+2), *((char *)data+3));
...
{
    struct sdio_modem_port *port = (struct sdio_modem_port *)file->private_data;
    unsigned long flags;
	struct sdio_buf_in_packet *packet = NULL;
...
static struct file *open_img_file(char *name, int *sec_fp_id)
{
	...
	LOGPRT(LOG_INFO, "sec_open fd = (%d)!\n", fp_id); 

	if(sec_fp_id != NULL)
		*sec_fp_id = fp_id;

	LOGPRT(LOG_INFO, "sec_open file ptr = (%p)!\n", osal_get_filp_struct(fp_id)); 

	return (struct file *)osal_get_filp_struct(fp_id);
#else
	LOGPRT(LOG_INFO, "std_open!\n");
...
{
#ifdef ENABLE_MD_IMG_SECURITY_FEATURE
	LOGPRT(LOG_INFO, "sec_close (%d)!\n", sec_fp_id);
	osal_filp_close(sec_fp_id);
#else
	LOGPRT(LOG_INFO, "std_close!\n");
	filp_close(filp_id, current->files);
#endif
}


static int signature_check(char* file_path, unsigned int *sec_tail_length)
{
	...
	if( masp_ccci_signfmt_verify_file(file_path, &bypass_sec_header_offset, &sec_total_len) == 0 ){
		...
		//-- check return value
		LOGPRT(LOG_INFO, "sign check ret value 0x%x, 0x%x!\n", bypass_sec_header_offset, sec_total_len);
		if(bypass_sec_header_offset > sec_total_len){
			LOGPRT(LOG_ERR, "sign check fail(0x%x, 0x%x!)!\n", bypass_sec_header_offset, sec_total_len);
			return -CCCI_ERR_LOAD_IMG_SIGN_FAIL;
		} else {
			LOGPRT(LOG_INFO, "sign check success(0x%x, 0x%x)!\n", bypass_sec_header_offset, sec_total_len);
		...
		LOGPRT(LOG_INFO, "sign check fail!\n");
		return -CCCI_ERR_LOAD_IMG_SIGN_FAIL;
	}
}


static int check_md_header(void *parse_addr, struct ccci_image_info *image)
{
	int ret;
	...
	struct md_check_header *head = NULL;
	char *img_str = c2k_img_info_str;
	
	header_size = *(((unsigned int *)parse_addr)-1);
	LOGPRT(LOG_INFO, "MD image header size = %d\n", header_size);

	while(!head){
		head = (struct md_check_header *)kzalloc(sizeof(struct md_check_header), GFP_KERNEL);
		if(head)
			break;
		LOGPRT(LOG_INFO, "alloc check_header fail, retry!\n");
		msleep(10);
	}
	...
	LOGPRT(LOG_INFO, "**********************MD image check %d***************************\n", (int)sizeof(struct md_check_header));
	ret = strncmp(head->check_header, MD_HEADER_MAGIC_NO, 12);
	if(ret) {
		LOGPRT(LOG_INFO, "md check header not exist!\n");
		ret = 0;
	}
	else {
		image->ap_info.image_type	= type_str[head->image_type];
		image->ap_info.platform		= get_ap_platform();
		image->ap_info.mem_size		= get_c2k_reserve_mem_size();
		...
		LOGPRT(LOG_INFO, "(MD)[type]=%s, (AP)[type]=%s\n", image->img_info.image_type, image->ap_info.image_type);
		LOGPRT(LOG_INFO, "(MD)[plat]=%s, (AP)[plat]=%s\n", image->img_info.platform, image->ap_info.platform);
		LOGPRT(LOG_INFO, "(MD)[build_ver]=%s, [build_time]=%s\n",image->img_info.build_ver, image->img_info.build_time);
		LOGPRT(LOG_INFO, "(MD)[product_ver]=%s\n",image->img_info.product_ver);
		sprintf(img_str, "MD:%s*%s*%s*%s*%s\nAP:%s*%s*%08x (MD)%08x\n",
					image->img_info.image_type, image->img_info.platform, 
					image->img_info.build_ver, image->img_info.build_time,
					image->img_info.product_ver, image->ap_info.image_type,
					image->ap_info.platform, image->ap_info.mem_size,
					image->img_info.mem_size);

		LOGPRT(LOG_INFO, "%s\n", img_str);
	}

	LOGPRT(LOG_INFO, "**********************MD image check***************************\n");
	kfree(head);
...
static int load_cipher_firmware(int fp_id,
					struct ccci_image_info *img,
					char *addr,
					unsigned int cipher_img_offset, 
					unsigned int cipher_img_len,
					int need_check_header)
{
	int ret;
	void *addr_bak = addr;
	...
	if (addr==NULL) {
		LOGPRT(LOG_INFO, "addr NULL, exit!\n");
	...
	if(SEC_OK != masp_ccci_decrypt_cipherfmt(fp_id, cipher_img_offset, (char*)addr, cipher_img_len, &data_offset) ) {
		LOGPRT(LOG_INFO, "cipher image decrypt fail!\n");
	...
	LOGPRT(LOG_INFO, "img->size %d, img->offset %d!\n", img->size, img->offset);

	if(need_check_header)
		ret = check_md_header(addr, img);
...
static int load_std_firmware(char *img_addr, struct file *filp, 
				struct ccci_image_info *img, int need_check_header)
{
	void __iomem *	start;
	int		ret = 0;
	int		check_ret = 0;
	...
	while (1) {
		ret = (int)filp->f_op->read(filp, img_addr + read_size, size_per_read, &filp->f_pos);
		
		if ((ret < 0) || (ret > size_per_read) || ((ret == 0) && (read_size == 0))) { //make sure image size isn't 0
			LOGPRT(LOG_INFO, "image read fail: size=%d\n", ret);
			...
			LOGPRT(LOG_INFO, "%s, image size=0x%x, read size:%d, tail:%d\n", 
	...
	if(need_check_header) {
		end_addr = img_addr + img->size;
		if((check_ret = check_md_header(end_addr, img)) < 0) {
			ret = check_ret;
			goto error;
		}
    }
	...
	set_fs(curr_fs);
	LOGPRT(LOG_INFO, "Load %s (size=0x%x)\n", img->file_name, read_size);
	return read_size;
...
static int load_c2k_img_to_dram(char *namep, struct sdio_modem_port *port, int need_check_header)
{
	int ret = 0;
	int fp_id;
	int offset=0;
	...
	if(need_check_header)
		...
		img = &modem->bl_img_info;
	strncpy(img->file_name, namep, sizeof(img->file_name) - 1);
	LOGPRT(LOG_INFO, "img_name %s\n", img->file_name);

	filp = open_img_file(img->file_name, &fp_id);
	if (IS_ERR(filp)) {
		LOGPRT(LOG_INFO, "open %s fail: %ld\n", img->file_name, PTR_ERR(filp));
		ret = -CCCI_ERR_LOAD_IMG_FILE_OPEN;
		filp = NULL;
		goto out;
	} else {
		LOGPRT(LOG_INFO, "open %s OK\n", img->file_name);
	}
...
#ifdef ENABLE_MD_IMG_SECURITY_FEATURE
	offset = signature_check(&img->file_name, &sec_tail_length);
	LOGPRT(LOG_INFO, "signature_check offset:%d, tail:%d\n", offset, sec_tail_length);
	if (offset<0) {
		LOGPRT(LOG_INFO, "signature_check failed ret=%d\n",offset);
		ret=offset;
		goto out;
	}
#endif
	...
	img_addr = modem->c2k_img_buf;
	if(!img_addr){
		LOGPRT(LOG_ERR, "ERR: no memory to load image, quit\n");
		goto out;
	}
	
#ifdef ENABLE_MD_IMG_SECURITY_FEATURE
	if (masp_ccci_is_cipherfmt(fp_id, offset, &img_len)) {
		LOGPRT(LOG_INFO, "cipher image\n");
		ret=load_cipher_firmware(fp_id, img, img_addr, offset, img_len, need_check_header);
		if(ret<0) {
			LOGPRT(LOG_ERR, "load_cipher_firmware failed:ret=%d!\n",ret);
			goto out;
		}
		LOGPRT(LOG_INFO, "load_cipher_firmware done! (=%d)\n",ret);
	} else {
#endif
		LOGPRT(LOG_INFO, "Not cipher image\n");
		ret=load_std_firmware(img_addr, filp, img, need_check_header);
		if(ret<0) {
			LOGPRT(LOG_INFO, "load_firmware failed: ret=%d!\n",ret);
			goto out;
		}
#ifdef ENABLE_MD_IMG_SECURITY_FEATURE           
	}
...
static long dev_char_ioctl( struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	char img_name[64];
	int ret = 0;
	int img_buf_reserved = 0;

	switch (cmd) {
		case C2K_IOCTL_PREPARE_BL:
			copy_from_user(img_name, argp, sizeof(img_name));
			LOGPRT(LOG_INFO, "ioctl from %s img_name %s\n", current->comm, img_name);
			load_c2k_img_to_dram(img_name, port, 0);
			break;
		case C2K_IOCTL_PREPARE_MD_IMG:
			copy_from_user(img_name, argp, sizeof(img_name));
			LOGPRT(LOG_INFO, "ioctl from %s img_name %s\n", current->comm, img_name);
			load_c2k_img_to_dram(img_name, port, 1);
			break;
		case C2K_IOCTL_GET_BL_SIZE:
			ret = put_user((unsigned int)port->modem->bl_img_info.size, (unsigned int __user *)arg);
			LOGPRT(LOG_INFO, "ioctl from %s, bl size(%d)\n", current->comm, port->modem->bl_img_info.size);
			break;
		case C2K_IOCTL_GET_MD_IMG_SIZE:
			ret = put_user((unsigned int)port->modem->img_info.size, (unsigned int __user *)arg);
			LOGPRT(LOG_INFO, "ioctl from %s, md img size(%d)\n", current->comm, port->modem->img_info.size);
			break;
		case C2K_IOCTL_IS_IMG_BUF_RESERVED:
			if(port->modem->c2k_img_buf)
				img_buf_reserved = 1;
			ret = put_user(img_buf_reserved, (unsigned int __user *)arg);
			LOGPRT(LOG_INFO, "ioctl from %s, img_buf_reserved(%d)\n", current->comm, img_buf_reserved);
			break;
		default:
...
{
	struct sdio_buf_in_packet *packet = NULL;
	int read_len=0, read_curr=0, full_pkt_done=0, user_available=count, ret;
    struct sdio_modem_port *port = (struct sdio_modem_port *)file->private_data;

	if(list_empty(&port->sdio_buf_in_list)) {
        if(!(file->f_flags & O_NONBLOCK)) {
            ret = wait_event_interruptible(port->rx_wq, !list_empty(&port->sdio_buf_in_list));    
            if(ret == -ERESTARTSYS) {
                ret = -EINTR;
                goto exit;
            }
...
get_one:
	...
	if(!packet->buffer || (packet->offset+read_curr)>packet->o_size) {
		LOGPRT(LOG_INFO, "reading on port%d %p o_size=%d size=%d offset=%d read_curr=%d\n", port->index, 
							packet, packet->o_size, packet->size, packet->offset, read_curr);
		BUG_ON(1);
	}
	if(copy_to_user(buf+read_len, packet->buffer+packet->offset, read_curr)) {
		LOGPRT(LOG_ERR, "reading on port%d %p copy_to_user fail: o_size=%d size=%d offset=%d read_curr=%d\n",
				port->index, packet, packet->o_size, packet->size, packet->offset, read_curr);
		ret = -EFAULT;
		mutex_unlock(&port->sdio_buf_in_mutex);
		goto exit;
	}
...
exit:
	if(port->index == EXCP_MSG_CH_ID-1 || port->index == SDIO_AT_CHANNEL_NUM-1 ||
			port->index == SDIO_AT2_CHANNEL_NUM-1 || port->index == EXCP_CTRL_CH_ID-1 ||
			port->index == AGPS_CH_ID-1 || port->index == SDIO_AT3_CHANNEL_NUM-1)
...
static ssize_t dev_char_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	...
	if(modem->status != MD_OFF){
		spin_unlock_irqrestore(&modem->status_lock, flags);
		chars_in_fifo = kfifo_len(&port->transmit_fifo);
		if ((port->index != (FLS_CH_ID-1)) && (count > (ONE_PACKET_MAX_SIZE - chars_in_fifo))){
			printk(KERN_DEBUG "[C2K]port%d too many(%d) data pending...flow ctrl(%d)\n", port->index, chars_in_fifo, atomic_read(&port->sflow_ctrl_state));
			return -EBUSY;
		}
		//ret = kfifo_in_locked(&port->transmit_fifo, buf, count, &port->write_lock);
		spin_lock_irqsave(&port->write_lock, fflags);
		ret = kfifo_from_user(&port->transmit_fifo, buf, count, &copied);
	...
	if((port->index == (EXCP_CTRL_CH_ID-1)) || (port->index == (EXCP_MSG_CH_ID-1))){
		LOGPRT(LOG_INFO, "write on port%d for %d/%d/%zu\n", port->index, ret, copied, count);
	}
	LOGPRT(LOG_DEBUG, "write on port%d for %d/%d/%zu\n", port->index, ret, copied, count);
	return ret;
}

static unsigned int dev_char_poll(struct file *fp, struct poll_table_struct *poll)
{
    struct sdio_modem_port *port = (struct sdio_modem_port *)fp->private_data;
    unsigned int mask = 0;
    ...
    //printk("[C2K] poll done on %d, mask=%x\n", port->index, mask);

    return mask;
}

static int dev_char_mmap(struct file *file, struct vm_area_struct *vma)
{
    unsigned long pfn = 0;
    unsigned long addr;
    struct sdio_modem_port *port = (struct sdio_modem_port *)file->private_data;
	unsigned long pos, size, start= 0;
	struct page *page = NULL;
	phys_addr_t img_phy_addr;
	unsigned int img_size_max = 0;

    /* only flashless channel can be mapped */
	...
	addr = (unsigned long)port->modem->c2k_img_buf;
	if(!addr){
		LOGPRT(LOG_INFO, "mmap fail: c2k_img_buf not prepared!\n");
        return -ENOMEM;
	}
	img_size_max = get_md3_img_size();

    LOGPRT(LOG_INFO, "remap addr:0x%lx map-len:%lu, vma: start(0x%lx),end(0x%lx)\n", addr, vma->vm_end-vma->vm_start, vma->vm_start,vma->vm_end);
    if ((vma->vm_end - vma->vm_start) > img_size_max) {
        LOGPRT(LOG_INFO, "Get invalid mm size request from ch%d!\n", port->index + 1);
        return -EINVAL;
    }
	...
	img_phy_addr = get_md3_img_buf_phy_addr();//port->modem->dma_handle;
    pfn = img_phy_addr >> PAGE_SHIFT;
    
    if (remap_pfn_range(vma, vma->vm_start, pfn, size, vma->vm_page_prot)) {
		LOGPRT(LOG_ERR, "remap error\n");
        return -EAGAIN;
    }
	LOGPRT(LOG_ERR, "remap done\n");
	mb();
    return 0;
}
...
int  modem_sdio_init(struct cbp_platform_data *pdata)
{
	...
	int index = 0;
	unsigned long flags;
	phys_addr_t img_buf_phy_addr;

	printk("%s %d: Enter.\n", __func__, __LINE__);

#if ENABLE_CHAR_DEV
	ret = alloc_chrdev_region(&char_dev_num, 0, SDIO_TTY_NR, "c2k_ccci_node");
	if(ret)
		return -ENOMEM;
	...
	modem->c2k_img_buf = NULL;
	img_buf_phy_addr = get_md3_img_buf_phy_addr();
	if(img_buf_phy_addr)
		modem->c2k_img_buf = ioremap_nocache(img_buf_phy_addr, get_md3_img_size());
...
#ifndef CONFIG_EVDO_DT_VIA_SUPPORT
	modem->as_packet = kzalloc(sizeof(struct sdio_assemble_packet), GFP_KERNEL);
	if(!modem->as_packet){
	...
	for (index = 0; index < SDIO_TTY_NR; index++) 
	{
		...
		if (ret){
		    ...
		    ret = cdev_add(&port->char_dev, MKDEV(char_dev_major, port->index), 1);
			dev = device_create(dev_class, NULL, MKDEV(char_dev_major, port->index), NULL, 
								"%s%d", "ttySDIO", port->index);
			if (IS_ERR(dev)) {
				ret = PTR_ERR(dev);
	...
	sysfs_create_group(modem->c2k_kobj, &g_modem_attr_group);
...
#ifdef TX_DONE_TRACE
	setup_timer(&timer_wait_tx_done, wait_tx_done_timer, (unsigned long)"C2K_TX");
#endif

#ifndef CONFIG_EVDO_DT_VIA_SUPPORT
	setup_timer(&modem->heart_beat_timer, c2k_heart_beat_timer, (unsigned long)modem);
	setup_timer(&modem->poll_timer, c2k_poll_status_timer, (unsigned long)modem);
	setup_timer(&modem->force_assert_timer, c2k_force_assert_timer, (unsigned long)modem);
#endif
