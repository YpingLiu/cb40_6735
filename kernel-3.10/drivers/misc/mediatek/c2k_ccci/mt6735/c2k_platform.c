...
#define CCCI_MD3_IMG_MEM_RESERVED_KEY "reserve-memory-ccci_md3_img"
...
#define MD3_IMG_SIZE_MAX (4*1024*1024)

phys_addr_t md3_mem_base;
phys_addr_t md3_img_mem_base = 0;
...
int modem_sdio_reserve_mem_of_init_for_img(struct reserved_mem * rmem, unsigned long node, const char * uname)
{
	phys_addr_t rptr = 0;
	unsigned int rsize= 0;
	...
	if(strcmp(uname, CCCI_MD3_IMG_MEM_RESERVED_KEY) == 0){
		if(rsize != MD3_IMG_SIZE_MAX)	{
			MTK_MEMCFG_LOG_AND_PRINTK(KERN_ERR "%s: reserve size=0x%x < 0x%x\n", __func__, rsize, MD3_IMG_SIZE_MAX);
			return 0;
		}
	}
	md3_img_mem_base = rmem->base;
	return 0;
}
RESERVEDMEM_OF_DECLARE(ccci_reserve_mem_md3_img_init,CCCI_MD3_IMG_MEM_RESERVED_KEY,modem_sdio_reserve_mem_of_init_for_img);
#endif
...
void dump_c2k_bootup_status()
{
	static int init_done = 0;
	static unsigned long c2k_iram_base_virt1 = 0;
	static unsigned long c2k_iram_base_virt2 = 0;
	static unsigned long c2k_uart0 = 0;
	static unsigned long bootst = 0;
	static unsigned long chipid = 0;
	static unsigned long c2ksys_uart0 = 0;
	int i;
	int retry = 0;
	...
	printk("[C2K] ======== Dump C2K bootup status begin ========\n");
	printk("[C2K] addr: 0x10211370 (UART GPIO)\n");
	...
	while(retry < 20){
		retry++;
		printk("[C2K] C2K_CONFIG = 0x%x, C2K_STATUS = 0x%x\n", 
	...
	printk("[C2K] ======== Dump C2K bootup status end ========\n");
...
phys_addr_t get_md3_img_buf_phy_addr()
{
	return md3_img_mem_base;
}

unsigned int get_md3_img_size()
{
	return MD3_IMG_SIZE_MAX;
}
