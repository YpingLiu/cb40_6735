...
EXPORT_SYMBOL(c2k_modem_not_ready);
...
void c2k_reset_modem(void)
{
	...
	if(atomic_add_return(1, &reset_on_going) > 1) {
		printk("[C2K] %s: one reset on going, %d\n", __func__, atomic_read(&reset_on_going));
		return;
	}
...
void c2k_power_on_modem(void)
{
	...
	printk("[C2K] Warnning: power on vmodem\n");
}
EXPORT_SYMBOL(c2k_power_on_modem);
...
void c2k_power_off_modem(void)
{
	...
	if(atomic_add_return(1, &reset_on_going) > 1) {
		printk("[C2K] %s: one power off on going, %d\n", __func__, atomic_read(&reset_on_going));
		return;
	}
	atomic_set(&modem_not_ready, 1);
    ...
    printk("[C2K] Warnning: power off vmodem\n");
    atomic_set(&reset_on_going, 0);
}
EXPORT_SYMBOL(c2k_power_off_modem);
...
EXPORT_SYMBOL(modem_ipoh_indication_usr);
...
ssize_t modem_force_assert_store(
	struct kobject *kobj, struct kobj_attribute *attr,
	const char *buf, size_t n)
{
    if (cmdata->modem){
...
ssize_t modem_force_assert_show(
	struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    int reset = 0;
    int ret = 0;
...
static int modem_excp_notify_misc(struct notifier_block *nb, unsigned long event,
			       void *ptr)
{
	printk("%s %d event=%ld\n",__func__,__LINE__,event);
    switch (event) {
        case MDM_EVT_NOTIFY_EXCP:
			printk("%s %d ASC_USER_MDM_EXCEPTION\n",__func__,__LINE__);
...
{
    if(GPIO_C2K_VALID(GPIO_C2K_MDM_PWR_IND)){
        c2k_gpio_set_irq_type(GPIO_C2K_MDM_PWR_IND, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING);
...
static int modem_sync_init(void)
{
    ...
    if(GPIO_C2K_VALID(GPIO_C2K_AP_WAKE_MDM) && GPIO_c2k_VALID(GPIO_C2K_MDM_RDY)){
    ...
    if(GPIO_C2K_VALID(GPIO_C2K_USB_MDM_WAKE_AP) && GPIO_C2K_VALID(GPIO_C2K_USB_AP_RDY)){
        memset(&cfg, 0, sizeof(struct asc_config));
    ...
    if(GPIO_OEM_VALID(GPIO_C2K_UART_MDM_WAKE_AP) && GPIO_OEM_VALID(GPIO_C2K_UART_AP_RDY)){
        memset(&cfg, 0, sizeof(struct asc_config));
		...
		cmd, unsigned long arg)
{
	...
	case CMDM_IOCTL_GET_MD_STATUS:
		if (cmdata->modem)
			ret = put_user((unsigned int)cmdata->modem->status, (unsigned int __user *)arg);
		else
			return -EFAULT;
		break;

	case CMDM_IOCTL_ENTER_FLIGHT_MODE:
		printk("[C2K SDIO]enter flight mode.\n");
		c2k_power_off_modem();
		if(!GPIO_C2K_VALID(GPIO_C2K_MDM_PWR_IND))
			modem_notify_event(MDM_EVT_NOTIFY_POWER_OFF);
	...
	case CMDM_IOCTL_LEAVE_FLIGHT_MODE:
		printk("[C2K SDIO]leave flight mode.\n");
		c2k_power_on_modem();
		modem_notify_event(MDM_EVT_NOTIFY_RESET_ON);
		break;
#ifndef CONFIG_EVDO_DT_VIA_SUPPORT
	case CMDM_IOCTL_FORCE_ASSERT:
		printk("[C2K SDIO]force C2K assert ioctl.\n");
		if (cmdata->modem){
			force_c2k_assert(cmdata->modem);
		}
		break;
    ...
    case CMDM_IOCTL_GET_SDIO_STATUS:
        printk("[C2K SDIO]get sdio status.\n");
        if (cmdata->modem)
            dump_c2k_sdio_status(cmdata->modem);
        break;
    case CMDM_IOCTL_DUMP_C2K_IRAM:
        printk("[C2K SDIO]dump c2k iram.\n");
        dump_c2k_iram_seg2();
        break;
	case CMDM_IOCTL_DUMP_BOOTUP_STATUS:
        printk("[C2K SDIO]dump c2k bootup status.\n");
...
{
    struct c2k_modem_data *d = (struct c2k_modem_data *)(filp->private_data);
...
static int __init modem_init(void)
{
    ...
    if(!cmdata){
        ret = -ENOMEM;
        printk("No memory to alloc cmdata");
    ...
    if(GPIO_C2K_VALID(GPIO_C2K_MDM_PWR_IND)){
        ...
        c2k_gpio_set_irq_type(GPIO_C2K_MDM_PWR_IND, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING);
        ret = c2k_gpio_request_irq(GPIO_C2K_MDM_PWR_IND, modem_power_indication_irq, \
                     IRQF_SHARED | IRQF_NO_SUSPEND | IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, \
                     "mdm_power_ind", cmdata);
    ...
    if(GPIO_C2K_VALID(GPIO_C2K_CRASH_CBP)){
		printk("%s %d GPIO_C2K_CRASH_CBP",__func__,__LINE__);
		c2k_gpio_direction_output(GPIO_C2K_CRASH_CBP, 1);
    }
