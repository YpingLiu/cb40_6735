...
//static struct i2c_board_info __initdata i2c_epl_sensor= { I2C_BOARD_INFO(EPL_DEV_NAME, (0x80>>1))};
...
#define APS_TAG                 	  	"[ALS/PS epl259x sensor] "
...
static DRIVER_ATTR(ps_w_calfile,				S_IROTH  | S_IWOTH, NULL,			                    epl_sensor_store_ps_w_calfile				);
#if PS_DYN_K
...
#endif
static DRIVER_ATTR(als_intr_table,					    S_IROTH  | S_IWOTH, epl_sensor_show_als_intr_table,            NULL                         );
...
//    printk("ALS/PS %s: i2c_number=%d,i2c_addr=0x%x\n", __func__,hw->i2c_num,hw->i2c_addr[0]);
    APS_LOG("ALS/PS %s: i2c_number=%d,i2c_addr=0x%x\n", __func__,hw->i2c_num,hw->i2c_addr[0]);
#if MTK_LTE
    alsps_driver_add(&epl_sensor_init_info);
#else
    if(err=(platform_driver_register(&epl_sensor_alsps_driver)))
    {
        APS_ERR("failed to register driver err=[%d]\n",err);
        return -ENODEV;
    }
	APS_ERR("err=[%d]\n",err);
#endif
