...
//	write_cmos_sensor(0x303f, 0x01); //pei_add
...
{
	printk(" ov8858-4lane get_imgsensor_id \n");
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				LOG_INF("ov8858-4lane i2c write id: 0x%x, sensor id: 0x%x, ov8858version = %d(0=r2a,1=r1a)\n", imgsensor.i2c_write_id,*sensor_id,ov8858version);
				return ERROR_NONE;
				}
				...
				LOG_INF("ov8858-4lane read ov8858 R1A R2A bate fail\n");
				return ERROR_SENSOR_CONNECT_FAIL;
				}
			}
			LOG_INF("ov8858-4lane Read sensor id fail, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
{
	int RG_Ratio_Typical = 0x134, BG_Ratio_Typical = 0x123;//pei_
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("ov8858-4lane i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
				break;
			}
			LOG_INF("ov8858-4lane Read sensor id fail, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
			retry--;
	...
	mdelay(10);
		...
		read_otp(&otp_ptr);
		apply_otp(&otp_ptr);
		printk("pxs_ov8858_4lane MID = %d \n " , otp_ptr.module_integrator_id ); //pei_add
#endif
