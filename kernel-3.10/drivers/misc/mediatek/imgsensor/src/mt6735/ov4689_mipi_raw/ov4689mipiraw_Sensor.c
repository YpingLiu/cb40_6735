...
#include "ov4689mipiraw_Sensor.h"
//#include "OV4689_otp.h"
/****************************Modify Following Strings for Debug****************************/
#define PFX "mtkOV4689_camera_sensor"
#define LOG_1 LOG_INF("OV4689,MIPI 2LANE\n")
/****************************   Modify end    *******************************************/
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = OV4689MIPI_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h

    .checksum_value = 0xafb5098f,      //checksum value for Camera Auto Test

    .pre = {
        .pclk = 120000000,                //record different mode's pclk
        .linelength = 2568, // Jeff //2544,                //record different mode's linelength
        .framelength = 1556, // Jeff //1572,            //record different mode's framelength
    ...
    .cap1 = {                            //capture for PIP 24fps relative information, capture1 mode must use same framelength, linelength with Capture mode for shutter calculate
        .pclk = 120000000,                //record different mode's pclk
        .linelength = 2568, // Jeff //2576,                //record different mode's linelength
        .framelength = 3114, // Jeff //3104,            //record different mode's framelength
        .startx = 0,                    //record different mode's startx of grabwindow
        .starty = 0,                    //record different mode's starty of grabwindow
        .grabwindow_width = 2688,        //0xA80//record different mode's width of grabwindow
        .grabwindow_height = 1520,       //0X5F0 //record different mode's height of grabwindow
    ...
    .hs_video = {
        .pclk = 120000000,                //record different mode's pclk
        .linelength = 2544,                //record different mode's linelength
        .framelength = 1572,            //record different mode's framelength
        .startx = 0,                    //record different mode's startx of grabwindow
        .starty = 0,                    //record different mode's starty of grabwindow
        .grabwindow_width = 640,        //0xA80//record different mode's width of grabwindow
        .grabwindow_height = 480,       //0X5F0 //record different mode's height of grabwindow
        /*     following for MIPIDataLowPwr2HighSpeedSettleDelayCount by different scenario    */
        .mipi_data_lp2hs_settle_dc = 23,//unit , ns
        /*     following for GetDefaultFramerateByScenario()    */
        .max_framerate = 300,
...
*    The registers 0x3500 ,0x3501 and 0x3502 control exposure of OV4689.
...
{
    LOG_INF("Enter OV4689 sensor_init.\n");
    ...
    write_cmos_sensor(0x380d, 0x08);     // HTS L
    write_cmos_sensor(0x380e, 0x06);     // VTS H
    write_cmos_sensor(0x380f, 0x14);     // VTS L
    ...
    write_cmos_sensor(0x3d85, 0x10);       //    power up load setting on, OTP write register load setting off
    /* _END_ */

    write_cmos_sensor(0x3d8c, 0x71);     // OTP start address H
    write_cmos_sensor(0x3d8d, 0xcb);     // OTP start address L
    write_cmos_sensor(0x3f0a, 0x00);     //SRAM
    write_cmos_sensor(0x4000, 0xf1);     // out of range trig off, format chg on, gain chg on, exp chg on, manual trig off, no
    ...
    write_cmos_sensor(0x4022, 0x09);     // Anchor left end H
    write_cmos_sensor(0x4023, 0x13);     //Anchor left end L
    write_cmos_sensor(0x4024, 0x0a);     // Anchor right start H
    write_cmos_sensor(0x4025, 0x40);     // Andhor right start L
    write_cmos_sensor(0x4026, 0x0a);     // Anchor right end H
    write_cmos_sensor(0x4027, 0x50);     //Anchor right end L
    ...
    write_cmos_sensor(0x4503, 0x01);     // ADC sync control
    write_cmos_sensor(0x4601, 0xa7);     // V fifo read start
...
static void preview_setting(void)
{
    ...
    write_cmos_sensor(0x3836, 0x01);      // r zline use num/2
    write_cmos_sensor(0x4001, 0x40);      // debug mode
    write_cmos_sensor(0x4022, 0x09);      //07 // Anchor left end H
    write_cmos_sensor(0x4023, 0x13);      //cf //Anchor left end L
    write_cmos_sensor(0x4024, 0x0a);      //09 // Anchor right start H
    write_cmos_sensor(0x4025, 0x40);      //60 // Andhor right start L
    write_cmos_sensor(0x4026, 0x0a);      //09 // Anchor right end H
    write_cmos_sensor(0x4027, 0x50);      //6f //Anchor right end L
    write_cmos_sensor(0x4502, 0x40);      // ADC sync control
    write_cmos_sensor(0x4601, 0xA7);      //04 // V fifo read start
    write_cmos_sensor(0x4837, 0x18);      // MIPI global timing
...
static void capture_setting(kal_uint16 currefps)
{
    ...
    /*
        @@Full Resolution 2688x1520 15fps
            ;;PCLK=120M, 2576x3104x15
    */
        LOG_INF("this is 15 fps capture setting\n");
     write_cmos_sensor(0x0100,0x00);    // Sensor go to standby. MIPI stream off, both clock and data lane in LP11 mode
       ...
       mDELAY(30);
        ...
        write_cmos_sensor(0x380c, 0x0a);      //HTS H
        write_cmos_sensor(0x380d, 0x08);      // HTS L
        write_cmos_sensor(0x380e, 0x0c);      // VTS H
        write_cmos_sensor(0x380f, 0x2a);      // VTS L
        write_cmos_sensor(0x3811, 0x08);      // H win off L
        write_cmos_sensor(0x3813, 0x04);      // V win off L
        write_cmos_sensor(0x3814, 0x01);      // H inc odd
        write_cmos_sensor(0x3820, 0x00);      // flip off, bin off
    ...
    {
            LOG_INF("currefps:%d,  which mode set this framerate?\n",currefps);
        write_cmos_sensor(0x0100,0x00);    // Sensor go to standby. MIPI stream off, both clock and data lane in LP11 mode
...
static void normal_video_setting(kal_uint16 currefps)
{
    ...
    /***here,  the setting is same to preview setting,  but it should exit or it will redscreen  in the engineer mode when you switch video size,  **/
    write_cmos_sensor(0x0100,0x00);    // Sensor go to standby. MIPI stream off, both clock and data lane in LP11 mode
...
static kal_uint32 open(void)
{
    ...
    sensor_init();
#if 0
//#if defined(OV4689_USE_WB_OTP)
    OV4689_otp_cali(imgsensor_info.sensor_id);
#endif
...
UINT32 OV4689_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    OV4689_MIPI_RAW_SensorInit    */
