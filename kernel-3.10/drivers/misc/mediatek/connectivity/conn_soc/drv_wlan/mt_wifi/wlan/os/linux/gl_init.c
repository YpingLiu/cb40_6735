...
//    prDev = &((struct sdio_func *) pvData)->dev; //samp
    ...
    prWdev->wiphy->vendor_commands = &mtk_wlan_vendor_ops;
    prWdev->wiphy->n_vendor_commands = sizeof(mtk_wlan_vendor_ops) / sizeof(struct wiphy_vendor_command);
    prWdev->wiphy->vendor_events = &mtk_wlan_vendor_events;
    prWdev->wiphy->n_vendor_events = ARRAY_SIZE(mtk_wlan_vendor_events);
#ifdef CONFIG_PM
...
//        UINT_8 aucBuf[32] = {0};
        ...
        for (i = 0; i < u4NumIPv4; i++) {
            ...
            prParamNetAddr = (P_PARAM_NETWORK_ADDRESS)((ULONG)prParamNetAddr + sizeof(PARAM_NETWORK_ADDRESS));
...
//                    prGlueInfo->rHifInfo.DmaOps->DmaRegDump(&prGlueInfo->rHifInfo);
        ...
        /* adjust roaming threshold */
        {
            ...
            if (rStatus != WLAN_STATUS_SUCCESS)
                DBGLOG(INIT, WARN, ("set roaming advance info fail 0x%x\n", rStatus));
            }
        ...
        if ( (i4Status = procCreateFsEntry(prGlueInfo)) < 0) {
