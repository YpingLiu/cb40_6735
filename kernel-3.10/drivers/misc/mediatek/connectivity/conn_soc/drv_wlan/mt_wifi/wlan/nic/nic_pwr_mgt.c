    ...
    while(1) {
		...
		|| fgIsResetting == TRUE) {
            ...
            if (fgIsResetting != TRUE)
            {
                ...
                printk("<WiFi> [1]MCR_D2HRM2R = 0x%x, ORI_MCR_D2HRM2R = 0x%x\n",
				...
				HAL_MCR_RD(prAdapter, MCR_D2HRM2R, &u4RegValue);
				printk("<WiFi> [2]MCR_D2HRM2R = 0x%x, ORI_MCR_D2HRM2R = 0x%x\n",
                		u4RegValue, u4OriRegValue);
                printk("<WiFi> Fatal error! Driver own fail!!!!!!!!!!!! %d fgIsBusAccessFailed:%d\n",
