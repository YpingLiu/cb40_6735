...
#else
        #define GPIO_GPS_LNA_PIN GPIO87
#endif
...
INT32
wmt_plat_init (UINT32 co_clock_type)
{
        CMB_STUB_CB stub_cb;
    INT32 iret;
...
UINT32 wmt_plat_read_clk_status()
{
        return CONSYS_REG_READ(conn_reg.mcu_base + CONSYS_CLK_STATUS_OFFSET);
}
UINT32 wmt_plat_read_dmaregs(UINT32 type)
{
        ...
        switch(type)
        {
                case CONNSYS_CLK_GATE_STATUS: return CONSYS_REG_READ(CONNSYS_CLK_GATE_STATUS_REG);
                case CONSYS_EMI_STATUS: return CONSYS_REG_READ(CONSYS_EMI_STATUS_REG);
                case SYSRAM1: return CONSYS_REG_READ(SYSRAM1_REG);
                case SYSRAM2: return CONSYS_REG_READ(SYSRAM2_REG);
                case SYSRAM3: return CONSYS_REG_READ(SYSRAM3_REG);
                default: return 0;
        }
#endif
}
...
UINT32 wmt_plat_force_trigger_assert(ENUM_FORCE_TRG_ASSERT_T type)
{
        ...
        switch(type){
                ...
                case STP_FORCE_TRG_ASSERT_DEBUG_PIN:
                        ...
                        CONSYS_REG_WRITE(conn_reg.topckgen_base + CONSYS_AP2CONN_OSC_EN_OFFSET,CONSYS_REG_READ(conn_reg.topckgen_base + CONSYS_AP2CONN_OSC_EN_OFFSET) & ~CONSYS_AP2CONN_WAKEUP_BIT);
...
INT32 wmt_plat_get_dump_info(UINT32 offset)
{
        ...
        WMT_PLAT_INFO_FUNC("connsys_reg_read (0x%x), (0x%p), (0x%x)\n", CONSYS_REG_READ(p_virtual_addr),p_virtual_addr, offset );
...
EXPORT_SYMBOL(wmt_plat_read_clk_status);
