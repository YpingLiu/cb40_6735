...
static volatile int out_status =0;
...
//swith to loud speaker
static ssize_t accdet_store_loud_speaker_state(struct device_driver *ddri, const char *buf, size_t count)
{
	ACCDET_DEBUG("accdet_store_loud_speaker_state\n");

	if (sscanf(buf, "%u", &out_status) != 1) {
			ACCDET_DEBUG("accdet: Invalid values\n");
			return -EINVAL;
	}
	...
	{
		case 0 :
			ACCDET_DEBUG("accdet_store_loud_speaker_state: 0\n");
			mt_set_gpio_out(GPIO_GSE_1_EINT_PIN, GPIO_OUT_ZERO);
		break;

		case 1 :
			ACCDET_DEBUG("accdet_store_loud_speaker_state: 1\n");
...
static DRIVER_ATTR(accdet_loud_speaker,      0664, NULL,         accdet_store_loud_speaker_state);
...
static struct driver_attribute *accdet_attr_list[] = {
	...
	&driver_attr_accdet_loud_speaker,
	//#ifdef ACCDET_PIN_RECOGNIZATION
