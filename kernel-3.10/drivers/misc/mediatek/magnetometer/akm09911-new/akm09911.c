 ...
 **************************/
     ...
     *********************/
    #if AKM09911_DEBUG
        #define MAGN_TAG                  "[Msensor akm09911] "
        ...
        #define MAGN_LOG(fmt, args...)   printk(KERN_ERR  MAGN_TAG"%s %d : "fmt, __FUNCTION__, __LINE__, ##args)
    #else
        #define MAGN_TAG
...
static void    akm_set_gpio64(int high);
...
static int AKECS_CheckDevice(void)
{
	...
	MAGN_LOG(" AKM check device id = [%x]\n",buffer[0]);
	MAGN_LOG("ret = %d\n",ret);
	if(ret < 0)
	{
		return ret;
...
/*set gpio64 for disable reset pin*/
static void    akm_set_gpio64(int high)
{
//	MAG_ERR("GPIO18=[%d],GPIO_MODE_00=[%d],GPIO_DIR_OUT=[%d],GPIO_OUT_ONE=[%d]\n",GPIO18,GPIO_MODE_00,GPIO_DIR_OUT,GPIO_OUT_ONE);
	...
	if(high)
	{
		...
		mdelay(30);
		mt_set_gpio_out(GPIO64,GPIO_OUT_ONE);
		mdelay(30);
	}
	else
	{
...
static int akm09911_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/*
	MAGN_ERR("akm set gpio42\n");
	 mt_set_gpio_mode(GPIO42,GPIO_MODE_00);
      	  mt_set_gpio_dir(GPIO42,GPIO_DIR_OUT);
    	  mt_set_gpio_out(GPIO42,GPIO_OUT_ONE);
*/
	MAGN_FUN();
...
*/
	...
	err=i2c_register_board_info(hw->i2c_num, &i2c_akm09911, 1);
	MAGN_LOG("err=[%d]\n",err);
	errr=mag_driver_add(&akm09911_init_info);
	MAGN_LOG("errr=[%d]\n",errr);
	return 0;
}
/*----------------------------------------------------------------------------*/
