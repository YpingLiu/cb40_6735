...
#include "msg28xx_jinlong_update_bin.h" // for MSG28xx
#include "msg28xx_yeji_update_bin.h"
...
u32 g_GestureWakeupMode[2] = {0x169F, 0xFFFFFFFF};
        ...
        if (bIsCorrectFormat)
        {
            ...
            switch (nWakeupMode)
            {
                case 0x58:
                    ...
                    input_report_key(g_InputDevice, KEY_GESTURE_DOUBLE_TAP, 1);
                    input_sync(g_InputDevice);
                    input_report_key(g_InputDevice, KEY_GESTURE_DOUBLE_TAP, 0);
                    input_sync(g_InputDevice);
                    break;
                case 0x60:
...
//                    input_report_key(g_InputDevice, KEY_UP, 1);
                    input_report_key(g_InputDevice, KEY_GESTURE_UP, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, KEY_UP, 0);
                    input_report_key(g_InputDevice, KEY_GESTURE_UP, 0);
                    input_sync(g_InputDevice);
                    break;
                case 0x61:
...
//                    input_report_key(g_InputDevice, KEY_DOWN, 1);
                    input_report_key(g_InputDevice, KEY_GESTURE_DOWN, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, KEY_DOWN, 0);
                    input_report_key(g_InputDevice, KEY_GESTURE_DOWN, 0);
                    input_sync(g_InputDevice);
                    break;
                case 0x62:
...
//                  input_report_key(g_InputDevice, KEY_LEFT, 1);
                    input_report_key(g_InputDevice, KEY_GESTURE_LEFT, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, KEY_LEFT, 0);
                    input_report_key(g_InputDevice, KEY_GESTURE_LEFT, 0);
                    input_sync(g_InputDevice);
                    break;
                case 0x63:
...
//                    input_report_key(g_InputDevice, KEY_RIGHT, 1);
                    input_report_key(g_InputDevice, KEY_GESTURE_RIGHT, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, KEY_RIGHT, 0);
                    input_report_key(g_InputDevice, KEY_GESTURE_RIGHT, 0);
                    input_sync(g_InputDevice);
                    break;
                case 0x64:
...
//                    input_report_key(g_InputDevice, KEY_C, 1);
                    input_report_key(g_InputDevice, KEY_GESTURE_C, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, KEY_C, 0);
                    input_report_key(g_InputDevice, KEY_GESTURE_C, 0);
                    input_sync(g_InputDevice);
                    break;
                case 0x67:
...
//                    input_report_key(g_InputDevice, KEY_V, 1);
                    input_report_key(g_InputDevice, KEY_GESTURE_V, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, KEY_V, 0);
                    input_report_key(g_InputDevice, KEY_GESTURE_V, 0);
                    input_sync(g_InputDevice);
                    break;
                case 0x69:
...
//                    input_report_key(g_InputDevice, KEY_O, 1);
                    input_report_key(g_InputDevice, KEY_GESTURE_O, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, KEY_O, 0);
                    input_report_key(g_InputDevice, KEY_GESTURE_O, 0);
                    input_sync(g_InputDevice);
                    break;
                case 0x6A:
...
//                    input_report_key(g_InputDevice, KEY_Z, 1);
                    input_report_key(g_InputDevice, KEY_GESTURE_Z, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, KEY_Z, 0);
                    input_report_key(g_InputDevice, KEY_GESTURE_Z, 0);
                    input_sync(g_InputDevice);
                    break;
                case 0x6C:
...
static void _DrvFwCtrlMsg28xxProgramEmem(EmemType_e eEmemType)
{
    ...
    if (eEmemType == EMEM_MAIN) // Program main block
    {
        ...
        nPageNum = (MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE * 1024) / 8; // 128*1024/8=16384 // TODO : add for testing
#else
        ...
        for (i = 0; i < nPageNum; i ++)
        {
...
#ifdef CONFIG_ENABLE_UPDATE_FIRMWARE_WITH_8_BYTE_EACH_TIME
                nLength = 8 - MSG28XX_EMEM_SIZE_BYTES_ONE_WORD; // 4 = 8 - 4 // TODO : add for testing
#else
            ...
            {
#ifdef CONFIG_ENABLE_UPDATE_FIRMWARE_WITH_8_BYTE_EACH_TIME
                nLength = 8; // TODO : add for testing
#else
                ...
                {
#ifdef CONFIG_ENABLE_UPDATE_FIRMWARE_WITH_8_BYTE_EACH_TIME
                    szBufferData[j] = g_FwData[i/128][8*(i-(128*(i/128)))+j]; // TODO : add for testing
#else
...
void _DrvFwCtrlMsg28xxCheckFirmwareUpdateBySwId(void)
{
    ...
    if (nCrcMainA == nCrcMainB)
    {
        ...
        if (eSwId == MSG28XX_SW_ID_JL)
        {
#ifdef CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY // By two dimensional array
            nUpdateBinMajor = msg28xx_jinlong_update_bin[127][1013]<<8 | msg28xx_jinlong_update_bin[127][1012];
            nUpdateBinMinor = msg28xx_jinlong_update_bin[127][1015]<<8 | msg28xx_jinlong_update_bin[127][1014];
#else // By one dimensional array
            nUpdateBinMajor = msg28xx_jinlong_update_bin[0x1FFF5]<<8 | msg28xx_jinlong_update_bin[0x1FFF4];
            nUpdateBinMinor = msg28xx_jinlong_update_bin[0x1FFF7]<<8 | msg28xx_jinlong_update_bin[0x1FFF6];
#endif
        }
     else if (eSwId == MSG28XX_SW_ID_YJ||eSwId == MSG28XX_SW_ID_YJ2)
        {
#ifdef CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY // By two dimensional array
            nUpdateBinMajor = msg28xx_yeji_update_bin[127][1013]<<8 | msg28xx_yeji_update_bin[127][1012];
            nUpdateBinMinor = msg28xx_yeji_update_bin[127][1015]<<8 | msg28xx_yeji_update_bin[127][1014];
#else // By one dimensional array
            nUpdateBinMajor = msg28xx_yeji_update_bin[0x1FFF5]<<8 | msg28xx_yeji_update_bin[0x1FFF4];
            nUpdateBinMinor = msg28xx_yeji_update_bin[0x1FFF7]<<8 | msg28xx_yeji_update_bin[0x1FFF6];
#endif
        }
        ...
        if ((nUpdateBinMinor > nMinor && nIsCompareVersion == 1) || (nIsCompareVersion == 0))
        {
            if (eSwId == MSG28XX_SW_ID_JL)
            {
                ...
                {
#ifdef CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY // By two dimensional array
                        _DrvFwCtrlStoreFirmwareData(msg28xx_jinlong_update_bin[i], 1024);
#else // By one dimensional array
                        _DrvFwCtrlStoreFirmwareData(&(msg28xx_jinlong_update_bin[i*1024]), 1024);
#endif
                    }
                }
          else if (eSwId == MSG28XX_SW_ID_YJ||eSwId == MSG28XX_SW_ID_YJ2)
                {
                    ...
                    {
#ifdef CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY // By two dimensional array
                        _DrvFwCtrlStoreFirmwareData(msg28xx_yeji_update_bin[i], 1024);
#else // By one dimensional array
                        _DrvFwCtrlStoreFirmwareData(&(msg28xx_yeji_update_bin[i*1024]), 1024);
#endif
                }
            }
            else
...
void DrvFwCtrlGetCustomerFirmwareVersion(u16 *pMajor, u16 *pMinor, u8 **ppVersion)
{
    ...
    mdelay(100);
        ...
        //DBG("msg_version write\n");
    //mdelay(50);
        IicReadData(SLAVE_I2C_ID_DWI2C, &szDbBusRxData[0], 4);
    //DBG("msg_version read\n");
    //mdelay(100);
