    ...
    if(0==nMajor)
        {
            ...
            DrvIcFwLyrGetCustomerFirmwareVersion(&nMajor, &nMinor, &_gFwVersion);
          DrvPlatformLyrTouchDeviceResetHw();
            DBG("msg_version rst2\n");
        }

/**********************
        CHIP_TYPE_MSG21XX     (0x01)
        CHIP_TYPE_MSG21XXA   (0x02)
        CHIP_TYPE_MSG26XXM   (0x03)
        CHIP_TYPE_MSG22XX     (0x7A)
        CHIP_TYPE_MSG28XX     (0x85)
**********************/

    res = snprintf(buf, PAGE_SIZE, "tpd_name:Mstar%x,fw_id:%x.%x\n", g_ChipType,nMajor,nMinor);
    DBG("show_tpd_fw_id=%x,pid=%x.%x\n",g_ChipType,nMajor,nMinor);
...
static DEVICE_ATTR(tpd_gesture_enable, 0664, show_gesture_enable, store_gesture_enable);
 ...
 struct device_attribute *mstar_attrs[] =
{
    &dev_attr_tpd_fw_id,
#ifdef CONFIG_ENABLE_GESTURE_WAKEUP
