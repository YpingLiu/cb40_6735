...
#if TOUCH_FILTER        /*sunyugg add for touchoanel debug.201411/06*/
...
static ssize_t show_tpd_fw_id(struct device *dev,struct device_attribute *attr, char *buf)
{
    ssize_t res;

    res = snprintf(buf, PAGE_SIZE, "tpd_name:gt%x,fw_id:%x\n", tpd_info.pid,tpd_info.vid);
    GTP_INFO("show_tpd_fw_id=%x,pid=%x\n",tpd_info.vid,tpd_info.pid);

    return res;

}
static DEVICE_ATTR(tpd_fw_id, 0664, show_tpd_fw_id, NULL);

#ifdef TPD_GESTURE_SUPPORT
static int tpd_ges_enable = 0;//0;//mly-temp//wentao temp
//=============================================================
static ssize_t show_gesture_enable(struct device *dev,struct device_attribute *attr, char *buf)
{
    ssize_t res;

    res = snprintf(buf, PAGE_SIZE, "%d\n", tpd_ges_enable);

    GTP_INFO("enter %s, %d, tpd_ges_enable=%d\n", __FUNCTION__, __LINE__,tpd_ges_enable);
    return res;

}
static ssize_t store_gesture_enable(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    int val=0x55;
    if (sscanf(buf, "%d", &val) != 1) {
            GTP_INFO("%s : Invalid values,buf[0]=%x\n",__func__,buf[0]);
            return -EINVAL;
    }
...
static DEVICE_ATTR(tpd_gesture_enable, 0664, show_gesture_enable, store_gesture_enable);
...
{
    GTP_INFO("GTP RESET!\n");
    GTP_GPIO_OUTPUT(GTP_INT_PORT, 0);   //henry add
...
//#ifdef VELOCITY_CUSTOM
    ...
    {
        input_set_capability(tpd->dev, EV_KEY, touch_keys[i]);
    }
    //>>>
#if GTP_GESTURE_WAKEUP
    input_set_capability(tpd->dev, EV_KEY, KEY_POWER);
    input_set_capability(tpd->dev, EV_KEY, KEY_GESTURE_C);
    input_set_capability(tpd->dev, EV_KEY, KEY_GESTURE_DOUBLE_TAP);
...
static int touch_event_handler(void *unused)
{
   ...
   u8  rqst_data[3] = {(u8)(GTP_REG_RQST >> 8), (u8)(GTP_REG_RQST & 0xFF), 0};
    ...
    do
    {
        ...
        {
            case 'c':
                    GTP_DEBUG("tengdq TP Gesture is KEY_GESTURE_C");
            ...
            break;
            case 'o':
                    GTP_DEBUG("tengdq TP Gesture is KEY_GESTURE_O");
            ...
            break;
            case 'z':
                    GTP_DEBUG("tengdq TP Gesture is KEY_GESTURE_Z");
            ...
            break;
            case 'v':
            GTP_DEBUG("tengdq TP Gesture is KEY_GESTURE_V");
            ...
            break;
            case 0xAA:
                    GTP_DEBUG("tengdq TP Gesture is KEY_GESTURE_RIGHT");
            ...
            break;
            case 0xAB:
                    GTP_DEBUG("tengdq TP Gesture is KEY_GESTURE_DOWN");
            ...
            break;
            case 0xBA:
                    GTP_DEBUG("tengdq TP Gesture is KEY_GESTURE_UP");
            ...
            break;
            case 0xBB:
                    GTP_DEBUG("tengdq TP Gesture is KEY_GESTURE_LEFT");
            ...
            break;
            case 0xCC:
                    GTP_DEBUG("tengdq TP Gesture is KEY_GESTURE_DOUBLE_TAP");
            input_report_key(tpd->dev, KEY_GESTURE_DOUBLE_TAP, 1);
            input_sync(tpd->dev);
            input_report_key(tpd->dev, KEY_GESTURE_DOUBLE_TAP, 0);
            input_sync(tpd->dev);
            // clear 0x814B
            doze_buf[2] = 0x00;
            gtp_i2c_write(i2c_client_point, doze_buf, 3);
        ...
        */
        ...
        if (touch_num)
        {
            for (i = 0; i < touch_num; i++)
            {
                ...
                //henry add
                if(0 <= input_x && 56 >= input_x)
                {
                     #ifdef TP_ICN6202_BIG_SCREEN
                     #ifndef NOPOWER
                     if((input_y >= 50) && (input_y <= 70))
                          key_value |= (1 << FIRSTKEY);
                     #endif
                     if((input_y >= 140) && (input_y <= 160))
                          key_value |= (1 << (FIRSTKEY+1));
                     if((input_y >= 225) && (input_y <= 245))
                          key_value |= (1 << (FIRSTKEY+2));
                     if((input_y >= 311) && (input_y <= 331))
                          key_value |= (1 << (FIRSTKEY+3));
                     if((input_y >= 397) && (input_y <= 417))
                          key_value |= (1 << (FIRSTKEY+4));
                     #else
                     #ifndef NOPOWER
                     if((input_y >= 100) && (input_y <= 130))
                          key_value |= (1 << FIRSTKEY);
                     #endif
                     if((input_y >= 175) && (input_y <= 205))
                          key_value |= (1 << (FIRSTKEY+1));
                     if((input_y >= 265) && (input_y <= 295))
                          key_value |= (1 << (FIRSTKEY+2));
                     if((input_y >= 340) && (input_y <= 370))
                          key_value |= (1 << (FIRSTKEY+3));
                     if((input_y >= 430) && (input_y <= 460))
                          key_value |= (1 << (FIRSTKEY+4));
                     #endif
                }
            }
       ...
       {
             int j;
             for(j = 0; j < TPD_KEY_COUNT; j++)
                input_report_key(tpd->dev, touch_keys[j], key_value & (1 << j));
             if((pre_key != 0) && (key_value == 0))
             {
                input_report_key(tpd->dev, BTN_TOUCH, 0);
                input_mt_sync(tpd->dev);
