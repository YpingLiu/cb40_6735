...
static s32 _hotknot_write(struct file *filp, const char __user *buff, unsigned long len, void *data);
static s32 _hotknot_read(char *page, char **start, off_t off, int count, int *eof, void *data);
...
{
  return _hotknot_read(buffer, NULL,0, count, NULL, ppos);
}

static ssize_t hotknot_write(struct file *file, const char __user *buffer,
               size_t count, loff_t *ppos)
{
  return _hotknot_write(file, buffer, count, ppos);
}
...
static s32 _hotknot_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    {
        //    memcpy(page, "gt8", cmd_head2.data_len);
        // memcpy(page, "GT818", 5);
        //  page[5] = 0;
