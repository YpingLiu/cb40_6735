...
///////@20160302 for battery led start
#ifndef CB04_6735_35U
#define CB04_6735_35U
#endif
#ifdef CB04_6735_35U
...
///////@20160302, for battery led start
#ifdef CB04_6735_35U
#define BATTERY_FULL_MAX_PERCENT	100
#define BATTERY_FULL_MIN_PERCENT	99
#define BATTERY_LOW_PERCENT			20
#define LED_BLK_TIME_MS				500
#define CHECK_RLED_TIME_MS			(1*1000)

#define BATTERY_LED_R	(GPIO78 | 0x80000000)
#define BATTERY_LED_G	(GPIO80 | 0x80000000)
//#define BATTERY_LED_B	(GPIO80 | 0x80000000)

#ifdef BATTERY_LED_B
static struct delayed_work battery_status_work;
static void battery_charging_led(struct work_struct *work);
static unsigned int work_flag = 0;
static DECLARE_DELAYED_WORK(battery_status_work, battery_charging_led);
#endif
static struct delayed_work rled_work;
static void battery_charging_rled(struct work_struct *work);
static unsigned int rled_work_flag = 0;
static DECLARE_DELAYED_WORK(rled_work, battery_charging_rled);
#endif
///////@20160302, for battery led end
...
extern struct timespec g_sleep_total_time;
...
/////// @2016-3-2 by bruce start
#ifdef CB04_6735_35U
static int battery_charging_led_control(int charger_flag);
#endif
/////// @2016-3-2 by bruce end
...
#ifdef CB04_6735_35U
	if (1 == chr_status)
		battery_charging_led_control(1);
#endif
/////// @2016-3-2 by bruce end
	return chr_status;
}
					...
					char *buf)
{
    ...
    if (KAL_TRUE == ta_check_chr_type &&
      ...
      BMT_status.UI_SOC >= TA_START_BATTERY_SOC &&
      BMT_status.UI_SOC < TA_STOP_BATTERY_SOC)
    {
...
///////@20160302 by bruce, for battery led start
#ifdef CB04_6735_35U
static int battery_charging_led_control(int charger_flag)
{
//	battery_log(BAT_LOG_CRTI,"BATTERY_STATUS===UI_SOC:%d,charger_exist:%d,bat_vol:%d,ICharging:%d,charger_vol:%d,tSOC:%d,ZCV:%d.\n",BMT_status.UI_SOC,BMT_status.charger_exist,BMT_status.bat_vol,BMT_status.ICharging,BMT_status.charger_vol,BMT_status.SOC,BMT_status.ZCV);

	// set gpio mode
	mt_set_gpio_mode(BATTERY_LED_G, GPIO_MODE_GPIO);
	mt_set_gpio_dir(BATTERY_LED_G, GPIO_DIR_OUT);

	#ifdef BATTERY_LED_B
	mt_set_gpio_mode(BATTERY_LED_B, GPIO_MODE_GPIO);
	mt_set_gpio_dir(BATTERY_LED_B, GPIO_DIR_OUT);
	#endif

	if (1 == BMT_status.charger_exist || 1 == charger_flag) {
		if (BATTERY_FULL_MIN_PERCENT < BMT_status.UI_SOC && BMT_status.UI_SOC <= BATTERY_FULL_MAX_PERCENT) { // battery is full
			mt_set_gpio_out(BATTERY_LED_R, GPIO_OUT_ZERO);
			mt_set_gpio_out(BATTERY_LED_G, GPIO_OUT_ONE);
			#ifdef BATTERY_LED_B
			mt_set_gpio_out(BATTERY_LED_B, GPIO_OUT_ZERO);
			#endif
		}
		else{  // charging
			mt_set_gpio_out(BATTERY_LED_R, GPIO_OUT_ZERO);
			mt_set_gpio_out(BATTERY_LED_G, GPIO_OUT_ZERO);
			#ifdef BATTERY_LED_B
			if (0 == work_flag) {
				schedule_delayed_work(&battery_status_work, (unsigned long)msecs_to_jiffies(LED_BLK_TIME_MS));
				work_flag = 1;
			}
			#endif
		}
		...
		if (0 == rled_work_flag) {
			rled_work_flag = 1;
			schedule_delayed_work(&rled_work, 0);
		}
	}
	return 0;
}
extern int get_poweroff_flag(void);
static void battery_charging_rled(struct work_struct *work)
{
//	battery_log(BAT_LOG_CRTI,"[vibrator][power-off]shutdown_flag:%d,rled_work_flag:%d",get_poweroff_flag(),rled_work_flag);
	if (0 == get_poweroff_flag()) {
		rled_work_flag = 0;
	}
	else {
		rled_work_flag = 1;
	...
	if (1 != BMT_status.charger_exist && 1 == rled_work_flag) {
		mt_set_gpio_mode(BATTERY_LED_R, GPIO_MODE_GPIO);
		mt_set_gpio_dir(BATTERY_LED_R, GPIO_DIR_OUT);
		if (0 < BMT_status.UI_SOC && BMT_status.UI_SOC <= BATTERY_LOW_PERCENT) { // battery is low
			mt_set_gpio_out(BATTERY_LED_R, GPIO_OUT_ONE);
		}
		else {
			mt_set_gpio_out(BATTERY_LED_R, GPIO_OUT_ZERO);
		}
		schedule_delayed_work(&rled_work, (unsigned long)msecs_to_jiffies(CHECK_RLED_TIME_MS));
	}
	else {
		mt_set_gpio_mode(BATTERY_LED_R, GPIO_MODE_GPIO);
		mt_set_gpio_dir(BATTERY_LED_R, GPIO_DIR_OUT);
...
static void battery_charging_led(struct work_struct *work)
{
	...
	if (1 == BMT_status.charger_exist) {
		if (BATTERY_FULL_MIN_PERCENT < BMT_status.UI_SOC && BMT_status.UI_SOC <= BATTERY_FULL_MAX_PERCENT) {
			mt_set_gpio_mode(BATTERY_LED_G, GPIO_MODE_GPIO);
			mt_set_gpio_dir(BATTERY_LED_G, GPIO_DIR_OUT);
			mt_set_gpio_out(BATTERY_LED_G, GPIO_OUT_ONE);
			...
			if (1 == work_flag) {
				mt_set_gpio_out(BATTERY_LED_B, (mt_get_gpio_out(BATTERY_LED_B) ? GPIO_OUT_ZERO : GPIO_OUT_ONE));
				schedule_delayed_work(&battery_status_work, (unsigned long)msecs_to_jiffies(LED_BLK_TIME_MS));
			}
		}
	}
...
#endif
/////// @20160302 by bruce, for battery led end
...
//////// @20160302 by bruce start
#ifdef CB04_6735_35U
	battery_charging_led_control(0);
#endif
//////// @20160302 by bruce end
