...
static bool voice_bt_Status = false;
...
static int mtk_voice_bt1_prepare(struct snd_pcm_substream *substream)
{
    ...
    printk("mtk_voice_bt1_prepare rate = %d  channels = %d period_size = %lu\n",
    ...
    SetDaiBtEnable(true);
    voice_bt1Pcm.mPcmModeWidebandSel = (runtimeStream->rate == 8000) ? Soc_Aud_PCM_MODE_PCM_MODE_8K : Soc_Aud_PCM_MODE_PCM_MODE_16K;
...
struct dev_pm_ops mtk_voice_bt_pm_ops =
{
