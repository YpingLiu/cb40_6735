...
static const char *i2s0_SIDEGEN[] = {"Off", "On48000", "On44100", "On32000", "On16000", "On8000", "On16000MD3"};
...
//    printk("%s() samplerate = %d, mi2s0_hdoutput_control = %d, mi2s0_extcodec_echoref_control = %d\n", __func__, samplerate, mi2s0_hdoutput_control, mi2s0_extcodec_echoref_control);
	...
	/*Afe_Set_Reg(AUDIO_TOP_CON1, (0x1 << 4) | (0x1 << 7),  (0x1 << 4)|  (0x1 << 7)); I2S0 I2S3 clock-gated */
	
        if (mi2s0_extcodec_echoref_control == true)
        {
            if (mi2s0_sidegen_control == 6) 
            {
            	SetConnection(Soc_Aud_InterCon_Connection, Soc_Aud_InterConnectionInput_I01, Soc_Aud_InterConnectionOutput_O27);
            }
            else
            {
            	SetConnection(Soc_Aud_InterCon_Connection, Soc_Aud_InterConnectionInput_I01, Soc_Aud_InterConnectionOutput_O24);
            }
#if 0
	...
	//Afe_Set_Reg(AUDIO_TOP_CON1, (0x0 << 4) | (0x0 << 7),  (0x1 << 4)|  (0x1 << 7));/* Clear I2S0 I2S3 clock-gated */
    ...
    else
    {
        if (mi2s0_extcodec_echoref_control == true)
        {
            if (mi2s0_sidegen_control == 6) 
            {
            	SetConnection(Soc_Aud_InterCon_DisConnect, Soc_Aud_InterConnectionInput_I01, Soc_Aud_InterConnectionOutput_O27);
            }
            else
            {
            	SetConnection(Soc_Aud_InterCon_Connection, Soc_Aud_InterConnectionInput_I01, Soc_Aud_InterConnectionOutput_O24);
...
#if 0 /* avoiding clock gating in FM */	    
	    if (mi2s0_extcodec_echoref_control == true)    
                Afe_Set_Reg(AUDIO_TOP_CON1, 0x1 << 4,  0x1 << 4); /* I2S0 clock-gated */
