...
#define SPK_AMP_MODE_PIN  GPIO66
...
void speak_amp_init()
{
    mt_set_gpio_mode(SPK_AMP_MODE_PIN, GPIO_MODE_00);
    mt_set_gpio_dir(SPK_AMP_MODE_PIN, GPIO_DIR_OUT);
}
void Sound_Speaker_Turnon(int channel)
{
    ...
    mt_set_gpio_out(SPK_AMP_MODE_PIN, 0);
    usleep_range(15, 20);

    for(i = 0; i < 3; i++)
    {
        mt_set_gpio_out(SPK_AMP_MODE_PIN, 1);
        usleep_range(5,10);
        mt_set_gpio_out(SPK_AMP_MODE_PIN, 0);
        usleep_range(5,10);
    }    
...
static int mtmachine_startup(struct snd_pcm_substream *substream)
{
    ...
    if(substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
       Sound_Speaker_Turnon(substream->number);
    return 0;
}
...
static int mtmachine_shutdown(struct snd_pcm_substream *substream)
{
    printk("mtmachine_shutdown \n");
    if(substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
       Sound_Speaker_Turnoff(substream->number);
    return 0;
}
