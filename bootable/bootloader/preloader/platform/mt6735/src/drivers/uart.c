...
//20160310 sxg add for radar, set uart4 parameters
#if defined(GPIO_UART_URXD3_PIN)// && defined(SIMCOM_RADAR)
#define SIMCOM_RADAR_BAUDRATE            (9600)
#endif
...
//20160310 sxg add for radar, set uart4 parameters
#if defined(GPIO_UART_URXD3_PIN)// && defined(SIMCOM_RADAR)
    mtk_serial_set_current_uart(UART3);
    ...
    UART_SET_BITS(UART_FCR_FIFO_INIT, UART_FCR(g_uart)); /* clear fifo */
    UART_WRITE16(UART_NONE_PARITY | UART_WLS_8 | UART_1_STOP, UART_LCR(g_uart));
    serial_setbrg(uartclk, SIMCOM_RADAR_BAUDRATE);
#endif //#if defined(GPIO_UART_URXD3_PIN) && defined(SIMCOM_RADAR)
