...
{    
    ...
    //pchr_turn_on_charging(KAL_TRUE);
    dprintf(CRITICAL, "[mt65xx_bat_init qixin] check VBAT=%d mV with %d mV\n", bat_vol, BATTERY_LOWVOL_THRESOLD);
...
#ifdef NO_LOW_BAT_CON
    /* do nothing if no battery */
    dprintf(CRITICAL, "[BATTERY] no battery control\n");
#else
    dprintf(CRITICAL, "[BATTERY] with battery control\n");
