...
public class Socket implements Closeable {
    ...
    private static SocketCheckHandler mSocketCheckHandler = null;
    ...
    public void setSoLinger(boolean on, int timeout) throws SocketException {
        ...
        System.out.println("[CDS]SO_LINGER:" + on + "/" + timeout);
        if (on) {
    ...
    public void connect(SocketAddress remoteAddr, int timeout) throws IOException {
        ...
        ///M: Support Mom Check @{
        if(this.mSocketCheckHandler != null && isEmailSendPort(port)) {
           System.out.println("Match Mom Check");
           if(!this.mSocketCheckHandler.checkEmailSendRequest()){
    ...
    public synchronized void setSoSndTimeout(int timeout) throws SocketException {
        ...
        System.out.println("[CDS]SO_SND_TIMEOUT:" + timeout);        
        impl.setOption(SocketOptions.SO_SND_TIMEOUT, Integer.valueOf(timeout));
    }
    /**
     * Configure MoM check handler.
     * @hide
     */
    public static void setSocketMomCheckHandler(final SocketCheckHandler socketCheckHandler){
        mSocketCheckHandler = socketCheckHandler;
    }

    /**
     * Check email smtp port number
     * @hide
     */    
    ...
    private static HttpNotifyHandler  mHttpNotifyHandler = null;
    /**
     * Set HTTP notify handler function for HTTP Redirect.
     * @hide
     */
    public static void setHttpRedirectNotifyHandler(final HttpNotifyHandler httpNotifyHandler){        
        mHttpNotifyHandler = httpNotifyHandler;        
    }

    /**
     * Notify HTTP Redirect.
     * @hide
     */
    public static void notifyHttpRedirect(String location) {
    	System.out.println("[NetworkHttpMonitor] notifyHttpRedirect");    	
        if (mHttpNotifyHandler != null) {
			System.out.println("[NetworkHttpMonitor] mHttpNotifyHandler.notifyHttpRedirect");			
            mHttpNotifyHandler.notifyHttpRedirect(location);
        }
    }    
