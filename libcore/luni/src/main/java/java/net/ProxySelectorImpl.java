...
final class ProxySelectorImpl extends ProxySelector {
    ...
    private Proxy selectOneProxy(URI uri) {
        ...
        String host = "";
        boolean isLocal = isLocalHost(uri);
        Proxy chinaProxy = new Proxy(Proxy.Type.HTTP,
                    InetSocketAddress.createUnresolved("10.0.0.172", 80));
        /** @} */
        ...
        if (proxy != null) {
            ...
            ///M: Support China Proxy for localhost @{
            if (proxy == chinaProxy && isLocal) {
                System.out.println("No proxy for 10.0.0.172 & localhost");;
                return Proxy.NO_PROXY;
            }
            //@}
    ...
    //M: Fix localhost over proxy issue @{ */
    private static final boolean isLocalHost(URI uri) {
        if (uri == null) {
            return false;
        }
