...
public abstract class URLConnection {
    ...
    ///M: Support write timeout in HttpUrlConnection class
    /**
       * Sets the maximum time to wait for an output stream write to complete before
       * giving up. Reading will fail with a {@link SocketTimeoutException} if the
       * timeout elapses before data becomes available. The default value of
       * {@code 0} disables read timeouts; write attempts will block indefinitely.
       *
       * @hide
       *
       * @param timeoutMillis the read timeout in milliseconds. Non-negative.
    ...
    /**
       * Returns the write timeout in milliseconds, or {@code 0} if writes never
       * timeout.
       *
       * @hide
       *   
