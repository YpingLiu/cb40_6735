...
int _mtk_nfc_sys_thread_exit(MTK_NFC_TASKID_E TaskId)
{
    ...
    {
        LOG_AOSP("_mtk_nfc_sys_thread_exit...(type: %dmsg: %p)\n",MTK_NFC_EXIT_REQ,prmsg);
        ...
        {
            LOG_AOSP("_mtk_nfc_sys_thread_exit fail\n");
            free(prmsg);
            return 1;
        }
    }
...
static int hal_open (const struct nfc_nci_device *p_dev, nfc_stack_callback_t *p_hal_cback, nfc_stack_data_callback_t *p_hal_data_callback)
{
    ...
    LOG_AOSP ("%s: exit %d" , __FUNCTION__, retval);
...
static int hal_pre_discover (const struct nfc_nci_device *p_dev)
{
    ...
    else
    {
        ...
        LOG_AOSP("Note: hal_pre_discover return 1 to indicate upper layer wait our HAL_NFC_PRE_DISCOVER_CPLT_EVT");
    }
...
int main (int argc, char** argv)
{
    ...
    g_NfcSmallAntennaMode = mtk_nfc_small_antenna_check();
