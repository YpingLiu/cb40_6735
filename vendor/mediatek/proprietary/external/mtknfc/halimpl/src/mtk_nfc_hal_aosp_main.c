...
void LOG_print_module(char* module,char *str, ...)
{
#define LOCAL_DBG_BUF (256*4)
    ...
    sprintf(trace, "%s%s", module ,str);
    
    va_start(args, trace);
    vsnprintf(szBuf, LOCAL_DBG_BUF-strlen(module), trace, args);
    va_end(args);

    mtk_nfc_sys_dbg_string(szBuf);
}
...
NFCSTATUS android_nfc_aosp_hal_send_msg(unsigned int type, unsigned int len, void *payload)
{
    ...
    if (prmsg == NULL)
    {
        LOG_AOSP("malloc msg fail\n");
        return FALSE;
    }
    else
    {
        LOG_AOSP("android_nfc_aosp_hal_send_msg...(type: %d, len: %d, payload: %p,msg: %p)\n",type, len, payload,prmsg);
        ...
        {
            LOG_AOSP("android_nfc_aosp_hal_send_msg fail\n");
            free(prmsg);
...
void _MtkNfcHalAosp_EvtCB(nfc_event_t event, UINT32 status)
{
    ...
    LOG_AOSP("_MtkNfcHalAosp_EvtCB evet:0x%x, status:%d" ,event, (int)event_status  );
...
{
    LOG_AOSP("_MtkNfcHalAosp_DataCB len %d", len  );
...
NFCSTATUS MtkNfcHalAosp_Open(nfc_stack_callback_t *p_halCB, nfc_stack_data_callback_t *p_hal_dataCB)
{
    ...
    LOG_AOSP("MtkNfcHalAosp_Open\n");
    if ((!p_halCB) || (!p_hal_dataCB))
    {
        LOG_AOSP("%s Invalid callback function pointers", __FUNCTION__);
        return MTK_NFC_ERROR;
    }
...
NFCSTATUS MtkNfcHalAosp_Core_inited(void){
    ...
    LOG_AOSP("MtkNfcHalAosp_Core_inited\n");
...
NFCSTATUS MtkNfcHalAosp_PreDiscover(void)
{
    ...
    LOG_AOSP("MtkNfcHalAosp_PreDiscover\n");
...
NFCSTATUS MtkNfcHalAosp_Ctrl_granted(void){
    ...
    LOG_AOSP("MtkNfcHalAosp_Ctrl_granted\n");
...
NFCSTATUS MtkNfcHalAosp_Power_Cycle(void){
    ...
    LOG_AOSP("MtkNfcHalAosp_Power_Cycle\n");
...
NFCSTATUS MtkNfcHalAosp_write(UINT8* pWriteBuf,UINT32 WriteLen)
{
    ...
    LOG_AOSP("MtkNfcHalAosp_write,WriteLen %d\n",WriteLen);
...
NFCSTATUS MtkNfcHalAosp_Init(void)
{
    ...
    {
        ALOGE(" PreDiscover Semaphore creation failed (errno=0x%x)", errno);
        //ret = MTK_NFC_ERROR;
    }
#endif
    return ret;
...
mtk_nfc_HalAosp_proc (UINT8 *prmsg)
{
    ...
    if (prmsg == NULL)
    {
        LOG_AOSP("mtk_nfc_HalAosp_proc, prmsg null\r\n");
        return MTK_NFC_ERROR;
    }
    ...
    LOG_AOSP("mtk_nfc_HalAosp_proc, proc msg type %d\r\n",pMsg->type);
    ...
    case MTK_NFC_AOSP_HAL_OEPN_RES:
        {
            LOG_AOSP("MTK_NFC_AOSP_HAL_OEPN_RES\r\n");
    ...
    case MTK_NFC_AOSP_HAL_CLOSE_RES:
        {
            LOG_AOSP("MTK_NFC_AOSP_HAL_CLOSE_RES\r\n");
    ...
    case MTK_NFC_AOSP_HAL_WRITE_RES:
        {
            LOG_AOSP("MTK_NFC_AOSP_HAL_WRITE_RES\r\n");
    ...
    case MTK_NFC_AOSP_HAL_CORE_INITED_RES:
        {
            LOG_AOSP("MTK_NFC_AOSP_HAL_CORE_INITED_RES\r\n");
    ...
    case MTK_NFC_AOSP_HAL_PRE_DISCOVERY_RES:
        {
            ...
            LOG_AOSP("MTK_NFC_AOSP_HAL_PRE_DISCOVERY_RES , result \r\n",pPreDiscoveryRes->u4Result);
    ...
    case MTK_NFC_AOSP_HAL_CTRL_GRANT_NTF:
        {
            LOG_AOSP("MTK_NFC_AOSP_HAL_CTRL_GRANT_NTF\r\n");
    ...
    case MTK_NFC_AOSP_HAL_CTRL_GRANT_RES:
        {
            LOG_AOSP("MTK_NFC_AOSP_HAL_CTRL_GRANT_RES\r\n");
    ...
    case MTK_NFC_AOSP_HAL_PWR_CYCLE_RES:
        {
            LOG_AOSP("MTK_NFC_AOSP_HAL_PWR_CYCLE_RES\r\n");
    ...
    case MTK_NFC_EXIT_REQ:
        return MTK_NFC_ERROR;
    default:
        LOG_AOSP("mtk_nfc_HalAosp_proc, unknow msg type %d\n",pMsg->type);
...
void* mtk_nfc_HalAosp_thread(void * arg)
{
    ...
    while (!gAospHalThreadExit)
    {
        ...
        if (ret == MTK_NFC_SUCCESS && (!gAospHalThreadExit))
        {
            ...
            {
                LOG_AOSP("TRIGGER EXIT Hal Aosp thread");
                gAospHalThreadExit = TRUE;
                break;
            }
        ...
        {
            //read msg fail...        
            LOG_AOSP("mtk_nfc_HalAosp_thread, read msg fail\n");
