...
public:
    ...
    virtual MBOOL get3AEXIFInfo(EXIF_INFO_T& rEXIFInfo) const;
	...
	*/
    ...
    virtual MVOID getMShutterCaptureParams(CaptureParam_T &a_rCaptureInfo); 
    virtual MINT32 getShutterValue(); 
...
private:
	...
	*/
	...
	/**
	* @brief PD virtual channel thread setting
	*/
    MVOID           changePDVCThreadSetting();
	/**
	...
	*/
	...
	/**
	* @brief PD virtul channel thread execution function
	*/
	static MVOID* PDVCThreadLoop(MVOID*);
...
public:
	...
	MVOID start(const char* info, MINT32 sensorDevId, MBOOL logEn, MBOOL bClrTime = 1) //used by global/static variables
	{
		...
		mIdx = sensorDevId;
		mLogEn = logEn;
        if (mLogEn) XLOGD("[Timer:%s: %s, SensorDevId: %d] timer start. bClrTime=%d\n", __FUNCTION__, mInfo, mIdx, bClrTime);
	...
	double End()
	{
        ...
        mTime_ms += t;
        */
        if (mLogEn) XLOGD("[Timer:%s: %s, SensorDevId: %d] =====> time(ms): current=%f,   sum=%f\n", __FUNCTION__, mInfo, mIdx, ts_elapsed_tv_msec, mTime_ms);
        return mTime_ms;
	}

	double printTime()
	{
        if (mLogEn) XLOGD("[Timer:%s: %s, SensorDevId: %d] =====> time(ms): sum=%f\n", __FUNCTION__, mInfo, mIdx, mTime_ms);
        return mTime_ms;
	}
