...
public class WorldPhoneOm extends Handler implements IWorldPhone {
    ...
    public WorldPhoneOm() {
        ...
        for (int i = 0; i < PROJECT_SIM_NUM; i++) {
            ...
            if (WorldPhoneUtil.isCdmaLteDcSupport() &&
                    sProxyPhones[i] instanceof SvltePhoneProxy) {
                sSvlteLteCi[i].setOnPlmnChangeNotification(this, EVENT_REG_PLMN_CHANGED_SVLTE_1 + i, null);
                sSvlteLteCi[i].setOnRegistrationSuspended(this, EVENT_REG_SUSPENDED_SVLTE_1 + i, null);
                sSvlteLteCi[i].registerForOn(this, EVENT_RADIO_ON_SVLTE_1 + i, null);
                sSvlteLteCi[i].setInvalidSimInfo(this, EVENT_INVALID_SIM_NOTIFY_SVLTE_1 + i, null);
                //[ALPS02156732]
                sSvlteLteCi[i].registerForGmssRatChanged(this, EVENT_WP_GMSS_RAT_CHANGED_1 + i, null);
            }
            //C2K world phone - end
        ...
        //[ALPS02045100]
        if (WorldPhoneUtil.isCdmaLteDcSupport()) {
            sUiccController.getInstance().registerForC2KWPCardTypeReady(this, EVENT_WP_CARD_TYPE_READY, null);
        }
    ...
    private final BroadcastReceiver mWorldPhoneReceiver = new  BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(TelephonyIntents.ACTION_SIM_STATE_CHANGED)) {
                ...
                //[ALPS02083291]-End: reset whan sim ready
    ...
    private void handleSwitchModem(int toModem) {
        ...
        if (!sIsAutoSelectEnable) {
            ...
            if (sDefaultBootuUpModem == ModemSwitchHandler.MD_TYPE_UNKNOWN) {
                ...
                if (WorldPhoneUtil.isLteSupport()) {
                    logd("[handleSwitchModem] Storing modem type: " + ModemSwitchHandler.MD_TYPE_WG);
                    //C2K world phone
                    ...
                    logd("[handleSwitchModem] Storing modem type: " + ModemSwitchHandler.MD_TYPE_LWG);
                    //C2K world phone
        ...
        if (sNwPlmnStrings[0] != null) {
            ...
            if (isAllowCampOn(plmnString, slotId)) {
                ...
                if (!WorldPhoneUtil.isCdmaLteDcSupport()) {
                    ...
                    CommandsInterface ci = getSvlteActiveCi(slotId);
                    if (ci != null) {
                        ci.setResumeRegistration(sSuspendId[slotId], obtainMessage(EVENT_RESUME_CAMPING));
                    }
                }
                //C2K world phone - end
            } else {
