...
public class SvlteModeController extends Handler {
    ...
    private static int sCdmaSocketSlotId = getCdmaSocketSlotId();
    ...
    */
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            case EVENT_CDMA_RADIO_CHANGE_FOR_AVALIABLE:
                if (mC2KCardTypeReady) {
                    tryToSwitchMode();
                }
                break;
            case EVENT_C2K_WP_CARD_TYPE_READY:
                mC2KCardTypeReady = true;
                tryToSwitchMode();
                break;
    ...
    private boolean mC2KCardTypeReady = false;
    private boolean mAlreadyRegistForRadioState = false;
    private static final int EVENT_CDMA_RADIO_CHANGE_FOR_AVALIABLE = 104;

    private void tryToSwitchMode() {
        RadioState rs = ((SvltePhoneProxy) PhoneFactory.getPhone(
            sCdmaSocketSlotId)).getNLtePhone().mCi.getRadioState();

        logicLog("tryToSwitchMode, RadioState: " + rs
            + " mC2KCardTypeReady: " + mC2KCardTypeReady
            + " mAlreadyRegistForRadioState: " + mAlreadyRegistForRadioState);

        if (rs != RadioState.RADIO_UNAVAILABLE && mC2KCardTypeReady) {
            mC2KCardTypeReady = false;
            if (mAlreadyRegistForRadioState) {
                ((SvltePhoneProxy) PhoneFactory.getPhone(sCdmaSocketSlotId))
                        .getNLtePhone().mCi.unregisterForRadioStateChanged(this);
                mAlreadyRegistForRadioState = false;
            }
            onC2kCardTypeReady();
        } else {
            if (!mAlreadyRegistForRadioState) {
                mAlreadyRegistForRadioState = true;
                ((SvltePhoneProxy) PhoneFactory.getPhone(sCdmaSocketSlotId))
    ...
    private void doSwitchRadioTech() {
        ...
        for (int i = 0; i < sCardModes.length; i++) {
            ...
            if (i == sCdmaSocketSlotId && sCardModes[i] == RADIO_TECH_MODE_CSFB) {
                firstSwitchSlot = i;
            }
    ...
    public static void setCdmaSocketSlotId(int slotId) {
        if (SvlteUtils.isValidateSlotId(slotId) && sCdmaSocketSlotId != slotId) {
    ...
    private void configModemStatus() {
        ...
        if (!mWaitingRilSocketConnect && !mConfgiC2kDone) {
            logicLog("configModemStatus for C2K, cdmaSocketId=" + sCdmaSocketSlotId);
            ((SvltePhoneProxy) proxyPhones[sCdmaSocketSlotId]).getNLtePhone().configModemStatus(
    ...
    private boolean isSwitchInAirplaneMode() {
        if (!CdmaFeatureOptionUtils.isFlightModePowerOffModemSupport()) {
            return false;
        }
