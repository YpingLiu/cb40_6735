...
public class BipManager {
    ...
    private static final String PROPERTY_IA_APN = "persist.radio.ia-apn";
                ...
                if (mApn != null && mApn.length() > 0) {
                    ...
                    if (myPhone != null) {
                        ...
                        if (TelephonyManager.NETWORK_TYPE_LTE == dataNetworkType) {
                            ...
                            if (iaApn != null && iaApn.length() > 0 && iaApn.equals(mApn)) {
...
//FIXME: multiple network for MSIN ?
    ...
    private boolean checkDataCapability(CatCmdMessage cmdMsg) {
        ...
        int subId[] = SubscriptionManager.getSubId(mSlotId);
        int currentPhoneId = SubscriptionManager.getPhoneId(subId[0]);
        CatLog.d("[BIP]", "checkDataCapability: simInsertedCount:" + simInsertedCount
            + " currentPhoneId:" + currentPhoneId + " capabilityPhoneId:"
            + capabilityPhoneId);
        if (simInsertedCount >= 2 &&
            ...
            currentPhoneId != capabilityPhoneId) {
