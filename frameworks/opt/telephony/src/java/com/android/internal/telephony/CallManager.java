...
public final class CallManager {
    ...
    public boolean registerPhone(Phone phone) {
        ...
        if (basePhone != null && !mPhones.contains(basePhone)) {
            ...
            /* M: Bug Fix for CR ALPS02224060 */
            registerForPhoneStates(basePhone, phone);
            /* M: Bug Fix end*/
            return true;
        }
        return false;
    ...
    public void unregisterPhone(Phone phone) {
        ...
        if (basePhone != null && mPhones.contains(basePhone)) {
            ...
            /* M: Bug Fix for CR ALPS02224060 */
            unregisterForPhoneStates(basePhone, phone);
            /* M: Bug Fix end */
    ...
    */
    ...
    private void registerForPhoneStates(Phone phone, Phone phoneMmi) {
        ...
        /* M: Bug Fix for CR ALPS02224060 */
        if (phoneMmi instanceof SvltePhoneProxy
                && SvlteUiccUtils.getInstance().isUsimCsimPhone(phoneMmi.getPhoneId())) {
            phoneMmi.registerForMmiInitiate(handler, EVENT_MMI_INITIATE, null);
            phoneMmi.registerForMmiComplete(handler, EVENT_MMI_COMPLETE, null);
        } else {
    ...
    private void unregisterForPhoneStates(Phone phone, Phone phoneMmi) {
        ...
        if (phoneMmi instanceof SvltePhoneProxy
                && SvlteUiccUtils.getInstance().isUsimCsimPhone(phoneMmi.getPhoneId())) {
            phoneMmi.unregisterForMmiInitiate(handler);
            phoneMmi.unregisterForMmiComplete(handler);
        } else {
    ...
    */
    ...
    private void resetImsPdnWhenUssdComplete() {
        ITelephonyExt telphonyExt = (ITelephonyExt) MPlugin.createInstance(ITelephonyExt.class.getName(), getContext());
        if (telphonyExt != null) {
             telphonyExt.resetImsPdnOverSSComplete(getContext());
             Rlog.d(LOG_TAG, "resetImsPdnWhenUssdComplete");
        } else {
            Rlog.d(LOG_TAG, "Fail to initialize ITelephonyExt");
        }
    }
