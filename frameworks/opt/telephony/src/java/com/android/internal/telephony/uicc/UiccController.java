...
public class UiccController extends Handler {
    ...
    private class InitCardtypeRunnable implements Runnable {
        Integer retryIndex;
        public InitCardtypeRunnable(Integer index) {
            retryIndex = index;
        }
        @Override
        public void run() {
            initC2KWPCardtype(retryIndex);
        }
    }
    private void initC2KWPCardtype(Integer index) {
        ...
        if (phoneCount == 1) {
            ...
            if (index == 0) {
                ...
                cardTypeReady = cardTypeSet_2.equals("1");
            } else {
        ...
        if (cardTypeReady) {
            ...
            InitCardtypeRunnable initCardtypeRunnable = new InitCardtypeRunnable(index);
                ...
                case EVENT_SIM_PLUG_OUT:
                    ...
                    if (CdmaFeatureOptionUtils.isCdmaLteDcSupport()) {
                        ...
                        */
                        ...
                        boolean md3SleepNeedReset = false;
                        if (CdmaFeatureOptionUtils.isCdmaLteDcSupport()
                            && !mOperatorSpec.equals(OPERATOR_OP09)
                            && !CdmaFeatureOptionUtils.isC2KWorldPhoneP2Support()) {
                            Rlog.d(LOG_TAG, "C2K OM solution1.5, check if MD3 esleep need reset");
                            // C+G ES3G=2 & G+C ES3G=1 and plug out GSM need reset
                            int capability = SystemProperties.
                                    getInt(PhoneConstants.PROPERTY_CAPABILITY_SWITCH, 1);
                            boolean isPlugOutGsm = ((CARD_TYPE_NONE < mC2KWPCardtype[index])
                                && (mC2KWPCardtype[index] < CARD_TYPE_RUIM));
                            if ((index == 0 && isPlugOutGsm && isCdmaCard(1) && capability == 1) ||
                                (index == 1 && isPlugOutGsm && isCdmaCard(0) && capability == 2)) {
                                Rlog.d(LOG_TAG, "C2K OM solution1.5, md3SleepNeedReset = true");
                                md3SleepNeedReset = true;
                            }
                        }
...
*/
    ...
    public void unSetSvlteCi(CommandsInterface ci) {
        ...
        mSvlteCi.unregisterForCommonSlotNoChanged(this);
    }
    /**
      * Set svlte index.
    ...
    public void switchCardType(int slot, int cardtype) {
        ...
        log("SwitchCardType slot = " + slot + "  cardtype = " + cardtype +
                "  mSwitchCardtype = " + mSwitchCardtype);
        //Switch c2k modem card type
        if (slot == SvlteModeController.getActiveSvlteModeSlotId()) {
            SvlteUtils.getSvltePhoneProxy(slot).getNLtePhone().mCi.switchCardType(cardtype);
        }
        //Switch gsm modem card type
