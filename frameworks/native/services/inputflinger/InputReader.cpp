...
void TouchInputMapper::configureSurface(nsecs_t when, bool* outResetNeeded) {
    ...
    if (viewportChanged) {
        ...
        if (mDeviceMode == DEVICE_MODE_DIRECT || mDeviceMode == DEVICE_MODE_POINTER) {
            ...
            mSurfaceOrientation += mParameters.orientationAware ?
                    mViewport.orientation : DISPLAY_ORIENTATION_0;
            mSurfaceOrientation %= (DISPLAY_ORIENTATION_270 + 1);
            #endif
            /*>>>*/
        } else {
    ...
    if (currentIdBits == lastIdBits) {
        ...
        while (!upIdBits.isEmpty()) {
            uint32_t upId = upIdBits.clearFirstMarkedBit();
            {
            	...
            	ALOGD("AP_PROF:AppLaunch_dispatchPtr:Up:%lld, ID:%d, Index:%d", when/1000000, upId, mLastCookedPointerData.idToIndex);
        ...
        while (!downIdBits.isEmpty()) {
            ...
            if (dispatchedIdBits.count() == 1) {
                ...
                mDownTime = when;
				{
					...
					ALOGD("AP_PROF:AppLaunch_dispatchPtr:Down:%lld, ID:%d, Index:%d", mDownTime/1000000, downId, mCurrentCookedPointerData.idToIndex);
					mtProf(1);
            }
            }
