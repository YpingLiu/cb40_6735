...
void AwSettings::PopulateWebPreferencesLocked(
  ...
  web_prefs->text_fit_column_to_screen_enabled =
      Java_AwSettings_getTextFitColumnToScreenEnabledLocked(env, obj);
  if (text_size_percent == 100)
  ...
  /// M: set double tap factor
  web_prefs->double_tap_factor =
      Java_AwSettings_getDoubleTapPercentLocked(env, obj) / 100.0f;
  ...
  /// M: set auto encoding detector
  web_prefs->uses_universal_detector =
      Java_AwSettings_getUsesEncodingDetectorLocked(env, obj);
