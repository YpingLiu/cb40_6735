...
/// M: To record time duration of ondraw and drawgl @{
struct RecordFunctionDuration {
  enum RecordType {
    RecordType_OnDraw,
    RecordType_DrawGL
  };
  RecordFunctionDuration(RecordType type)
    : type_(type) {

    switch(type) {
    case RecordType_OnDraw:
      base::android::SysUtils::RecordOnDrawStartTime();
      break;
    case RecordType_DrawGL:
      base::android::SysUtils::RecordDrawGLStartTime();
      break;
    }
  }
  ~RecordFunctionDuration() {
    switch(type_) {
    case RecordType_OnDraw:
      base::android::SysUtils::RecordOnDrawEndTime();
      break;
    case RecordType_DrawGL:
      base::android::SysUtils::RecordDrawGLEndTime();
      break;
    }
  }
  RecordType type_;
...
void AwContents::InitAutofillIfNecessary(bool enabled) {
  ...
  /// M: add save password handler
  AwPasswordHandlerDelegateImpl::CreateForWebContents(web_contents);
  PasswordHandler::CreateForWebContentsAndDelegate(
      web_contents,
      AwPasswordHandlerDelegateImpl::FromWebContents(web_contents));
}
...
void AwContents::DrawGL(AwDrawGLInfo* draw_info) {
  ...
  /// M: Schedule idle task to avoid block drawGL @{
  {
    RecordFunctionDuration rfd(RecordFunctionDuration::RecordType_DrawGL);
...
bool AwContents::SavePage(JNIEnv* env, jobject obj) {
  web_contents_->OnSavePage();
  return true;
}

void AwContents::GetSaveDir(bool can_save_as_complete) {
  LOG(WARNING) << "SP>>> GetSaveDir";
  JNIEnv* env = AttachCurrentThread();
  ScopedJavaLocalRef<jobject> obj = java_ref_.get(env);
  if (obj.is_null())
    return;
  Java_AwContents_getSaveDir(env, obj.obj(), can_save_as_complete);
}

void AwContents::SetSaveDir(JNIEnv* env, jobject obj, jstring value) {
  LOG(WARNING) << "SP>>> SetSaveDir path:" << ConvertJavaStringToUTF8(env, value);
  receiver_->SetSaveDir(ConvertJavaStringToUTF8(env, value));
}

void AwContents::SetReceiver(Receiver* r) {
  receiver_ = r;
}

void AwContents::OnSavePageStart(const uint32 id, const std::string& path) {
  LOG(WARNING) << "SP>>> OnSavePageStart id:" << id << ", path:" << path;
  is_destroy_ = false;
  LOG(WARNING) << "SP>>> Set is_destroy_ as False";
  JNIEnv* env = AttachCurrentThread();
  ScopedJavaLocalRef<jobject> obj = java_ref_.get(env);
  if (obj.is_null())
    return;
  ScopedJavaLocalRef<jstring> jpath = ConvertUTF8ToJavaString(env, path);
  Java_AwContents_onSavePageStart(env, obj.obj(), id, jpath.obj());
}

void AwContents::OnSaveProgressChange(const uint32 id, const int progress) {
  LOG(WARNING) << "SP>>> OnSaveProgressChange id:" << id << ", progress:" << progress;
  JNIEnv* env = AttachCurrentThread();
  ScopedJavaLocalRef<jobject> obj = java_ref_.get(env);
  if (obj.is_null())
    return;
  LOG(WARNING) << "SP>>> obj was not null";
  Java_AwContents_onSaveProgressChange(env, obj.obj(), id, progress);
}

void AwContents::OnSaveFinish(const uint32 id, const int flag) {
  LOG(WARNING) << "SP>>> OnSaveFinish id:" << id << ", flag:" << flag;
  JNIEnv* env = AttachCurrentThread();
  ScopedJavaLocalRef<jobject> obj = java_ref_.get(env);
  if (obj.is_null())
    return;
  LOG(WARNING) << "SP>>> obj was not null";
  Java_AwContents_onSaveFinish(env, obj.obj(), id, flag);
}
...
void AwContents::ReleaseHardwareDrawIfNeeded(bool lost_context) {
  ...
  if (hardware_initialized) {
    ...
    /// M: for stream texture leak
    browser_view_renderer_.ReleaseHardware(lost_context);
  }
  DCHECK(!hardware_renderer_);

  /// M: Request Draw GL before remove key from GLViewRendererManager
  ///  otherwise, it will cause a dead lock on InProcessCommandBuffer
  if (hardware_initialized) {
...
bool AwContents::OnDraw(JNIEnv* env,
                        ...
                        jint visible_right,
                        jint visible_bottom) {
  /// M: Record drawing time
  RecordFunctionDuration rfd(RecordFunctionDuration::RecordType_OnDraw);
  DCHECK(BrowserThread::CurrentlyOn(BrowserThread::UI));
...
void AwContents::TrimMemory(JNIEnv* env,
  ...
  if (level >= TRIM_MEMORY_MODERATE) {
    /// M: for stream texture leak
    ReleaseHardwareDrawIfNeeded(true);
    return;
  }
...
void AwContents::DumpMemoryStatus(JNIEnv* env, jobject obj) {
  render_view_host_ext_->DumpMemoryStatus();
}
/// @}
