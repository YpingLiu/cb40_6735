...
AwContentsClientBridge::~AwContentsClientBridge() {
  ...
  /// M: delete the dialog callback @{
  if (!pending_js_dialog_callbacks_.IsEmpty()) {
      IDMap<content::JavaScriptDialogManager::DialogClosedCallback, IDMapOwnPointer>::const_iterator
          iter(&pending_js_dialog_callbacks_);
      while (!iter.IsAtEnd()) {
        int id = iter.GetCurrentKey();
        LOG(WARNING) << "Found pending JavaScript dialog callback, id=" << id;
