...
void BrowserViewRenderer::SetIsPaused(bool paused) {
  ...
  if (compositor_)
    compositor_->SetViewIsPaused(paused);
  /// @}
...
void BrowserViewRenderer::OnAttachedToWindow(int width, int height) {
  ...
  /// M: for tab performance @{
  if (compositor_ && compositor_->HasContentToDraw()) {
    cc::RasterWorkerCoordinator::GetInstance()->OnAttachToWindow();
  }
  /// @}
}
...
void BrowserViewRenderer::ReleaseHardware(bool lost_context) {
  DCHECK(hardware_enabled_);
  ...
  if (compositor_) {
    /// M: for stream texture leak
    compositor_->ReleaseHwDraw(lost_context);
    SynchronousCompositorMemoryPolicy zero_policy;
    RequestMemoryPolicy(zero_policy);
  }
...
void BrowserViewRenderer::ScrollTo(gfx::Vector2d scroll_offset) {
  ...
  base::android::SysUtils::RecordScrollToTime();
...
void BrowserViewRenderer::FallbackTickFired() {
  ...
  /// M: By timing, FallbackTick maybe scheduled before onDetachedFromWindow @{
  ///   Make sure again is it need to run
  bool throttle_fallback_tick =
      (is_paused_ && !clear_view_) || (attached_to_window_ && !window_visible_);
  if (throttle_fallback_tick) {
    LOG(INFO)<<"[FallbackTickFired] ["<<this<<"] ["<<is_paused_<<"]["<<clear_view_<<"]["
      <<attached_to_window_<<"]["<<window_visible_<<"] => cancel throttleFallback !";
    return;
  }
  /// @}
