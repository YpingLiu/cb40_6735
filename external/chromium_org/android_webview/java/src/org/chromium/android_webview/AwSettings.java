...
public class AwSettings {
    ...
    /// M: Double tap percent, default is 100.
    private int mDoubleTapPercent = 100;
    ...
    /// M: add for auto detector @ {
    private static final String AUTO_DETECTOR = "auto-detector";
    private String mDefaultTextEncoding = AUTO_DETECTOR;
    private boolean mUsesEncodingDetector = true;
    /// @ }
    ...
    public AwSettings(Context context,
       ...
       boolean hasInternetPermission = context.checkPermission(
        ...
        synchronized (mAwSettingsLock) {
            ...
            /// M: Support text-reflow in chromium
            mIsBrowserApp = context.getPackageName().equals("com.android.browser");
        }
    ...
    /**
     * M: save password setting item switch
     * @param save whether to enable the save password feature
     */
    public void setSavePassword(boolean save) {
        AwPasswordHandler.setSavePassword(save);
    }
    ...
    /**
     * Assign the default zoom ratio of double tap.
     * @param doubleTapZoom assign the default zoom ratio of double tap.
     */
    public void setDoubleTapZoom(int doubleTapZoom) {
        synchronized (mAwSettingsLock) {
            if (mDoubleTapPercent != doubleTapZoom) {
                mDoubleTapPercent = doubleTapZoom;
                mEventHandler.updateWebkitPreferencesLocked();
            }
        }
    }
    ...
    /**
     * M: Get double tap zoom.
     */
    @CalledByNative
    private int getDoubleTapPercentLocked() {
        return mDoubleTapPercent;
    }
    ...
    /**
     * Gets whether Text Fit-Column-To-Screen layout algorithm is enabled.
     *
     * @return true if Text Fit-Column-To-Screen layout algorithm is enabled
     * @hide
     */
    @CalledByNative
    private boolean getTextFitColumnToScreenEnabledLocked() {
        assert Thread.holdsLock(mAwSettingsLock);
        return mIsBrowserApp ? true : false;
    }
    ...
    /**
     * M: Whether use auto encodingDetector or not.
     */
    @CalledByNative
    private boolean getUsesEncodingDetectorLocked() {
        return mUsesEncodingDetector;
    }
