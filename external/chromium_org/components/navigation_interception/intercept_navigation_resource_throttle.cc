...
/// M: When it's browser process and the scheme is file, don't ask UI thread to ignore or not
...
bool InterceptNavigationResourceThrottle::CheckIfShouldIgnoreNavigation(
  ...
  /// M: When it's browser process and the scheme is file, don't ask UI thread to ignore or not @{
  if(LowerCaseEqualsASCII(url.scheme(), "file") &&
    base::SysInfo::IsBrowserProcess()) {
    LOG(WARNING) << "CheckIfShouldIgnoreNavigation return false because of url is prefixed with file";
    return false;
  }
  /// @}
