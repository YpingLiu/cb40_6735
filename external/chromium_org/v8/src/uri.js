...
(function() {
  ...
  function URIAddEncodedOctetToBuffer(octet, result, index) {
    /// M: avoid unnecessary array access @{
    result[index] = 37; // Char code of '%'.
    var hi = octet >> 4;
    // 0-15 -> char code of '0'-'9', 'A'-'F'
    if (hi < 10) {
        result[index+1] = hi + 48; // 48 = char code('0') - 0
    } else {
        result[index+1] = hi + 55; // 55 = char code('A') - 10
    }
    var lo = octet & 0x0F;
    if (lo < 10) {
        result[index+2] = lo + 48;
    } else {
        result[index+2] = lo + 55;
    }
    return index+3;
    /// @}
  }
  ...
  function URIEncodeSingle(cc, result, index) {
    ...
    if (cc <= 0x007F) {
      var octets0 = cc;
      URIAddEncodedOctetToBuffer(octets0, result, index);
      return index + 3;
    } else if (cc <= 0x07FF) {
      var y = (cc >> 6) & 63;
      var z = cc & 63;
      ...
      var octets1 = z + 128;
      URIAddEncodedOctetToBuffer(octets0, result, index);
      URIAddEncodedOctetToBuffer(octets1, result, index+3);
      return index + 6;
    } else {
      var x = (cc >> 12) & 0xF;
      var y = (cc >> 6) & 63;
      ...
      URIAddEncodedOctetToBuffer(octets0, result, index);
      URIAddEncodedOctetToBuffer(octets1, result, index+3);
      URIAddEncodedOctetToBuffer(octets2, result, index+6);
      return index + 9;
    }
    /// @}
  }
  ...
  function URIEncodePair(cc1 , cc2, result, index) {
    ...
    URIAddEncodedOctetToBuffer(octets1, result, index+3);
    URIAddEncodedOctetToBuffer(octets2, result, index+6);
    URIAddEncodedOctetToBuffer(octets3, result, index+9);
    return index + 12;
    /// @}
  }
