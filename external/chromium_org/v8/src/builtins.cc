...
BUILTIN(ArraySplice) {
  ...
  if (n_arguments == 1) {
    ...
    if (n_arguments > 1) {
      ...
      /// M: Array.splice(start, delete_count, ...) delete_count may be heap number, handle this case here @{
      } else if (arg2->IsHeapNumber()){
        double heap_value = HeapNumber::cast(arg2)->value();
        if (heap_value < kMinInt || heap_value > kMaxInt) {
          AllowHeapAllocation allow_allocation;
          return CallJsBuiltin(isolate, "ArraySplice", args);
        }
        value = std::isnan(heap_value) ? 0 : static_cast<int>(heap_value);
      /// @}
      } else {
        AllowHeapAllocation allow_allocation;
        return CallJsBuiltin(isolate, "ArraySplice", args);
