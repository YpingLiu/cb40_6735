...
/// M: restore input handler
  : need_restore_routing_id_(0)
  , need_restore_input_handler_(0) {
/// @}
}
...
void SynchronousInputEventFilter::DidAddInputHandler(
  ...
  if (compositor)
    ...
    LOG(WARNING)<<"[DidAddInputHandler] compositor not found routering=["<<routing_id<<"]["<<input_handler<<"]";
    need_restore_routing_id_ = routing_id;
    need_restore_input_handler_ = input_handler;
  }
}

/// M: restore InputHandler when popup window @{
void SynchronousInputEventFilter::RestoreInputHandler(SynchronousCompositorImpl* impl) {

  if (!need_restore_routing_id_)
    return;

  LOG(INFO)<<"[RestoreInputHandler] ["<<need_restore_routing_id_<<"]["
    <<need_restore_input_handler_<<"]";
  SynchronousCompositorImpl* restore_compositor =
      SynchronousCompositorImpl::FromRoutingID(need_restore_routing_id_);
  if (restore_compositor == impl) {
    restore_compositor->SetInputHandler(need_restore_input_handler_);
    need_restore_routing_id_ = 0;
    need_restore_input_handler_ = 0;
  }
}
