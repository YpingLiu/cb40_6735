...
void SynchronousCompositorImpl::ReleaseHwDraw(bool lost_context) {
  DCHECK(CalledOnValidThread());
  DCHECK(output_surface_);
  output_surface_->ReleaseHwDraw();
  /// M: for stream texture leak
  g_factory.Get().CompositorReleasedHardwareDraw(lost_context);
}
...
cc::InterruptableCompletionEvent* SynchronousCompositorImpl::GetInterruptableCompletionEvent() {
  return g_factory.Get().GetInterruptableCompletionEvent();
}

void SynchronousCompositorImpl::DidBeginMainFrameScheduled() {
  g_factory.Get().DidBeginMainFrameScheduled();
}

void SynchronousCompositorImpl::DidBeginMainFrameStarted() {
  g_factory.Get().DidBeginMainFrameStarted();
}

void SynchronousCompositorImpl::DidBeginMainFrameEnded() {
  g_factory.Get().DidBeginMainFrameEnded();
}

void SynchronousCompositorImpl::RunInterruptClosures() {
  g_factory.Get().RunInterruptClosures();
}
/// @}
...
void SynchronousCompositor::SetClientForWebContents(
  ...
  if (client) {
    ...
    /// M: restore input handler was not set when popup window @{
    if (SynchronousCompositorImpl* instance =
      SynchronousCompositorImpl::FromWebContents(contents)) {
      g_factory.Get().synchronous_input_event_filter()->RestoreInputHandler(instance);
    }
    /// @}
  }
  if (SynchronousCompositorImpl* instance =
...
void SynchronousCompositorImpl::SetViewIsPaused(bool paused) {
  if (output_surface_)
    output_surface_->SetViewIsPaused(paused);
}
/// @}
/// M: for tab performance @{
bool SynchronousCompositorImpl::HasContentToDraw() {
  if (output_surface_)
    return output_surface_->HasContentToDraw();
  return true;
}
/// @}
}  // namespace content
