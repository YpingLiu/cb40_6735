...
void SynchronousCompositorOutputSurface::SetViewIsPaused(bool paused) {
  //LOG(INFO)<<"[SetViewIsPaused] ["<<output_surface_client_<<"]["<<paused<<"]";
  if (output_surface_client_)
    output_surface_client_->SetViewIsPaused(paused);
}
/// @}
/// M: for tab performance @{
bool SynchronousCompositorOutputSurface::HasContentToDraw() {
  if (output_surface_client_)
    return output_surface_client_->HasContentToDraw();
  return true;
}
/// @}
}  // namespace content
