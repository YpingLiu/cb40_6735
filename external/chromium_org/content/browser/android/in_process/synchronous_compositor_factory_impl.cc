...
class SynchronousCompositorFactoryImpl::VideoContextProvider
 ...
 public:
  ...
  /// M: To Release StreamTexture @{
  void ReleaseContext() {
    FOR_EACH_OBSERVER(StreamTextureFactoryContextObserver,
                      observer_list_,
                      ReleaseStreamTextureProxy());
  }
  /// @}
...
SynchronousCompositorFactoryImpl::SynchronousCompositorFactoryImpl()
    : record_full_layer_(true),
      ...
      /// M: for interruptCompletionEvent of BeginMainFrame
      begin_main_frame_state_(BeginMainFrame_None) {
...
void SynchronousCompositorFactoryImpl::CompositorReleasedHardwareDraw(bool lost_context) {
  base::AutoLock lock(num_hardware_compositor_lock_);
  DCHECK_GT(num_hardware_compositors_, 0u);
  num_hardware_compositors_--;

  /// M: To release the stream texture @{
  if (lost_context && !num_hardware_compositors_
      && main_thread_proxy_ && video_context_provider_.get()) {
    cc::CompletionEvent completion;
    base::Closure release_closure = base::Bind(
            &SynchronousCompositorFactoryImpl::ReleaseVideoContextOnMainThread,
            base::Unretained(this)
            , &completion);
    if (!CheckIsBeginMainFrameRunningAndInsertClosure(release_closure))
      main_thread_proxy_->PostTask(FROM_HERE, release_closure);
    completion.Wait();
  }
  /// @}
...
/// M: To release the stream texture @{
void SynchronousCompositorFactoryImpl::ReleaseVideoContextOnMainThread(cc::CompletionEvent* completion) {
  if (video_context_provider_)
    video_context_provider_->ReleaseContext();
  completion->Signal();
}
/// @}
...
void SynchronousCompositorFactoryImpl::DidBeginMainFrameScheduled() {
  base::AutoLock lock(begin_main_frame_state_lock_);
  begin_main_frame_state_ = BeginMainFrame_Scheduled;
}

void SynchronousCompositorFactoryImpl::DidBeginMainFrameStarted() {
  base::AutoLock lock(begin_main_frame_state_lock_);
  begin_main_frame_state_ = BeginMainFrame_Started;
}

void SynchronousCompositorFactoryImpl::DidBeginMainFrameEnded() {
  base::AutoLock lock(begin_main_frame_state_lock_);
  begin_main_frame_state_ = BeginMainFrame_None;
}


void SynchronousCompositorFactoryImpl::RunInterruptClosures() {
  std::vector<base::Closure> ready_to_run_callbacks;
  {
    base::AutoLock lock(begin_main_frame_state_lock_);
    ready_to_run_callbacks.swap(interrupt_callbacks_);
  }
  for (size_t i = 0; i < ready_to_run_callbacks.size(); ++i)
    ready_to_run_callbacks[i].Run();
}

bool SynchronousCompositorFactoryImpl::CheckIsBeginMainFrameRunningAndInsertClosure(base::Closure& closure) {
  base::AutoLock lock(begin_main_frame_state_lock_);
  if (begin_main_frame_state_ >= BeginMainFrame_Scheduled) {
