...
ResourceDispatcherHostImpl::ResourceDispatcherHostImpl()
    : save_file_manager_(new SaveFileManager()),
      ...
      /// M: init render_frame_id_ as MSG_ROUTING_NONE
...
DownloadInterruptReason ResourceDispatcherHostImpl::BeginDownload(
  ...
  /// M: If we create request info without valid render frame id, it will be set as MSG_ROUTING_NONE (-2),
  ///    and in AwResourceDispatcherHostDelegate::DownloadStarting, it can't find io_client because
  ///    request_info->GetRenderFrameID() will be -2, and then download operation is aborted.
  ///    So, we set a valid render_frame_id in BeginRequest, and use it to create request info
  ResourceRequestInfoImpl* extra_info =
      CreateRequestInfoWithRenderFrameID(child_id, route_id, true, context, render_frame_id_);
...
void ResourceDispatcherHostImpl::BeginRequest(
  ...
  /// M: keep valid render_frame_id_ for download use
  render_frame_id_ = request_data.render_frame_id;
...
/// M: add for create request info with render frame id @{
ResourceRequestInfoImpl* ResourceDispatcherHostImpl::CreateRequestInfoWithRenderFrameID(
    int child_id,
    int route_id,
    bool download,
    ResourceContext* context,
