...
void BrowserAccessibilityManagerAndroid::NotifyAccessibilityEvent(
  ...
  /// M: ignore AX_EVENT_LOCATION_CHANGED which should only send after AX_EVENT_LAYOUT_COMPLETE.
  /// And we will only send AccessibilityEvent.TYPE_WINDOW_CONTENT_CHANGED here for both two types
  /// We can ignore AX_EVENT_LOCATION_CHANGED here.
  if (event_type == ui::AX_EVENT_LOCATION_CHANGED)
    return;
