...
enum DebugInfoFlag {
  ...
  ShowLayerAnimationBoundsRects = ShowFpsCounter << 1,
  ShowPaintRects = ShowLayerAnimationBoundsRects << 1,
...
scoped_ptr<RenderWidgetCompositor> RenderWidgetCompositor::Create(
  ...
  __system_property_get("debug.chromium.debuginfo", value);
  ...
  if (strlen(value) > 0) {
    ...
    settings.initial_debug_state.show_layer_animation_bounds_rects = ((flags & ShowLayerAnimationBoundsRects) > 0) ? true : false;
