...
static const int kHttpNotModified = 304; /// M: workaround for special video sites.
...
void MediaInfoLoader::didReceiveResponse(
  ...
  /// M: Treating HTTP 304 as ok to workaround on some special servers @{
  if (response.httpStatusCode() == kHttpNotModified) {
    DidBecomeReady(kOk);
    return;
  }
  /// @}
