...
void RendererMediaPlayerManager::EnterFullscreen(int player_id,
  ...
  /// M: Fix a issue that when video is playing in full screen back to back.
  ///    the fullscreen state will not be synchronized between browser/render mediaplayer manager
  ///    After the 2nd video started. @{
  if (!IsInFullscreen(frame))
    pending_fullscreen_frame_ = frame;
  else {
    LOG(INFO) << "EnterFullscreen(): frame already in Full screen, will only update player_id";
  }
  /// @}
