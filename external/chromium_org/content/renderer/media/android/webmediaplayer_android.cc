...
WebMediaPlayerAndroid::~WebMediaPlayerAndroid() {
  ...
  if (stream_id_) {
    ...
    stream_id_ = 0;
    /// M: flush the delete texture command
    gl->Flush();
  }
  ...
  /// M: When back key pressed during video playback, pause won't be invoked.
  ///    Invoke PlayerGone() to inform view so wake lock can be released.
  if (!paused() && player_type_ == MEDIA_PLAYER_TYPE_URL && delegate_)
    delegate_->PlayerGone(this);
...
void WebMediaPlayerAndroid::load(LoadType load_type,
  ...
  if (player_type_ != MEDIA_PLAYER_TYPE_URL) {
    ...
    /// M: don't load info and always report ok for rtsp url, see CR 01234567 @{
    if (!url_.SchemeIs("rtsp")) {
    /// @}
      LOG(INFO) << "info_loader->start()";
      info_loader_.reset(
          new MediaInfoLoader(
              url,
              cors_mode,
  ...
  /// M: don't load info and always report ok for rtsp url @{
  if (url_.SchemeIs("rtsp")) {
    DidLoadMediaInfo(MediaInfoLoader::kOk, url_, frame_->document().firstPartyForCookies(),
        cors_mode != WebMediaPlayer::CORSModeAnonymous);
  }
  /// @}
}
...
bool WebMediaPlayerAndroid::hasVideo() const {
  /// M: always let it has video for live/rtsp streams because we can only get width/height
  ///    at a later stage and for video with posters, the poster will never have a chance to hide
  if (url_.has_path() && (duration() == 0 || url_.SchemeIs("rtsp")))
    return true;
...
double WebMediaPlayerAndroid::currentTime() const {
  ...
  // M: fix: HLS (duration_ = 0)  will always display playtime as 0
  return (duration_.InSecondsF() == 0) ?
      (const_cast<media::TimeDeltaInterpolator*>(
            &interpolator_))->GetInterpolatedTime().InSecondsF() :
        std::min((const_cast<media::TimeDeltaInterpolator*>(
...
void WebMediaPlayerAndroid::ReleaseStreamTextureProxy() {
  DCHECK(main_thread_checker_.CalledOnValidThread());
  if (stream_id_) {
    GLES2Interface* gl = stream_texture_factory_->ContextGL();
    gl->DeleteTextures(1, &texture_id_);
