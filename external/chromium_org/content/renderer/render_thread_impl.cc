...
void RenderThreadImpl::Init() {
  ...
  /// M: Try to Enable multiple raster thread @{
  int raster_thread_num = base::android::SysUtils::GetRasterThreadNum();
  if (raster_thread_num > 1) {
    LOG(INFO)<<"[Init] Enable ["<<raster_thread_num<<"] raster threads";
    cc::RasterWorkerPool::SetNumRasterThreads(raster_thread_num);
  }
  /// @}
  ...
  /// M: set num_raster_threads @{
  ///  load settings from system properties
  char value[PROP_VALUE_MAX] = {'\0'};
  __system_property_get("chromium.num_raster_threads", value);
  int num_raster_threads = atoi(value);
  if (num_raster_threads > 0) {
    LOG(INFO) << "[RenderThreadImpl::Init] num_raster_threads is " << num_raster_threads;
    cc::RasterWorkerPool::SetNumRasterThreads(num_raster_threads);
  }
  if (cc::RasterWorkerPool::GetNumRasterThreads() > 1) {
    LOG(INFO)<<"[RenderThreadImpl::Init] EnableFontHostTLSCache";
    SkFontHost::EnableFontHostTLSCache(true);
  }
  /// @}
