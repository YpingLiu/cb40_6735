...
/// M: On Android platform, use ds->originalRequest().url() as OriginalRequestURL, 
///    instead of checking redirect chain
#if !defined(OS_ANDROID)
...
bool RenderFrameImpl::runModalBeforeUnloadDialog(
  ...
  /// M: add log for ALPS01955742
  LOG(INFO) << "runModalBeforeUnloadDialog#0 message=" << message.utf8();
  bool success = false;
  ...
  /// M: add log for ALPS01955742
  LOG(INFO) << "runModalBeforeUnloadDialog#1 success=" << success << " ignored_result=" << ignored_result;
  return success;
}
