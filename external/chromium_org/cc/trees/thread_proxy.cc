...
/// M: change CompletionEvent to InterruptableCompletionEvent
#include "content/browser/android/in_process/synchronous_compositor_impl.h"

#define ENABLE_BEGIN_MAIN_FRAME_LOG 0
...
/// M: Run interrupt closures for BeginMainFrame @{
struct BeginMainFrameScope {
  BeginMainFrameScope() {
    content::SynchronousCompositorImpl::DidBeginMainFrameStarted();
  }
  ~BeginMainFrameScope() {
    content::SynchronousCompositorImpl::DidBeginMainFrameEnded();
...
void ThreadProxy::ScheduledActionSendBeginMainFrame() {
  ...
  /// M: Run interrupt closures for BeginMainFrame
  content::SynchronousCompositorImpl::DidBeginMainFrameScheduled();
...
void ThreadProxy::BeginMainFrame(
  ...
  /// M: Run interrupt closures for BeginMainFrame
  BeginMainFrameScope begin_main_frame_scope;
...
#if ENABLE_BEGIN_MAIN_FRAME_LOG
  LOG(INFO)<<"[BeginMainFrame] +++++++++++++++++++++++++++++";
#endif
  ...
  /// M: For Optimization
  blocked_main().layer_updated = updated;
...
#if ENABLE_BEGIN_MAIN_FRAME_LOG
    LOG(INFO)<<"[BeginMainFrame] ============ start to commit";
#endif
...
#define USE_INTERRUPTABLE_COMPLETION_EVENT 1
#if USE_INTERRUPTABLE_COMPLETION_EVENT
    CompletionEvent* completion = content::SynchronousCompositorImpl::GetInterruptableCompletionEvent();
#else
    CompletionEvent completion;
#endif
    Proxy::ImplThreadTaskRunner()->PostTask(
...
void ThreadProxy::ScheduledActionCommit() {
  ...
  /// M: For Optimization @{
#if 0
  LOG(INFO)<<"[ScheduledActionCommit] updated=["<<blocked_main().layer_updated<<"] textureLayer=["
    <<layer_tree_host()->GetTextureLayerNum()
    <<"] pictureLayer=["<<layer_tree_host()->GetPictureLayerNum()<<"] updated=["
    <<layer_tree_host()->IsPictureLayerUpdated()<<"]";
#endif
  //// check if animation :
  ///   1. texture layer updated only
  ///   2. no layer (include picture layer) was updated
  if (blocked_main().layer_updated
      && layer_tree_host()->GetTextureLayerNum() > 0
      && !layer_tree_host()->IsPictureLayerUpdated())
    impl().scheduler->SetTextureLayerAnimate(true);
  else
    impl().scheduler->SetTextureLayerAnimate(false);
  /// @}
...
void ThreadProxy::RenewTreePriority() {
  ...
  /// M: To fix timing issue
  ///  the web view destroyed after smoothness_priority_expiration_notifier was scheduled.
  if (!impl().layer_tree_host_impl.get()) {
    LOG(WARNING)<<"[RenewTreePriority] ["<<this<<"] layerTreeHostImpl was destroyed, skip";
    return;
  }
  /// @}
...
/// M: For SendBeginMainFrame early @{
bool ThreadProxy::HasPendingTreeToActivate() {
  return impl().scheduler->HasPendingTreeToActivate();
}

void ThreadProxy::SetViewIsPaused(bool paused) {
  impl().scheduler->SetViewIsPaused(paused);
}
/// @}

/// M: For sina flash @{
bool ThreadProxy::IsPendingTreeInManage() {
  return impl().scheduler->IsPendingTreeInManage();
}
/// @}
}  // namespace cc
