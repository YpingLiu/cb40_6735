...
/// M: For Optimization @{
void LayerTreeHost::RegisterPictureLayer(PictureLayer* layer) {
  DCHECK(std::find(picture_layers_.begin(), picture_layers_.end(), layer) ==
         picture_layers_.end());
  picture_layers_.push_back(layer);
}

void LayerTreeHost::UnregisterPictureLayer(PictureLayer* layer) {
  std::vector<PictureLayer*>::iterator it =
      std::find(picture_layers_.begin(), picture_layers_.end(), layer);
  DCHECK(it != picture_layers_.end());
  picture_layers_.erase(it);
}

void LayerTreeHost::RegisterTextureLayer(TextureLayer* layer) {
  DCHECK(std::find(texture_layers_.begin(), texture_layers_.end(), layer) ==
         texture_layers_.end());
  texture_layers_.push_back(layer);
}

void LayerTreeHost::UnregisterTextureLayer(TextureLayer* layer) {
  std::vector<TextureLayer*>::iterator it =
      std::find(texture_layers_.begin(), texture_layers_.end(), layer);
  DCHECK(it != texture_layers_.end());
