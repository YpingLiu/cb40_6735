...
void LayerTreeHostImpl::ManageTiles() {
  ...
  tile_priorities_dirty_ = false;
  /// M: to activatePendingTree when unchnaged.
  tile_manager_->ManageTiles(global_tile_state_, client_->HasPendingTreeToActivate());
...
DrawResult LayerTreeHostImpl::PrepareToDraw(FrameData* frame) {
  ...
  if (settings_.impl_side_painting) {
    RasterWorkerCoordinator::GetInstance()->ResetTileStats();
  }
  /// @}
  ...
  /// For scrolling flash on sina @{
  if (IsActivelyScrolling() && pending_tree() && client_->IsPendingTreeInManage()) {
      gfx::Vector2dF diff = pending_tree()->TotalScrollOffset();
      diff -= active_tree()->TotalScrollOffset();

      bool need_update = false;
      if (diff.LengthSquared() > 160000 && client_->IsPendingTreeInManage()) {
        need_update = true;
      }
      if (need_update) {
        TRACE_EVENT0("cc", "LayerTreeHostImpl::PrepareToDraw::SyncScrollOffsetFromActive");
        pending_tree()->root_layer()->SyncScrollOffsetFromActive();
  ...
  /// M: for tab performance @{
  if (settings_.impl_side_painting) {
    RasterWorkerCoordinator::GetInstance()->CommitTileStats();
  }
  /// @}
...
void LayerTreeHostImpl::SetViewIsPaused(bool paused) {
  proxy()->SetViewIsPaused(paused);
}
/// @}
/// M: for tab performance @{
bool LayerTreeHostImpl::HasContentToDraw() {
  LOG(INFO)<<"[HasContentToDraw] layers=["<<active_tree()->NumLayers()<<"]";
  return (active_tree()->NumLayers() > 0);
}
/// @}
}  // namespace cc
