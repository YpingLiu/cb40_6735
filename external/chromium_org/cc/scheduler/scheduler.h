...
class CC_EXPORT Scheduler {
 public:
  ...
  /// M: For SendBengMainFrame early @{
  bool HasPendingTreeToActivate() const;

  void SetViewIsPaused(bool paused) {
    state_machine_.SetViewIsPaused(paused);
  }

  void SetTextureLayerAnimate(bool animate) {
    state_machine_.SetTextureLayerAnimate(animate);
  }
  /// @}

  /// M: for sina scrolling flash @{
  bool IsPendingTreeInManage() const {
    return state_machine_.commit_state() == SchedulerStateMachine::COMMIT_STATE_WAITING_FOR_ACTIVATION;
  }
  /// @}
