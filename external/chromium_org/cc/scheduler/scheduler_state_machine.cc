...
/// M: for performance optimization
...
SchedulerStateMachine::SchedulerStateMachine(const SchedulerSettings& settings)
    : settings_(settings),
      ...
      /// M: For Optimization
      , webview_is_paused_(false)
      , texture_layer_animate_(false) {
}
...
bool SchedulerStateMachine::ShouldSendBeginMainFrame() const {
  ...
  /// M: For Optimization @{
  ///  Here we let scheduler to send BeginMainFrame early
  ///   NOTE the upper comment, this will block the input event
  ///   Send BeginMainFrame early when:
  ///    1. TextureLayer update only
  ///    2. webview is not paused.
  ///    3. ActivatePendingTree was done
  bool send_main_frame_early = false;
  if (texture_layer_animate_
      && !webview_is_paused_
      && !has_pending_tree_
      && active_tree_needs_first_draw_
      && !base::android::SysUtils::HasTouchEventDuringVsync())
    send_main_frame_early = true;
  /// @}
#if 0
  LOG(INFO)<<"[ShouldSendBeginMainFrame] Commit=["<<CommitStateToString(commit_state_)<<"] frameState=["
    <<BeginImplFrameStateToString(begin_impl_frame_state_)<<"] paused=["<<webview_is_paused_
    <<"]["<<texture_layer_animate_<<"] ["<<has_pending_tree_<<"]["<<active_tree_needs_first_draw_
    <<"] ==> early=["<<send_main_frame_early<<"] checker=["
    <<base::android::SysUtils::HasTouchEventDuringVsync()<<"]";
#endif

  if (!send_main_frame_early &&
      begin_impl_frame_state_ == BEGIN_IMPL_FRAME_STATE_IDLE &&
      BeginFrameNeeded()) {
    return false;
  }
  ...
  if (send_main_frame_early
      && begin_impl_frame_state_ == BEGIN_IMPL_FRAME_STATE_IDLE
      && BeginFrameNeeded())
    LOG(INFO)<<"[ShouldSendBeginMainFrame] SendBeginMainFrame EARLY !!";
#endif
