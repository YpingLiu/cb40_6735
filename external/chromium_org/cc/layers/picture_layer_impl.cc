...
void PictureLayerImpl::AppendQuads(
  ...
  for (PictureLayerTilingSet::CoverageIterator iter(tilings_.get(),
    ...
    total_tile_count++;
    int raster_mode = 0;
    if (*iter && iter->IsReadyToDrawAndGetMode(&raster_mode)) {
    /// @}
      const ManagedTileState::TileVersion& tile_version =
          iter->GetTileVersionForDrawing();
      /// M: for tab performance @{
      if (raster_mode == HIGH_QUALITY_RASTER_MODE)
        high_res_tile_count++;
      else
        low_res_tile_count++;
      /// @}
  ...
  /// M: for tab performance @{
  RasterWorkerCoordinator::GetInstance()->UpdateTileStats(high_res_tile_count, low_res_tile_count
                                                        , missing_tile_count, total_tile_count);
  /// @}
...
void PictureLayerImpl::UpdateTiles(const Occlusion& occlusion_in_content_space,
  ...
  /// M: for tab performance @{
  if (IsActive()) {
    bool is_attaching_window = RasterWorkerCoordinator::GetInstance()->IsTriggered();
    if (skip_low_res_ && !is_attaching_window) {
      LOG(INFO)<<"[UpdateTiles] ["<<this<<"] re-AddTilingsForRasterScale";
      AddTilingsForRasterScale();
      skip_low_res_ = false;
    }
  }
...
gfx::Size PictureLayerImpl::CalculateTileSize(
  ...
  if (pile_->is_mask()) {
    ...
    /// M: for mobile01-live abnormal screen @{
    ///  the page was create a large mask layer
    ///  By this function, if mask layer doesn't execeed max texture size.
    ///  the mask layer will create a large tile same as content bounds
    ///  it need check with tile manage if it can create such large tile
    if (!layer_tree_impl()->tile_manager()->CanAllocateLargeSize(content_bounds))
      return gfx::Size();
    /// @}
...
void PictureLayerImpl::AddTilingsForRasterScale() {
  ...
  /// M: for tab resume performance @{
  bool isAttachingWindow = RasterWorkerCoordinator::GetInstance()->IsTriggered() && IsActive();
  bool need_add_low_res = layer_tree_impl()->create_low_res_tiling() && !is_pinching &&
