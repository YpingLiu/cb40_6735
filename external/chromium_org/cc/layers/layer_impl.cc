...
/// M: for ALPS01757886, sina.com flash when scrolling @{
void LayerImpl::SyncScrollOffsetFromActive() {
  if (layer_tree_impl()->IsActiveTree())
    return;
  ...
  for (OwnedLayerImplList::iterator it = children_.begin();
       it != children_.end();
       ++it) {
    (*it)->SyncScrollOffsetFromActive();
  }
  LayerImpl* active_twin = layer_tree_impl()->FindActiveTreeLayerById(id());
  if (active_twin) {
    SetScrollDelta(active_twin->ScrollDelta() - active_twin->sent_scroll_delta());
  }
}
/// @}
}  // namespace cc
