...
void TileManager::ManageTiles(const GlobalStateThatImpactsTilePriority& state, bool has_pending_to_activate) {
  ...
  /// M: For Optimization @{
  if (tiles_that_need_to_be_rasterized.size() == 0) {
    if (!has_pending_to_activate) {
      //LOG(INFO)<<"[ManageTiles] =FAST PATH= no need to schedule tasks";
      return;
    }

    bool is_ready = IsReadyToActivate();
    if (is_ready) {
...
bool TileManager::CanAllocateLargeSize(const gfx::Size& size) {
  size_t bytes_consumed = BytesConsumedBySize(size);
  if (bytes_consumed > (global_state_.hard_memory_limit_in_bytes >> 2))
    return false;
  return true;
}
/// @}
