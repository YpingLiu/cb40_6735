...
class CC_EXPORT TileManager : public RasterizerClient,
 ...
 public:
  ...
  void ManageTiles(const GlobalStateThatImpactsTilePriority& state, bool has_pending_to_activate = true);
  ...
  bool CanAllocateLargeSize(const gfx::Size& size);
 ...
 private:
  ...
  /// M: for mobile01-live abnormal screen @{
  inline size_t BytesConsumedBySize(const gfx::Size& size) const {
    return Resource::MemorySizeBytes(size,
                                     resource_pool_->resource_format());
  }
  /// @}
