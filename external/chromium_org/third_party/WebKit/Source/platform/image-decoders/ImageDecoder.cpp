...
PassOwnPtr<ImageDecoder> ImageDecoder::create(const SharedBuffer& data, ImageSource::AlphaOption alphaOption, ImageSource::GammaAndColorProfileOption gammaAndColorProfileOption)
{
    ...
    if (matchesWBMPSignature(contents)) {
        WBMPImageDecoder::WBMP_Header header;
        // Due to WBMP poor signature identification, we check the data size in WBMP header's init()
        if (header.init((SharedBuffer*)&data))
            return adoptPtr(new WBMPImageDecoder(alphaOption, gammaAndColorProfileOption, maxDecodedBytes));
    }
    /// @}
