...
FrameView::FrameView(LocalFrame* frame)
    ...
    , m_textWrapWidth(DEFAULT_TEXT_WRAP_WIDTH)
    , m_textWrapScale(DEFAULT_TEXT_WRAP_SCALE)
    /// @}
{
    ASSERT(m_frame);
    init();
...
void FrameView::init()
{
    ...
    if (page) {
        m_textWrapWidth = page->textWrapWidth();
        m_textWrapScale = page->textWrapScale();
    }
    /// @}
...
/// M: Support text-reflow in chromium @{
void FrameView::setTextWrapWidth(int width)
{
    m_textWrapWidth = width;

    Page* page = m_frame->page();
    if (page)
        page->setTextWrapWidth(width);
}

void FrameView::setTextWrapScale(float scale)
{
    m_textWrapScale = scale;

    Page* page = m_frame->page();
    if (page)
        page->setTextWrapScale(scale);
}
/// @}
