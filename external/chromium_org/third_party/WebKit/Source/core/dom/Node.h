...
#if ENABLE(WML)
    bool hasTagName(const WMLQualifiedName&) const;
#endif
...
#if ENABLE(WML)
    bool isWMLElement() const { return getFlag(IsWMLFlag); }
#endif
...
#if ENABLE(WML)
    bool isStyledElement() const { return isHTMLElement() || isSVGElement() || isWMLElement(); }
#else
...
#if ENABLE(WML)
        CreateWMLElement = CreateElement | IsWMLFlag,
#endif
    };
