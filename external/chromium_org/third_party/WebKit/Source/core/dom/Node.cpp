...
bool Node::hasEditableStyle(EditableLevel editableLevel, UserSelectAllTreatment treatment) const
{
    ...
    for (const Node* node = this; node; node = node->parentNode()) {
        if ((node->isHTMLElement()
                ...
                || node->isWMLElement()
#endif
                || node->isDocumentNode()) && node->renderer()) {
...
{
    /// M: fix some websites use "TransitionEnd" as eventType @{
    if (eventType.lower() == EventTypeNames::transitionend)
        return tryAddEventListener(this, EventTypeNames::transitionend, listener, useCapture);
    /// @}
