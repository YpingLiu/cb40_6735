...
#if ENABLE(WML)
    if (isWMLInputElement(*m_innerNonSharedNode)) {
        WMLImageElement& image = toWMLImageElement(*m_innerNonSharedNode);
        return image.fastGetAttribute(altAttr);
    }
#endif
...
#if ENABLE(WML)
        || isWMLImageElement(*m_innerNonSharedNode)
#endif
       ) {
...
#if ENABLE(WML)
    if (isWMLAElement(m_innerURLElement) || isWMLAnchorElement(m_innerURLElement))
