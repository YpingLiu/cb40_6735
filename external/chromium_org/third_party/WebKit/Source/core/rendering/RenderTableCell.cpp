...
void RenderTableCell::setCellLogicalWidth(int tableLayoutLogicalWidth, SubtreeLayoutScope& layouter)
{
    ...
    if (view()->frameView()) {
        ...
        if (settings->textFitColumnToScreenEnabled())
            visibleWidth = view()->frameView()->textWrapWidth();
    }

    if (tableLayoutLogicalWidth == logicalWidth() && (visibleWidth <= 0 || visibleWidth == getVisibleWidth()))
    //if (tableLayoutLogicalWidth == logicalWidth())
    /// @}
        return;

    /// M: Support text-reflow in chromium @{
    if (view()->frameView() && visibleWidth > 0 && visibleWidth != getVisibleWidth())
