    ...
    const BidiStatus& cleanLineBidiStatus)
{
    ...
    while (!endOfLine.atEnd()) {
        ...
        if (layoutState.lineInfo().isEmpty()) {
            ...
            /// M: for Vellamo Performance @{
            if (endOfLine.object() && endOfLine.object()->isText()) {
                RenderObject* nextObj = endOfLine.object()?endOfLine.object()->nextSibling():0;
                RenderStyle* nextObjStyle = nextObj?nextObj->style():0;
                if (!nextObj || nextObj->isText())
                    isText = true;
            }
            /// @}
            bidiRuns.deleteRuns();
...
void RenderBlockFlow::layoutInlineChildren(bool relayoutChildren, LayoutUnit& paintInvalidationLogicalTop, LayoutUnit& paintInvalidationLogicalBottom, LayoutUnit afterEdge)
{
    ...
    if (firstChild()) {
        ...
        bool doTextWrap = settings && settings->textFitColumnToScreenEnabled();
        if (doTextWrap) {
            ...
            doTextWrap = autowrap && !positioned &&
                    ...
                    (((dir == LTR && cssfloat != RightFloat) ||
                    (dir == RTL && cssfloat != NoFloat)) &&
                    ((ta == RenderStyle::initialTextAlign()) || (ta == JUSTIFY) ||
                ...
                if (doTextWrap && !hasTextToWrap && o->isText()) {
                    ...
                    if (node && node->isTextNode() &&
                        ...
                        if (length > 25 || (length > 3 &&
                                (!node->parentOrShadowHostNode()->hasTagName(HTMLNames::aTag) &&
                                !node->parentOrShadowHostNode()->hasTagName(HTMLNames::liTag))))
                            hasTextToWrap = true;
                    }
                }
                /// @}
        ...
        if (doTextWrap && hasTextToWrap) {
            ...
            if (!isConstrained) {
                ...
                if (textWrapWidth > 0 && width() > (textWrapWidth + padding)) {
                    ...
                    setWidth(std::min(width().ceil(), maxWidth));
                    m_minPreferredLogicalWidth = std::min(m_minPreferredLogicalWidth.ceil(), maxWidth);
                    m_maxPreferredLogicalWidth = std::min(m_maxPreferredLogicalWidth.ceil(), maxWidth);
