...
void RenderMarquee::timerFired()
{
    ...
    /// M: for vellamo ocean scroller @{
    ///  if leaving from the visible region, stop the marquee element update.
    ///  resume it until back to visible region
    if (view() && view()->frameView()) {
        LayoutRect rect = view()->frameView()->viewportConstrainedVisibleContentRect();
        IntRect absRect = absoluteBoundingBoxRect();
        absRect.inflate(5);
        IntRect visibleRect(rect.pixelSnappedLocation(), rect.pixelSnappedSize());
        if (!visibleRect.intersects(absRect))
            return;
    }
    /// @}
