...
bool RenderBox::paintInvalidationLayerRectsForImage(WrappedImagePtr image, const FillLayer& layers, bool drawingBackground)
{
    ...
    for (const FillLayer* curLayer = &layers; curLayer; curLayer = curLayer->next()) {
        if (curLayer->image() && image == curLayer->image()->data() && curLayer->image()->canRender(*this, style()->effectiveZoom())) {
                for (Vector<RenderObject*>::const_iterator it = layerRenderers.begin(); it != layerRenderers.end(); ++it) {
			(*it)->setShouldDoFullPaintInvalidation(true);
   	         }
		return true;
        }
    }
...
/// M: Support text-reflow in chromium @{
void RenderBox::setVisibleWidth(int newWidth) const
{
    const Settings* settings = document().settings();
    ASSERT(settings);
    if (!settings->textFitColumnToScreenEnabled() || m_visibleWidth == newWidth)
        return;
    m_isVisibleWidthChangedBeforeLayout = true;
    m_visibleWidth = newWidth;
}

bool RenderBox::checkAndSetRelayoutChildren(bool* relayoutChildren) const
{
