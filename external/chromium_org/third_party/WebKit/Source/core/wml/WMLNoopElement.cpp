...
WMLNoopElement::WMLNoopElement(Document& document)
    : WMLElement(noopTag, document)
{
}

PassRefPtr<WMLNoopElement> WMLNoopElement::create(Document& document)
{
    return adoptRef(new WMLNoopElement(document));
}
...
Node::InsertionNotificationRequest WMLNoopElement::insertedInto(ContainerNode* insertionPoint)
{
    ...
    if (parent->hasTagName(doTag)) {
        ...
        doElement->lazyReattachIfAttached();
