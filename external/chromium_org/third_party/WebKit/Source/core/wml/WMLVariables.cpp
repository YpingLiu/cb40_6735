...
template<typename CharType> static bool isValidFirstVariableNameCharacter(const CharType& character)
{
    return WTF::isASCIIAlpha(character)
           || character == '_';
}

template<typename CharType> static bool isValidVariableNameCharacter(const CharType& character)
{
    return WTF::isASCIIAlpha(character)
...
bool isValidVariableName(const String& name)
{
    ...
    if (!isValidFirstVariableNameCharacter(name[0]))
        return false;

    unsigned length = name.length();
    for (unsigned i = 1; i < length; ++i) {
        if (!isValidVariableNameCharacter(name[i]))
            return false;
    }
