...
WMLSelectElement::WMLSelectElement(Document& document)
    : WMLFormControlElement(selectTag, document)
    , m_initialized(false)
{
}

PassRefPtr<WMLSelectElement> WMLSelectElement::create(Document& document)
{
    return adoptRef(new WMLSelectElement(document));
}
...
bool WMLSelectElement::isKeyboardFocusable(KeyboardEvent* event) const
{
    ...
    return WMLFormControlElement::isKeyboardFocusable();
}
...
void WMLSelectElement::didRecalcStyle(StyleRecalcChange change)
{
    WMLFormControlElement::didRecalcStyle(change);
}

void WMLSelectElement::dispatchFocusEvent(Element* oldFocusedElement, FocusType type)
{
    SelectElement::dispatchFocusEvent(m_data, this);
    WMLFormControlElement::dispatchFocusEvent(0, type);
}

void WMLSelectElement::dispatchBlurEvent(Element* newFocusedElement)
{
    SelectElement::dispatchBlurEvent(m_data, this);
    WMLFormControlElement::dispatchBlurEvent(0);
}
...
void WMLSelectElement::childrenChanged(const ChildrenChange& change)
{
    SelectElement::setRecalcListItems(m_data, this);
    Element::childrenChanged(change);
}

void WMLSelectElement::parseAttribute(const QualifiedName& name, const AtomicString& value)
{
    if (name == HTMLNames::multipleAttr)
        SelectElement::parseMultipleAttribute(m_data, this, name, value);
    else
        WMLFormControlElement::parseAttribute(name, value);
}

RenderObject* WMLSelectElement::createRenderer(RenderStyle*)
{
    return new RenderWMLMenuList(this);
}
...
void WMLSelectElement::accessKeyAction(bool sendMouseEvents)
{
    focus();
...
void WMLSelectElement::updateVariables()
{
    ...
    for (unsigned i = 0; i < items.size(); ++i) {
        ...
        nameString.append(optionElement->value());
        inameString.append(String::number(optionIndex));
    }
...
Vector<unsigned> WMLSelectElement::parseIndexValueString(const String& indexValue) const
{
    ...
    for (Vector<String>::const_iterator it = indexStrings.begin(); it != end; ++it) {
        ...
        if (indices.find(parsedValue) == kNotFound)
...
String WMLSelectElement::optionIndicesToValueString() const
{
    ...
    for (Vector<unsigned>::const_iterator it = m_defaultOptionIndices.begin(); it != end; ++it) {
        ...
        ASSERT(listIndex >= 0);
        ASSERT(listIndex < (int)items.size());
        /// M: add boundary check to prevent NE @{
        ...
        if (OptionElement* optionElement = toOptionElement(items[listIndex])) {
            ...
            valueString.append(optionElement->value());
        }
    }
