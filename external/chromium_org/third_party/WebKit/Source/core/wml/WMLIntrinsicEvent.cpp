...
#if ENABLE(WML)
#include "core/wml/WMLIntrinsicEvent.h"
...
{
    return static_pointer_cast<WMLTaskElement>(WMLElementFactory::createWMLElement(goTag.localName(), document, false));
}

WMLIntrinsicEvent::WMLIntrinsicEvent(Document& document, const String& targetURL)
    : m_taskElement(createTaskElement(document))
{
    m_taskElement->setAttribute(HTMLNames::hrefAttr, AtomicString(targetURL));
}
