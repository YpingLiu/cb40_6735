...
void reportWMLError(Document& doc, WMLErrorCode error)
{
    ...
    if (parser && error != WMLErrorDeckNotAccessible) {
        ...
        parser->handleError(XMLErrors::ErrorTypeFatal, errorMessage.latin1().data(), parser->textPosition());
    } else {
        if (LocalDOMWindow* window = doc.domWindow())
            window->frameConsole()->addMessage(ConsoleMessage::create(WMLMessageSource, ErrorMessageLevel, errorMessage));
    }
}
