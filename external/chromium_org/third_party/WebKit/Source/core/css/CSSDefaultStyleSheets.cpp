...
void CSSDefaultStyleSheets::ensureDefaultStyleSheetsForElement(Element* element, bool& changedDefaultStyle)
{
    ...
    if (element->isWMLElement() && !m_wmlStyleSheet) {
        m_wmlStyleSheet = parseUASheet(wmlCss, sizeof(wmlCss));
        m_defaultStyle->addRulesFromSheet(wmlStyleSheet(), screenEval());
        m_defaultPrintStyle->addRulesFromSheet(wmlStyleSheet(), printEval());
        changedDefaultStyle = true;
    }
#endif
