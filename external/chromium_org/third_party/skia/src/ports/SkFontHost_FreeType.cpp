...
/// M: for TLS SkFontHost_FreeType @{
#define SKFreeType_TLS_ENABLE 1
...
/// M: for TLS SkFontHost_FreeType @{
struct LCDSupportInfo {
    LCDSupportInfo()
      : fLCDSupportValid(false)
      , fLCDSupport(false)
      , fLCDExtra(0) {}
    bool fLCDSupportValid;
    bool fLCDSupport;
    int fLCDExtra;
};

static bool InitFreetypeInternal(FT_Library* library, LCDSupportInfo* lcdSupport) {
    ...
    err = FT_Library_SetLcdFilter(*library, FT_LCD_FILTER_DEFAULT);
    if (0 == err) {
        lcdSupport->fLCDSupport = true;
        lcdSupport->fLCDExtra = 2; //Using a filter adds one full pixel to each side.
        ...
        err = FT_Library_SetLcdFilterWeights(*library, gGaussianLikeHeavyWeights);
        ...
        if (self) {
            ...
            *reinterpret_cast<void**>(&setLcdFilterWeights) = dlsym(self, "FT_Library_SetLcdFilterWeights");
            ...
            if (setLcdFilterWeights) {
                err = setLcdFilterWeights(*library, gGaussianLikeHeavyWeights);
            }
        }
#endif
#endif
...
#else
    lcdSupport->fLCDSupport = false;
#endif
    lcdSupport->fLCDSupportValid = true;
    return true;
}

// Caller must lock gFTMutex before calling this function.
static bool InitFreetype() {
    LCDSupportInfo lcdSupport;
    bool ret = InitFreetypeInternal(&gFTLibrary, &lcdSupport);
    gLCDSupportValid = lcdSupport.fLCDSupportValid;
    gLCDSupport = lcdSupport.fLCDSupport;
    gLCDExtra = lcdSupport.fLCDExtra;
    return ret;
}
/// @}
...
/// ==============================================================================
/// M: for TLS SkFontHost_FreeType @{
#if SKFreeType_TLS_ENABLE
class SkFreeTypeTLSRecord {
public:
    SkFreeTypeTLSRecord(bool useMutex = false)
        : mFTLibrary(NULL)
        , mFTCount(0)
        , mFaceRecHead(NULL)
        , mFTMutex(useMutex ? SkNEW(SkMutex) : NULL) {
        bool ret = initFreetype();
    }
    ~SkFreeTypeTLSRecord() {
        /// TODO: should we unref mFaceRecHead ?
        doneFreetype();
        if (mFTMutex)
            SkDELETE(mFTMutex);
    }
    ...
    bool isLCDSupported() {
        if (!mLCDSupportInfo.fLCDSupportValid) {
            // This will determine LCD support as a side effect.
            initFreetype();
        }
        SkASSERT(mLCDSupportInfo.fLCDSupportValid);
        return mLCDSupportInfo.fLCDSupport;
    }

    SkFaceRec** getFaceHead() {
        return &mFaceRecHead;
    }

    SkFaceRec* ref_ft_face(const SkTypeface* typeface) {
        const SkFontID fontID = typeface->uniqueID();
        SkFaceRec* rec = mFaceRecHead;
        while (rec) {
        ...
        FT_Error err = FT_Open_Face(mFTLibrary, &args, face_index, &rec->fFace);
        if (err) {    // bad filename, try the default font
            ...
            SkASSERT(rec->fFace);
            //SkDebugf(stderr, "Opened font '%s'\n", filename.c_str());
            rec->fNext = mFaceRecHead;
            mFaceRecHead = rec;
            return rec;
        }
    }
...
private:
    bool initFreetype() {
        if (mFTLibrary)
            return true;
        return InitFreetypeInternal(&mFTLibrary, &mLCDSupportInfo);
    }
    void doneFreetype() {
    ...
    int mFTCount;
    SkFaceRec* mFaceRecHead;
    LCDSupportInfo mLCDSupportInfo;
    SkMutex* mFTMutex;
};

/// Global FreeType Record
static SkFreeTypeTLSRecord gSharedFTRecord(true);


class SkFreeTypeLibTLS {
public:
    // can return NULL
    static SkFreeTypeTLSRecord* FindTLS() {
        SkFreeTypeTLSRecord* ptr = (SkFreeTypeTLSRecord*)SkTLS::Find(CreateTLS);
        return ptr;
    }
    static SkFreeTypeTLSRecord& GetTLS() {
        SkFreeTypeTLSRecord* ptr = (SkFreeTypeTLSRecord*)SkTLS::Get(CreateTLS, DeleteTLS);
        return *ptr;
    }
    static void DeleteTLS() {
        SkTLS::Delete(CreateTLS);
    }
private:
    static void* CreateTLS() {
        return SkNEW(SkFreeTypeTLSRecord);
    }
    static void DeleteTLS(void* ptr) {
        SkDELETE((SkFreeTypeTLSRecord*)ptr);
    }
};

// Returns the TLS globals
static SkFreeTypeTLSRecord& getGlobals() {
    if (!SkFontHost::IsEnableFontHostTLSCache())
        return gSharedFTRecord;
    SkFreeTypeTLSRecord* tls = SkFreeTypeLibTLS::FindTLS();
    if (!tls)
        tls = &SkFreeTypeLibTLS::GetTLS();
    return *tls;
}
...
public:
...
#if SKFreeType_TLS_ENABLE
        SkFreeTypeTLSRecord& globals = getGlobals();
        if (globals.getFTMutex())
            globals.getFTMutex()->acquire();
        globals.incCount();
        fRec = globals.ref_ft_face(tf);
#else
        gFTMutex.acquire();
        if (1 == ++gFTCount) {
            if (!InitFreetype()) {
    ...
    ~AutoFTAccess() {
        ...
        SkFreeTypeTLSRecord& globals = getGlobals();
        if (fFace)
            globals.unref_ft_face(fFace);
        globals.decCount();
        if (globals.getFTMutex())
            globals.getFTMutex()->release();
#else
        if (fFace) {
            unref_ft_face(fFace);
        }
...
/// M: for TLS SkFontHost_FreeType @{
#if SKFreeType_TLS_ENABLE
    bool lcd_supported = getGlobals().isLCDSupported();
#else
    bool lcd_supported = is_lcd_supported();
#endif
/// @}
    if (!lcd_supported && isLCD(*rec)) {
...
#if SKFreeType_TLS_ENABLE
    SkFreeTypeTLSRecord& globals = getGlobals();
    SkAutoMutexAcquire  ac(globals.getFTMutex());
    globals.incCount();
#else
...
/// M: for TLS SkFontHost_FreeType @{
#if SKFreeType_TLS_ENABLE
    fFaceRec = globals.ref_ft_face(typeface);
#else
...
/// M: for TLS SkFontHost_FreeType @{
#if SKFreeType_TLS_ENABLE
        SkAutoMutexAcquire  ac(getGlobals().getFTMutex());
#else
        SkAutoMutexAcquire  ac(gFTMutex);
#endif
/// @}
