...
static const MimeInfo primary_mappings[] = {
  ...
  { "image/jpeg", "jpeg,jpg" },
  { "image/webp", "webp" },
  { "image/vnd.wap.wbmp", "wbmp" }, /// M: WBMP support
  { "image/png", "png" },
  { "video/mp4", "mp4,m4v" },
  { "audio/x-m4a", "m4a" },
  { "audio/mp3", "mp3" },
...
static const char* const supported_image_types[] = {
  ...
  "image/x-xbitmap",  // xbm
  "image/x-png",
  "image/vnd.wap.wbmp" /// M: WBMP support
};
...
/// M: add hardware platform support media types/codecs
/// @{
static const char* const platform_media_types[] = {
  // 3GP
  "video/3gpp",
  "audio/3gpp",
  "video/3gpp2",
...
/// M: platform media type @{
  for (size_t i = 0; i < arraysize(platform_media_types); ++i) {
    LOG(INFO) << "insert media type: " << platform_media_types[i];
    media_map_.insert(platform_media_types[i]);
  }
/// @}
...
/// M: platform media types/codecs @{
  for (size_t i = 0; i < arraysize(platform_media_types); ++i)
    media_map_.erase(platform_media_types[i]);
/// @}
