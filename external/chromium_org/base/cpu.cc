 ...
 public:
  LazyCpuInfoValue() : has_broken_neon_(false) {
    ...
    while (std::getline(iss, line)) {
      /// M: brand name may be process prefix @{
      if (brand_.empty()) {
        if (line.compare(0, strlen(kProcessorPrefix), kProcessorPrefix) == 0)
          brand_.assign(line.substr(strlen(kProcessorPrefix)));
        else if (line.compare(0, strlen(kModelNamePrefix), kModelNamePrefix) == 0)
    ...
    /// M: @{
    std::string cpu_freq;
    ReadFileToString(FilePath("/sys/devices/system/cpu/cpu0/cpufreq/cpuinfo_max_freq"), &cpu_freq);
    cpu_freq.erase(std::find_if(cpu_freq.rbegin(), cpu_freq.rend(), std::not1(std::ptr_fun<int, int>(std::isspace))).base(), cpu_freq.end());
    LOG(INFO)<<"[LazyCpuInfoValue] cpufreq=["<<cpu_freq<<"]";
    cpu_max_freq_ = atoi(cpu_freq.c_str());
    /// @}
  }

  const std::string& brand() const { return brand_; }
  bool has_broken_neon() const { return has_broken_neon_; }
  unsigned cpu_max_freq() const { return cpu_max_freq_; }

 private:
  std::string brand_;
  bool has_broken_neon_;
  unsigned cpu_max_freq_;
...
void CPU::Initialize() {
  ...
  /// M: get cpu amount
  cpu_amount_ = sysconf(_SC_NPROCESSORS_CONF);
  cpu_max_freq_ = g_lazy_cpuinfo.Get().cpu_max_freq();
#endif
}
