...
static char gProcessName[MAX_PROCESS_NAME] = {'\0'};
static bool gIsBrowserProcess = false;
...
bool SysInfo::IsBrowserProcess() {
  ...
  if (!gIsBrowserProcess) {
    if (GetCurrentProcessName() == "com.android.browser")
      gIsBrowserProcess = true;
    else
      gIsBrowserProcess = false;
  }
  return gIsBrowserProcess;
}
/// @}
...
/// M: To get product model
std::string SysInfo::GetBoardModel() {
  char platform_str[PROP_VALUE_MAX];
  __system_property_get("ro.hardware", platform_str);
  LOG(INFO)<<"[GetBoardModel] === ["<<platform_str<<"]";
