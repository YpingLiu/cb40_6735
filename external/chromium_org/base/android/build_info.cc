...
/// M: parse the device model @{
void BuildInfo::ParseDeviceModel() {

  std::string product_model = base::SysInfo::GetBoardModel();
  LOG(INFO)<<"[ParseDeviceModel] model=["<<product_model<<"]";
  bool success = true;
  char product_num[12];
  memset(product_num, '\0', 12);
  if (!product_model.compare(0, 2, "mt")) {
    for (size_t i = 2; i < product_model.size(); ++i) {
      const char c = product_model[i];
      if (isdigit(c))
        product_num[i - 2] = c;
      else {
        success = false;
  ...
  if (success) {
    sprintf(device_prefix_, "%s", "mt");
    device_num_ = atoi(product_num);
  } else {
    sprintf(device_prefix_, "%s", "unknown");
    device_num_ = 0;
  }
}
/// @}
