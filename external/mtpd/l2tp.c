...
static void send_packet()
{
    ...
    int send_count = send(the_socket, outgoing.buffer, outgoing.length, 0);
    if((send_count == -1)||(send_count < outgoing.length))
    	log_print(FATAL, "send_packet() should send %d, but it really sends %d, error:%s", outgoing.length , send_count, strerror(errno));
    acknowledged = 0;
}
...
static void send_ack()
{
    ...
    int send_count = send(the_socket, buffer, ACK_SIZE, 0);
    if((send_count == -1)||(send_count < ACK_SIZE))
    	log_print(FATAL, "send_ack() should send %d, but it really sends %d, error:%s", ACK_SIZE , send_count, strerror(errno));  
}
...
static int recv_packet(uint16_t *session)
{
    ...
    if (incoming.length == -1) {
        if (errno == EINTR) {
        	log_print(FATAL, "recv_packet() errno == EINTR");
            return 0;
        }
...
static int l2tp_timeout()
{
    ...
    int send_count = send(the_socket, outgoing.buffer, outgoing.length, 0);
    if((send_count == -1)||(send_count < outgoing.length))
    	log_print(FATAL, "l2tp_timeout() should send %d, but it really sends %d, error:%s", outgoing.length , send_count, strerror(errno));
    return TIMEOUT_INTERVAL;
}
