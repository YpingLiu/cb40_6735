...
ssize_t readAtDRM(EAS_HW_DATA_HANDLE hwInstData, EAS_FILE_HANDLE file, off_t offset, void *data, size_t size) {
    ...
    if (file->mDrmBuf != NULL && file->mDrmBufSize > 0 && offset >= file->mDrmBufOffset && (offset + size) <= (file->mDrmBufOffset + file->mDrmBufSize)) {
        ...
        if (file->mDrmBufSize > 0) {
            ...
            ALOGE("readAtDRM: file [%p], failed to read any valid data from DRM file. mDrmBufSize [%lld]",
                    file, file->mDrmBufSize);
        ...
        if (result <= 0) {
            ALOGE("readAtDRM: file [%p], failed to read any valid data from DRM file. result [%ld]",
                    file, result);
        }
        return result;
    }
    ...
    for (i = 0; i < EAS_MAX_FILE_HANDLES; i++)
    {
        ...
        if (file->fd < 0)
        {
...
#ifdef MTK_DRM_APP
            ALOGV("EAS_HWOpenFile ----> file [%p] mDrmManagerClient [%d] mDecryptHandle [%d] mDrmBufOffset [%lld] mDrmBufSize [%lld] mDrmBuf [%d]",
                    file, file->mDrmManagerClient, file->mDecryptHandle, file->mDrmBufOffset, file->mDrmBufSize, file->mDrmBuf);
            ...
            if (file->fd) {
                ...
                ALOGE("EAS_HWOpenFile invalid fd & path");
            }
            ...
            if (1 == isDcf) { // check dcf first so we can avoid create DrmManagerClient unnecessarily.
                ...
                if (file->mDecryptHandle != NULL) {
                    ...
                    ALOGD("EAS_HWOpenFile ----> file [%p], valid key, realFileSize [%ld]", file, realFileSize);
                    if (realFileSize >= 0) {
                        file->fileSize = realFileSize;
                    }
                } else {
                    ALOGD("EAS_HWOpenFile ----> file [%p], invalid key.", file);
            ...
            file->mDrmBufSize = 0;
            file->mDrmBuf = NULL;
            ALOGV("EAS_HWOpenFile [result] ----> file [%p] mDrmManagerClient [%d] mDecryptHandle [%d] mDrmBufOffset [%lld] mDrmBufSize [%lld] mDrmBuf [%d]",
                    file, file->mDrmManagerClient, file->mDecryptHandle, file->mDrmBufOffset, file->mDrmBufSize, file->mDrmBuf);
#endif
...
EAS_RESULT EAS_HWDupHandle (EAS_HW_DATA_HANDLE hwInstData, EAS_FILE_HANDLE file, EAS_FILE_HANDLE *pDupFile)
{
    ...
    for (i = 0; i < EAS_MAX_FILE_HANDLES; i++)
    {
        ...
        if (dupFile->fd < 0)
        {
            ...
            ALOGV("EAS_HWDupHandle: file [%p] dupFile [%p] mDrmManagerClient [%d] mDecryptHandle [%d] mDrmBufOffset [%lld] mDrmBufSize [%lld] mDrmBuf [%d]",
                    file, dupFile, dupFile->mDrmManagerClient, dupFile->mDecryptHandle, dupFile->mDrmBufOffset, dupFile->mDrmBufSize, dupFile->mDrmBuf);
#endif
...
EAS_RESULT EAS_HWCloseFile (EAS_HW_DATA_HANDLE hwInstData, EAS_FILE_HANDLE file1)
{
...
#ifdef MTK_DRM_APP
    ALOGV("EAS_HWCloseFile ----> file [%p] mDrmManagerClient [%d] mDecryptHandle [%d] mDrmBufOffset [%lld] mDrmBufSize [%lld] mDrmBuf [%d]",
            file1, file1->mDrmManagerClient, file1->mDecryptHandle, file1->mDrmBufOffset, file1->mDrmBufSize, file1->mDrmBuf);
    ...
    file1->mDrmBufOffset = 0;
    file1->mDrmBufSize = 0;
    ALOGV("EAS_HWCloseFile [result] ----> file [%p] mDrmManagerClient [%d] mDecryptHandle [%d] mDrmBufOffset [%lld] mDrmBufSize [%lld] mDrmBuf [%d]",
            file1, file1->mDrmManagerClient, file1->mDecryptHandle, file1->mDrmBufOffset, file1->mDrmBufSize, file1->mDrmBuf);
#endif
