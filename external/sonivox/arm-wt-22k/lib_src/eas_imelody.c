...
static EAS_RESULT IMY_Event (S_EAS_DATA *pEASData, EAS_VOID_PTR pInstData, EAS_INT parserMode)
{
    ...
    // if ENDTOKEN is recognized, the state is EAS_STATE_PRESTOPPING
...
#ifdef MTK_AOSP_ENHANCEMENT
                    else // the case is actually infinite loop
                    { 
...
//3 We need a different mode for metadata parsing where we don't loop at all
            ...
            case '@':
#ifdef MTK_AOSP_ENHANCEMENT
                // modify the return type to EAS_RESULT, if it is EAS_TOKENEND, means "END:IMELODY" is parsed and ready to set STATE_PRESTOPPING
            ...
            case 'l':
                ...
                break;
#ifdef MTK_AOSP_ENHANCEMENT
            // combine the vibrator and volume control to one case 
...
#endif
#ifdef MTK_AOSP_ENHANCEMENT
    // if state is STATE_PRESTOPPING, means "END:IMELODY" is parsed, set STATE_PRESTOPPING and release till next IMY_EVENT
    if(pData->state != EAS_STATE_PRESTOPPING)
    {
    ...
    for (i = 0; i < 6; i++)
    {
        ...
        switch (i)
        {
            ...
            default:
                ...
                {
                    EAS_INT j;
                    // rollback the pointer of pData if it is not 'ibeo' string
                    for (j = i; j >=0; j--)
                        PutBackChar(pData);
                    return EAS_FALSE;
                }
...
static EAS_RESULT IMY_GetVolume (EAS_HW_DATA_HANDLE hwInstData, S_IMELODY_DATA *pData, EAS_BOOL inHeader)
{
    ...
    if (IsDigit(c))
        ...
        PutBackChar(pData);
    // add: c==0, if not inHeader, it means TOKEN_END
    else if(!inHeader)
        result = EAS_TOKENEND;

    if ((temp >= 0) && (temp <= 15))
    {
// remove the original design: Ignoring V0 in header
#if 0
        if (inHeader && (temp == 0))
    ...
    *temp = 0;
    ...
    for (;;)
    {
        ...
        else
        {
            if (c)
                PutBackChar(pData);
            else if(!inHeader) // add: c==0, if not inHeader, it means TOKEN_END
                ok = EAS_TOKENEND;
...
#endif
// add the handling of exceeds 75 characters in a line
//    LOGV("IMY_ReadLine last i = %d",i);
// remove the setting 2010/05/25
#if 0
    if( i == MAX_LINE_SIZE )
    {
        if ((result = EAS_HWGetByte(hwInstData, fileHandle, &c)) == EAS_SUCCESS)
