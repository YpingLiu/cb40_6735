...
#endif
//#define MTK_SKIP_TAILING_DELTA_TIME_ON_SYS_EVENT
#endif
        ...
        if (parserMode == eParserModeMetaData)
        {
...
#ifdef MTK_SKIP_TAILING_DELTA_TIME_ON_SYS_EVENT
...
#endif
            {
                ...
                if(pSMFData->time_TmpMsgs != 0)
                    LOGV("SMF_Event SMF_UpdateTime done: pSMFData->time = %lld, time_TmpMsgs = %lld, new ticks = %ld, old ticks = %ld",
                         pSMFData->time, pSMFData->time_TmpMsgs, pSMFData->nextStream->ticks - ticks, ticks);
...
#ifdef MTK_AOSP_ENHANCEMENT
    // add the condition of stop state
...
static EAS_RESULT SMF_GetDeltaTime (EAS_HW_DATA_HANDLE hwInstData, S_SMF_STREAM *pSMFStream)
{
...
#ifdef MTK_AOSP_ENHANCEMENT
    {
        ...
        // do check if next event is channel messages
...
static EAS_RESULT SMF_ParseEvent (S_EAS_DATA *pEASData, S_SMF_DATA *pSMFData, S_SMF_STREAM *pSMFStream, EAS_INT parserMode)
{
    ...
    else
    {
        ...
        if (parserMode == eParserModeMetaData) {
            ...
            count = 0;
            pData = data;
            EAS_HWMemSet(data, 0, sizeof(data));
        }
#endif
...
EAS_RESULT SMF_ParseHeader (EAS_HW_DATA_HANDLE hwInstData, S_SMF_DATA *pSMFData)
{
...
#ifdef MTK_AOSP_ENHANCEMENT
    // move to here, we must allocate pSMFData in case of following EAS_Close
    ...
    for (i = 0; i < pSMFData->numStreams; i++)
    {
        ...
        pSMFData->streams[i].ticks = 0;
#ifdef MTK_AOSP_ENHANCEMENT
        pSMFData->streams[i].flag_isChannelMsg = 0;
#endif
