...
static EAS_RESULT EAS_ParseEvents (S_EAS_DATA *pEASData, EAS_HANDLE pStream, EAS_I64 endTime, EAS_INT parseMode)
{
    ...
    while (!done)
    {
        ...
        if ((parserState > EAS_STATE_PLAY)&&(parserState != EAS_STATE_PRESTOPPING))
        {
            ...
            {
                // add condition except iMelody, because iMelody play before updating time, should use current time
                //smf  updating time before playing, should use previous available time 
                //iMelody: read current time
                LOGD("EAS_ParseEvents EAS_FILE_IMELODY time = %lld, currentTime = %lld, pStream->time = %lld parserState = %ld",
                     time, currentTime, pStream->time, parserState);
                pStream->time = currentTime << 8;
            }
            // other format, keep original code
        ...
        // add for forcely stop the duration processing
...
repeat:     if ((result = (*pParserModule->pfTime)(pEASData, pStream->handle, (EAS_U32 *)&time)) != EAS_SUCCESS)
            {
...
EAS_PUBLIC EAS_RESULT EAS_ParseMetaData (EAS_DATA_HANDLE pEASData, EAS_HANDLE pStream, EAS_I32 *playLength)
{
    ...
    VMInitWorkload(pEASData->pVoiceMgr);
#ifdef MTK_AOSP_ENHANCEMENT
    // enlarge the maximum ticks for total duration calculation
...
EAS_PUBLIC EAS_RESULT EAS_Locate (EAS_DATA_HANDLE pEASData, EAS_HANDLE pStream, EAS_I32 milliseconds, EAS_BOOL offset)
{
    ...
    /* add: Initialize and Set Reverb effect Parameters to default*/
