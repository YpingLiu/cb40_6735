...
mDNSexport mStatus mDNS_Init(mDNS *const m, mDNS_PlatformSupport *const p,
    ...
    m->timenow_adjust = (mDNSs32)(0x90000000 + (unsigned int) mDNSRandom(0x0FFFFFFF));

	timenow = mDNS_TimeNow_NoLock(m);

    syslog(LOG_WARNING, "[mDNS_Init] timenow = %d; timenow_adjust = %d; mDNSPlatformRawTime() = %d", 
           timenow, m->timenow_adjust, mDNSPlatformRawTime());
