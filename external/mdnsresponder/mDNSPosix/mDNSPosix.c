...
// for NEWLINK event
struct timeval gNewlinkStartTime;
...
// be torn down and rebuilt, return affected indices as a bitmask. Otherwise return 0.
	...
	if ((currtv.tv_sec - gNewlinkStartTime.tv_sec) < (3 * 60)) {
		syslog(LOG_WARNING, "[ProcessRoutingNotification] currtv.tv_sec - gNewlinkStartTime.tv_sec = %d",
				currtv.tv_sec - gNewlinkStartTime.tv_sec);
		result = 0;
	}

	syslog(LOG_WARNING, "[ProcessRoutingNotification] return result = 0x%X; nlmsg_type = 0x%X; RTM_GETLINK = 0x%X; RTM_NEWLINK = 0x%X; RTM_DELADDR = 0x%X; RTM_NEWADDR = 0x%X",
			result, pNLMsg->nlmsg_type, RTM_GETLINK, RTM_NEWLINK, RTM_DELADDR, RTM_NEWADDR);
