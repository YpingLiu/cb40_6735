...
mDNSlocal mStatus MainLoop(mDNS *m) // Loop until we quit.
	...
	for (; ;)
		...
		if (!gotData)
            ...
            syslog(LOG_WARNING, "[MainLoop] ticks = %d", ticks);
			if (ticks < 1) ticks = 1;
			}
...
int main(int argc, char **argv)
	...
	// for NEWLINK event
	gettimeofday(&gNewlinkStartTime, NULL);
	syslog(LOG_WARNING, "[main] gNewlinkStartTime.tv_sec = %d", gNewlinkStartTime.tv_sec);
